...
func TestRecord_AllFieldsZeroValueWellDefined(t *testing.T) {
	...
	if !r.StartTime.IsZero() {
		t.Errorf("StartTime zero-value not zero: %v", r.StartTime)
	}
	if r.Method != "" {
		t.Errorf("Method zero-value not empty: %q", r.Method)
	}
	if r.Path != "" {
		t.Errorf("Path zero-value not empty: %q", r.Path)
	}
	if r.Protocol != "" {
		t.Errorf("Protocol zero-value not empty: %q", r.Protocol)
	}
	if r.ResponseCode != 0 {
		t.Errorf("ResponseCode zero-value not 0: %d", r.ResponseCode)
	}
	if r.BytesSent != 0 {
		t.Errorf("BytesSent zero-value not 0: %d", r.BytesSent)
	}
	if r.Duration != 0 {
		t.Errorf("Duration zero-value not 0: %v", r.Duration)
	}
	if r.Authority != "" {
		t.Errorf("Authority zero-value not empty: %q", r.Authority)
	}
	if r.UserAgent != "" {
		t.Errorf("UserAgent zero-value not empty: %q", r.UserAgent)
	}
	if r.UpstreamHost != "" {
		t.Errorf("UpstreamHost zero-value not empty: %q", r.UpstreamHost)
	}
}
...
func TestSink_InterfaceImplementation(t *testing.T) {
	...
	if cs.recs[0].Method != "GET" {
		t.Errorf("captured Method = %q, want GET", cs.recs[0].Method)
	}
}
