...
func testBuildCompiledConfigParseReject(t *testing.T) {
	cases := []struct {
		...
		// ---- §5.1 RATIFIED-from-config Arm 4: grpc_success_status > 16 codes ----
		{
			...
			mutate: func(ac *acv3.AdmissionControl) {
				codes := make([]uint32, 20) // clearly more than 16
				for i := range codes {
					codes[i] = uint32(i)
				}
...
func TestIsHTTPSuccess(t *testing.T) {
	...
	// I3: end==600 acceptance boundary — [500,600) is a valid range (end==600 allowed
	// as the exclusive upper bound per isValidHTTPRange; code 599 is included, 600 is not).
	ac3 := validConfig()
	ac3.GetSuccessCriteria().HttpCriteria = &acv3.AdmissionControl_SuccessCriteria_HttpCriteria{
		HttpSuccessStatus: []*typev3.Int32Range{
			{Start: 500, End: 600},
		},
	}
	cc3, err3 := buildCompiledConfig(toAny(t, ac3))
	if err3 != nil {
		t.Fatalf("end==600 range [500,600) should be ACCEPTED, got PARSE-REJECT: %v", err3)
	}
	if cc3 == nil {
		t.Fatalf("expected non-nil *compiledConfig for end==600 range")
	}
	if !cc3.isHTTPSuccess(599) {
		t.Errorf("isHTTPSuccess(599) with [500,600): want true, got false")
	}
	if cc3.isHTTPSuccess(600) {
		t.Errorf("isHTTPSuccess(600) with [500,600) [exclusive end==600]: want false, got true")
	}
}
