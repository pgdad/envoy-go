...
func TestLeastRequest_FirstDrawnWinsTies(t *testing.T) {
	...
	ep, _, err := lr.Pick(0, false, SubsetMatch{}, false)
	if err != nil {
		t.Fatalf("Pick: %v", err)
	}
	if ep.Host != "a" {
...
func TestLeastRequest_WithReplacementNoClamp(t *testing.T) {
	...
	if _, _, err := lr.Pick(0, false, SubsetMatch{}, false); err != nil {
...
func TestLeastRequest_IncDecBalance(t *testing.T) {
	...
	_, release, _ := lr.Pick(0, false, SubsetMatch{}, false)
...
func TestLeastRequest_NoEndpoints(t *testing.T) {
	...
	_, release, err := lr.Pick(0, false, SubsetMatch{}, false)
...
func TestLeastRequest_SkewAvoidsLoadedEndpoint(t *testing.T) {
	...
	held := make([]func(), 0, 3)
	for i := 0; i < 3; i++ {
		_, rel, _ := lr.Pick(0, false, SubsetMatch{}, false)
	...
	ep, rel, _ := lr.Pick(0, false, SubsetMatch{}, false)
	rel()
	if ep.Host == "a" {
...
func TestNewLeastRequest_ProductionRNGSeeds(t *testing.T) {
	...
	for i := 0; i < 50; i++ { // concurrency smoke (mutex-guarded rng)
		wg.Add(1)
		go func() { defer wg.Done(); _, rel, _ := lr.Pick(0, false, SubsetMatch{}, false); rel() }()
	}
	wg.Wait()
