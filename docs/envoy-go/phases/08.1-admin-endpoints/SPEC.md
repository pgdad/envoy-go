...
### 7.1 Equivalence claims (per BRAINSTORM §2.6, §7.2 — refined per §11 empirical findings)

The fixture exercises all four new endpoints under a controlled load and asserts per-endpoint equivalence under per-endpoint tolerance:

- **`/config_dump`:** parse both bodies as JSON via `encoding/json.Unmarshal`. Recursively compare with the §13.2 allow-list applied: `bootstrap.node.user_agent_name`, `bootstrap.node.user_agent_build_version`, `bootstrap.node.extensions[]`, `<*ConfigDump>.last_updated` are allow-listed (envoy-go emits empty / partial values for these; Envoy emits auto-populated values). Three-envelope ordering (Bootstrap, Listeners, Clusters) is asserted in both. The `dynamic_*` arrays are absent in both (`EmitUnpopulated: true` does NOT cause empty arrays to appear because the upstream proto's `oneof` for the dynamic resources is `nil` when no dynamic resources exist — verified in §11.1).
- **`/clusters`:** parse both bodies into `(cluster_name, key, value)` tuple sets (one tuple per `<cluster>::<key>::<value>` line); compare set-equally with tolerance for hot-path counters. The `cx_total`, `cx_connect_fail`, `rq_total`, `rq_active`, `rq_error` counters are allowed `±N` tolerance where N is the round-robin-distributed delta of the 5-request load (in practice, both endpoints will see ≥1 connection-attempt; the tolerance is `abs(envoy_go - envoy) <= 1` for a 5-request load with round-robin LB across 2 endpoints — a defensive 1-unit tolerance).
- **`/listeners`:** body byte-equal modulo the framing deviation (envoy emits chunked; envoy-go emits Content-Length). The differential harness's existing dechunk preprocessor handles this; once dechunked, byte-equality is asserted.
- **`/server_info`:** parse both bodies as JSON. Recursively compare with the §13.2 allow-list applied: `version`, `uptime_current_epoch`, `uptime_all_epochs`, `command_line_options.*` (subset-on-envoy-go side; Envoy emits ~40 fields, envoy-go emits 1), `hot_restart_version`, `node.*` (same allow-list as `/config_dump`). The `state` field MUST be byte-equal (`"LIVE"` on both sides).
