...
2. **Two new HTTP filters under `internal/filter/http/`:**
   - **`cors/`** — real Envoy filter (`envoy.filters.http.cors`); decode-side detects preflight (`OPTIONS` + `Origin` + `Access-Control-Request-Method`) and synthesizes a `200 OK` preflight response via `SendLocalReply` (per §11.2 empirical pin; BRAINSTORM §2.4's hypothesized `204 No Content` was empirically corrected against reference Envoy v1.37.2); encode-side appends `Access-Control-Allow-Origin` / `Access-Control-Expose-Headers` / `Access-Control-Allow-Credentials` per the resolved per-route config. ~150 LoC + ~200 LoC tests.
