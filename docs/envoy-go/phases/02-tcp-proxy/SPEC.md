...
- **`internal/cluster/loadbalancer.go`** — `LoadBalancer` interface (`Pick([]Endpoint) (Endpoint, error)`) and the round-robin implementation. The round-robin uses a per-cluster `atomic.Uint64` counter; `Pick` returns `endpoints[counter.Add(1) % uint64(len(endpoints))]`. A 1-indexed counter makes the first pick `endpoints[1 % len(endpoints)]`, which for `len>=2` is not `endpoints[0]` — that's fine, the project's contract is *uniform distribution*, not *sequence starts at index 0* (the round-robin SPEC note in §2 and the BEHAVIOR_CONTRACT subsection make this explicit).
...
- **`test/fixtures/0001-tcp-proxy-rr/`** — new fixture directory. Contents:
  ...
  - `envoy.yaml` — equivalent reference bootstrap. 1 listener on `0.0.0.0:15000`. 1 STATIC cluster with 3 `lb_endpoints`, each at `host.docker.internal` with distinct ports (backends are host-side; see ADR-0010 for the `dns_lookup_family: V4_ONLY` constraint, which applies here too but per endpoint — actually STATIC uses literal socket_addresses and `host.docker.internal` needs STRICT_DNS or an `/etc/hosts` entry resolved through `extra_hosts`; since the reference uses STATIC the addresses must be literal IPs, which would diverge per-runner). The fixture's reference bootstrap therefore uses STRICT_DNS (same as fixture 0000) with multiple endpoints. The subject's bootstrap uses STATIC with three literal 127.0.0.1 entries (subject-on-host). This is a benign divergence in the *config shape*, not in behaviour; it is documented in the fixture README on the same discipline as fixture 0000's STATIC-vs-STRICT_DNS divergence.
  ...
  - `driver/driver.go` — fixture driver. `SubjectConfig` renders the bootstrap with injected ports. Exposes `ReferenceBootstrap() string` with the STRICT_DNS reference config (three backend port placeholders substituted by the runner). `Drive(ctx, refAddr, subjAddr)` sends 9 TCP round-trip requests against each address (`helpers.TCPRoundTrip`), capturing per-backend accept counts from a shared `*atomic.Uint64[3]` tracker that each backend increments on Accept. Returns the concatenated response bytes for differential comparison. The driver also exposes a post-run assertion `AssertDistribution(refCounts, subjCounts [3]uint64) error` that checks each proxy's distribution independently.
