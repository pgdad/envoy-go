...
- **`internal/cluster/loadbalancer_test.go`** — unit tests for round-robin: distribution over N=30 picks with 3 endpoints yields exactly 10/10/10 (the mod-index is deterministic); correctness under concurrent `Pick` from 100 goroutines × 30 picks each = N=3000 picks yields exactly 1000/1000/1000 (because `atomic.Uint64.Add(1)` guarantees each goroutine observes a unique `i` and the full i-range `[0, N)` mod 3 is exactly balanced when 3 | N). No tolerance needed.
...
4. Resolve `cluster` name against `cm.Get(clusterName)`. Missing → error `tcpproxy: cluster %q not found`. The resolved `*cluster.Cluster` pointer is stored on the `Filter` struct and consulted by `Handle` for endpoint-picking and connect-timeout lookup.
...
| Concurrent `PickEndpoint` producing non-uniform distribution under high load. | The `atomic.Uint64` counter is exact and each goroutine observes a unique `i`; mod-indexing is deterministic. Unit test covers 100 goroutines × 30 picks each = 3000 picks and asserts distribution is exactly 1000/1000/1000 (no tolerance — the `i`-range `[0, 3000)` mod 3 balances exactly). |
...
- [ ] Commit message follows `BOOTSTRAP_PROMPT.md` §5.3 format (`phase NN: <title> [ADR-NNNN, …]`) and references the ADRs introduced or referenced.
