...
# IMPL record (2026-08-06)

**Stage:** IMPL (lifecycle-state `3` -> `DONE`). **ROW 83 FLIPPED `in-progress` -> `done`; THE PHASE IS CLOSED.** Base master **`1c93c6ad`** (from `git rev-parse master`, **not** a SHA quoted in the router), branch `phase-83-impl`. Sentinel `want` **STAYED 115** — the flip is IN PLACE per §Schema `:18`, so the `ROADMAP.md` diff is **exactly one line**.

## What landed

**EIGHT task commits**, subagent-driven, each committing locally only, squashed at close. All nine scope items: **S1** the two body Pause arms · **S2** the two trailers Pause arms · **S3** generation counters with the guard **ahead of the CAS** · **S4** the census replacement, RE-INVERTED · **S5/S6** the doc surface (10 s kept, derivation replaced) · **S7** the de-hardcoded test port · **S8** a non-parking status · **S9** disposed inside S3 via the disarm bump.

Realized **+3536 net `.go`** (production +202, test +3334 across the eight commits) against the PLAN's **~1950** budget.

## The stage's headline

⚠️ **THE ROW CLOSED A *LIVE* PRODUCTION LEAK THAT THE BRAINSTORM, THE SPEC AND THE PLAN ALL MISSED.** S8 was chartered as **one** park; the PLAN widened it to **three**; the S2 task found a **fourth**; the real roster is **ELEVEN `TerminateStream` arms across THREE status axes**. The one that matters is `EncodeHeaders`' captured-local-response arm, which parks `RunEncodeHeaders` — **an iterator all three HCM dispatchers drive**. Its own comment claimed the return *"prevents the response from continuing through the chain"*; measured, it **parked and did not return in 3 s**. ⚠️ **It was found only because S8's remedy forced a census of the whole status surface rather than of the one arm the charter named** — a scope item's *remedy* can be a better detector than its *diagnosis*.

⚠️ **SECOND: THE §6.1 LoC TRIGGER IS CROSSED AGAINST A BAND THAT WAS ITSELF MEASURED.** The PLAN replaced the SPEC's analogue-scaled figure with measured prototypes precisely to stop the phase-81/82 overruns; the IMPL overran anyway, **~1.8x**. ⚠️ **`reference_measured_prototype_is_a_lower_bound` has now fired on FOUR CONSECUTIVE ROWS, and measuring the prototype did NOT neutralise it — the cause is under-ENUMERATION, not under-SCALING, and a prototype measures only the items someone thought to enumerate.** Two tasks refuted their own cost lines for exactly that reason: one production figure came in at **5.4x** because 41 of its 43 lines were a comment the task text mandated and the estimate priced at **zero**; another at **2.0x** because the PLAN's own file table put a signature's call sites in a *later* task than the signature change, **so the two would not compile apart**. **DISPOSITION: RECORDED, NOT RETRO-SPLIT.**

⚠️ **THIRD: THE ROW SHIPS TWO STATEMENTS NO GATE COVERS, AND NAMES BOTH.** The watchdog log-string correction is **UNGATED** — 22 test files in the package, **zero** log-capture idioms, against a firing control of 5 on the non-test side; a gate is constructible (`var logf = log.Printf` is swappable) and uncharered. And, discovered only at the gate stage, the **`Add(1)`-before-`Store`** ordering is a correctness constraint with **no probe that catches its violation**.

⚠️ **FOURTH: THE PLAN'S OWN BREAK ROSTER WAS WRONG ABOUT WHICH BREAKS REDDEN.** It recorded all three S3 single-arm breaks as green, making the stacked arm *stacked by necessity*. Measured at the landed tip, **the guard-alone break REDDENS at 400/400 on both sides.** The error is diagnosable and worth keeping: **the PLAN's only S3 instrument was the fire-count harness, which does stay green under that break** — the probes that catch it were written afterwards. ⇒ **A break roster is a claim about the gates that existed when it was written, and it goes stale the moment a later task adds a probe.**

## Refutation count: **THIRTY-SEVEN**, of which **TWELVE are load-bearing**

Beyond the four above: **the PLAN's Task-2 code block does not compile** (it logs an identifier with zero hits repo-wide, owned by a later task) · **CAS-first was COMPLETELY UNDETECTED** by the chartered gate (448 PASS / 0 FAIL — `Stop()` cancels superseded timers ~99 ms before their deadline, so none ever runs), and a deterministic constructed-overlap probe was built to catch it · **the encode mirror would have shipped UNGATED** (the chartered harness is decode-only and the existing encode test pins the watchdog to an hour) · **a THIRD pacing boundary the PLAN omits** — `n*pace` must exceed the watchdog, or **all four 2x2 cells read 1**, a fully vacuous gate that reads as a clean pass · **the PLAN's mandated `pace == watchdog` was REPLACED** because its GREEN arm is a ~60 µs race, not a structure · **five newly-false comments were minted by this row's OWN earlier tasks**, one carrying a claim refuted **forty lines below it in the same file** · **`0036` (n)'s stated dependency is INVERTED** and it is the entire stated basis for the original freeze · **the S6 upper bound is additionally false about the GUEST** — the cited crate *does* call the resume hostcall, which no phase-83 document said · **a step-1 break is NOT EXECUTABLE at this tip**, so the vacuity was reproduced faithfully at the pre-S1 commit rather than quoted · **BOTH framework doubles are required** or the census reports three encode arms broken on a *correct* tree.

⚠️ **AND A NEGATIVE CONTROL WAS ITSELF BROKEN BEFORE IT WAS FIXED.** The NC written to prove the structural stat-surface assertion selected a commit whose **path-filtered** diff contained none of the pattern — which reads exactly like *"the pattern never fires"*. It was trusted only after a commit whose *filtered* diff genuinely carries it was found (8 matches by the byte-identical command). **A negative control is a claim too.**

## Gates — an IMPL owes (a)-(f) as REAL RESULTS

**(a)/(b)** full differential **120/120, `INNER_EXIT=0`, 384.311 s** (baseline 388.961 s), 0 FAIL/SKIP, 0 `no driver registered`, 0 panic/race/segv, fixture set-difference EMPTY, denominator **120** on the faithful `^[0-9]{4}[a-z]?-` predicate (a bare `^[0-9]{4}-` gives 118). ⚠️ **LAUNCH 1 ABORTED — `INNER_EXIT=1`, a panic on `0084-otlp-access-log` (`bind: address already in use`), 85 PASS + 1 FAIL of 120 — WHILE THE SURROUNDING TOOLING REPORTED SUCCESS. Only the explicitly captured inner status exposed it.** `0036` alone **19.28 s subtest / 19.355 s package**, against the **19.33-20.36 s envelope** (the subtest sits 0.05 s under the floor). ⚠️ **THE DIFFERENTIAL IS STRUCTURALLY BLIND TO THIS ROW — the row touches ZERO fixture files and the suite COULD NOT have gone RED on it. The green is evidence the row broke nothing, NOT that it works.**
**(c)** proxy-wasm conformance **all 10 families PASS — 10 of the cpp-host's 16 (62.5%), 6 deferred**; measured matches chartered.
**(d)** **VACUOUS** — this row adds no fuzzer (**55**, `-- '*.go'`-scoped; unrestricted gives 161, which is the wrong number).
**(e)** `go test ./...` **`INNER_EXIT=0`, 126 ok / 0 FAIL**; `-race` as a SECOND run **125 ok / 1 FAIL, `DATA RACE` count 0** — the single failure is a panic on `0082-grpc-access-log-buffering`, the **pre-existing driver-receiver port race** (a *different* fixture from launch 1's), and **isolate-re-run under `-race`: both PASS**. The three named known flakes did not fire. h2spec **53 tests / 53 passed / 0 skipped / 0 failed — my own denominator is 53**, one report, no reference arm; **it matches the phase-82 IMPL and does NOT reproduce the phase-82 PLAN's "106 passed"**. `gofmt -l` gated on OUTPUT and empty; `golangci-lint` exit 0 / 0 lines **with the misspell liveness proof fired and byte-identical restore verified by sha256 at every code task** — and misspell fired UNFORCED on three occasions. Stat surface **+0**, asserted STRUCTURALLY over the production diff with a corrected negative control; **1205 stays 1205 and no ledger entry is owed**.
**(f)** `REVIEW.md` **ABSENT — the STANDING LINEAGE DEPARTURE**, named not claimed: **37 of 124** phase dirs carry one, **87 do not**, and none since 25.3.

## Sentinel

Input measured **231 lines / 115 data rows** first. **(1) SILENT — every ROADMAP row is now `done`.** ⚠️ **THIS IS THE STAGE AT WHICH SILENCE BECAME INDISTINGUISHABLE FROM A BROKEN CHECK, so the doctored-copy NC stopped being optional: row 62 doctored => `NOT DONE: row 62`, with `NC LANDED? [ in-progress ]` INSPECTED BEFORE the result was trusted.** **(2) FIVE, UNCHANGED — the THIRTY-NINTH consecutive phase without a decrease; this row narrowed nothing and that is STRUCTURAL.** **(3) `NEVER OPENED: gRPC` — ALONE.** The condition is a CONJUNCTION, so **the sentinel does NOT fire** and `stop` was **NOT** created. **FIVE negative controls, ALL FIRED.** **Leak check by BEFORE/AFTER COUNT, not by diff-line grep** — the changed row already carried a check-(3) matcher string, so a naive `grep` over the `+` line reads as a leak; the check-(2) union, the `-family row` total, the line count and the data-row denominator are all **INVARIANT**.

## Handoff

**The phase is CLOSED.** Next: **SELF-PICK the next subject** per the 2026-07-12 standing directive; no banked mid-lifecycle work remains. ⚠️ **Carry the two uncovered statements** (the log-string gate, constructible but uncharered; and the `Add`/`Store` ordering probe) and ⚠️ **the two-seam trailers finding** — a trailers-paused guest is blind because the header-map router sends the trailer map types to its default arm **and** the trailers dispatchers never capture the trailers at all, **so any future row must price BOTH seams**.
