...
## Task 13 — fixture 0020 expectations + README + end-to-end differential pass + §18.P11 closure

**Objective:** Close the fixture with `expectations.yaml` + `README.md`,
run the 7-scenario differential to PASS, run the full 21-fixture suite to PASS,
and formally close the §18.P11 RATIFIED-PENDING-IMPL-TIME pin.

### Iteration 1 — first differential run

First run: `go test -v -count=1 -timeout=180s ./test/differential/ -run 'TestDifferential/0020'`

```
[ref] scenario 1: status=200 body=<echo_json>
[ref] scenario 2: status=0  body=""    ← FAIL: connection error
[ref] scenario 3: status=503 body=""
...
[subj] scenario 2: status=0  body=""   ← FAIL: connection error
[subj] scenario 3: status=0  body=""   ← FAIL: connection error
```

Scenarios 2 and 3 failed with `status=0` (connection error / timeout) on the
subject side, and scenario 2 failed on the reference side as well (the reference
container's auth cluster also timed out because the in-process auth server wasn't
reachable from inside Docker before the timeout — addressed separately).

**Root cause (subject side — scenarios 2 + 3):**

The async goroutine calls `SendLocalReply(...)` but the dispatch goroutine remains
blocked forever in `parkDecode` (waiting on `decodeResumeCh`). `SendLocalReply`
alone sets `c.localReplyDone` but does NOT signal `decodeResumeCh`. Without a
subsequent `ContinueDecoding()` call, the parked goroutine never wakes up and the
connection hangs until the client's timeout fires.

Two missing `ContinueDecoding()` calls identified:

1. `applyDisposition` `dispDeny` case in `extauthz.go`: `SendLocalReply(403,
   "access denied", oh)` without `ContinueDecoding()`.
2. `applyErrorPosture` `failure_mode_allow:false` branch in `extauthz.go`:
   `SendLocalReply(int(cc.statusOnError), "", nil)` without `ContinueDecoding()`.

The phase-09 fault filter (`fault.go:321-324`) is the canonical precedent showing
the required `SendLocalReply + ContinueDecoding()` pattern.

**Fix applied:**

`internal/filter/http/extauthz/extauthz.go`:

- `dispDeny` case in `applyDisposition`:
  Added `f.dcb.ContinueDecoding()` after `f.dcb.SendLocalReply(...)` with a
  detailed comment explaining the `parkDecode` unblocking requirement.

- `applyErrorPosture` `failure_mode_allow:false` branch:
  Added `f.dcb.ContinueDecoding()` after `f.dcb.SendLocalReply(...)` with the
  same explanatory comment pattern.

**Unit test updates (`extauthz_test.go`):**

Two unit tests had `want=0` assertions on `ContinueDecoding` call count for the
deny/error paths (written before the fix, expecting the old buggy behavior):
- `TestDecodeHeaders_AsyncDeny_SendLocalReply`: changed `want 0` → `want 1`.
- `TestDecodeHeaders_AsyncError_FailureModeAllow_False`: changed `want 0` → `want 1`.

### Unit test run after fix

```
$ go test -race -count=1 ./internal/filter/http/extauthz/...
ok  github.com/esalaine/envoy-go/internal/filter/http/extauthz  1.309s
```

All extauthz unit tests PASS.

### Iteration 2 — all 7 scenarios PASS

```
$ FIXTURE_0020_DUMP_BYTES=1 FIXTURE_0020_DUMP_STATS=1 \
  go test -v -count=1 -timeout=180s ./test/differential/ -run 'TestDifferential/0020'

[ref] scenario 1: status=200 body=<echo_json>   x-authz-result="allowed"
[ref] scenario 2: status=403 body="access denied"
[ref] scenario 3: status=503 body=""
[ref] scenario 4: status=200 body=<echo_json>   x-envoy-auth-failure-mode-allowed="true"
[ref] scenario 5: status=200 body=<echo_json>
[ref] scenario 6: status=200 body=<echo_json>
[ref] scenario 7: status=200 body=<echo_json>

[subj] scenario 1: status=200 body=<echo_json>   x-authz-result="allowed"
[subj] scenario 2: status=403 body="access denied"
[subj] scenario 3: status=503 body=""
[subj] scenario 4: status=200 body=<echo_json>   x-envoy-auth-failure-mode-allowed="true"
[subj] scenario 5: status=200 body=<echo_json>
[subj] scenario 6: status=200 body=<echo_json>
[subj] scenario 7: status=200 body=<echo_json>

--- PASS: TestDifferential/0020-http-ext-authz-http (1.91s)
```

ALL 7 SCENARIOS PASS. Byte-stream diff = EQUAL.

### Empirical counter scrape (Task-13 confirmation)

Reference Envoy v1.37.2:
```
envoy_http_ext_authz_ok{hcm_local_a}          = 3
envoy_http_ext_authz_denied{hcm_local_a}       = 1
envoy_http_ext_authz_error{hcm_local_b}        = 1
envoy_http_ext_authz_error{hcm_local_c}        = 1
envoy_http_ext_authz_failure_mode_allowed{hcm_local_c} = 1
envoy_http_ext_authz_invalid{all}              = 0
envoy_http_ext_authz_disabled{all}             = 0
```
...
Cross-side equivalence: CONFIRMED. Summed totals: ok=3, denied=1, error=2,
failure_mode_allowed=1, invalid=0. Match the plan-time hypotheses in the
driver's `AssertStats` comment.

Note: `lookupExtAuthzCounter` concern from Task 12 (summation across three
`hcm_local_{a,b,c}` labels) confirmed working correctly — the helper sums all
matching metric lines regardless of label value.

### Full differential suite

```
$ go test -count=1 -timeout=600s ./test/differential/
ok  github.com/esalaine/envoy-go/test/differential  58.791s
```

All 21 fixtures (0000-0020) PASS.
...
### §18.P11 RATIFIED — deny-path header-ordering CONFIRMED

SPEC §5.P11 states: "auth-supplied/decision headers FIRST, Envoy housekeeping
(content-length, date, server: envoy) AFTER."

**Implementation (`internal/filter/http/extauthz/check.go` `buildDenyHeaders`):**

1. Extract `allowedClient`-matched headers from the auth HTTP response (decision
   headers, e.g. `x-authz-denied: true`).
2. Append `content-type: text/plain` fallback ONLY IF no content-type is present
   in the decision headers (housekeeping last).

`headerKVToOrderedHeaders` in `extauthz.go` preserves insertion order when
passing the slice to `SendLocalReply`. Framework housekeeping (`content-length`,
`date`, `server: envoy`) is appended by the downstream chain framework AFTER the
decision headers set by `SendLocalReply`.

**Task-13 empirical confirmation (scenario 2 differential):**

Reference Envoy v1.37.2 scenario 2 response:
```
...
Decision header `x-authz-denied` appears BEFORE framework housekeeping
(`content-length`, `date`, `server`) on BOTH sides. Differential byte-stream
verdict for scenario 2: EQUAL (status=403 body=ok). **§18.P11 CONFIRMED —
RATIFIED.**

**Verdict:** §18.P11 deny-path header-ordering RATIFIED at Task 13 — the
`buildDenyHeaders` + `headerKVToOrderedHeaders` + `SendLocalReply` pipeline
produces the SPEC §5.P11-compliant wire shape confirmed byte-equivalent to
reference Envoy v1.37.2 at the fixture-harness differential.

**Commit SHA:** `093e5a6`

---

## Task 14 — BEHAVIOR_CONTRACT 6-edit bundle + ROADMAP row 18.1 done + STATE advance + 6-gate verification

**Status:** COMPLETE

**Notes:** Applied the full BEHAVIOR_CONTRACT.md 6-patch bundle, flipped ROADMAP row 18.1 from `in-progress` to `done`, advanced STATE.md to `18.2-ext-authz-grpc` active-phase, and passed all 6 phase-done gates.

### BEHAVIOR_CONTRACT.md 6-patch bundle

**§13.1 — New `### envoy.filters.http.ext_authz` subsection:** Inserted AFTER `### envoy.filters.http.jwt_authn` (landing-chronological convention; D10 fallback fires — alphabetical-after-csrf was the PLAN spec, but BEHAVIOR_CONTRACT.md is landing-chronological; insertion point after jwt_authn@17). Subsection covers: field decomposition table (consumed vs deferred); HTTP-outbound auth-check (ADR-0159 disposition (b)); request-side header filtering (ADR-0160); bidirectional header-mutation (ADR-0161); request body inclusion (ADR-0162); failure mode/error posture; per-route 5th-canonical REUSE + SHARED-stats (ADR-0163); wire shapes (allow/deny/error); `response_code_details` NOT emitted; stat surface + Prometheus rendering; gRPC 18.2 forward-pointer. INSERTION NOTE documenting the landing-chronological fallback from the PLAN D10 alphabetical hypothesis.

**§13.2 — Stat table 71 → 77:** Added 6 new `ext_authz.*` counter rows after the jwt_authn block. Updated running total from 71 to **77 internal names**: `(17 from 06.1 + 5 from 09 + 4 from 11 + 3 from 12 + 0 from 13 + 17 from 14 + 14 from 15 + 4 from 16 + 7 from 17 + 6 from 18.1)`. `disabled` counter documented as STRUCTURALLY UNREACHABLE under MVP.

**§13.3 — Equivalence matrix row 0020-http-ext-authz-http:** Added after the 0019-http-jwt-authn row: `0020-http-ext-authz-http | envoy.filters.http.ext_authz (decode-side external-authorization gate; HTTP service mode; 5th-canonical per-route REUSE + SHARED-stats; ADR-0156/0157/0159/0160/0161/0162/0163) | byte-exact status; byte-exact body on allow paths (echo passthrough) AND deny paths (verbatim auth-service body — scenario 2 "access denied" 13B); response_code_details NOT asserted (envoy-go MVP defers per SPEC §2.8 + §8 item 10); cross-side counter-delta equivalence on 5 actively-emitted counters (ok, denied, error, failure_mode_allowed, invalid); disabled counter STRUCTURALLY UNREACHABLE, not asserted; SHARED per-route stats per ADR-0163; per-route disabled:true arm (scenario 6) + check_settings arm (scenario 7) exercised; deny-path header-set ordering confirmed RATIFIED per §18.P11 at Task 13`.

**§13.4 — Phase 18.1 forward-pointer notes:** Appended `### Phase 18.1 forward-pointer notes` under `## Forward-pointer notes`. Covers 11 deferral items: `allowed_client_headers_on_success` (item 1); full gRPC AttributeContext (item 2, deferred to 18.2); grpc_service activation (item 3, deferred to 18.2); `filter_enabled`/`filter_enforced` runtime gates (item 4); deferred proto fields 5 families (item 5); `include_peer_certificate`/`include_tls_session` (item 6, 18.2); `encode_raw_headers` (item 7, 18.2); `context_extensions` HTTP-mode no-op (item 8); `dynamic_metadata_from_headers` (item 9); `response_code_details` emission joint divergence-window (item 10); access-log integration (item 11). Plus `response_code_details` joint divergence-window note (phases 16+17+18.1 cumulative). Plus `internal/httpclient/` generalization forward-pointer (ADR-0159 disposition (b), oauth2-trigger). Plus no-new-canonical note (5th-canonical-REUSE; `grep -cE '^\*\*\(xiv\)\*\*' docs/envoy-go/DECISIONS.md = 0` confirmed). Plus no-new-framework-primitive note (ext_authz-local per ADR-0159; see `## HTTP outbound auth-check framework note` above).

**§13.6 — Per-route canonical patterns cross-reference:** Added new `## Per-route canonical patterns cross-reference (ADR-0125 roster; updated through phase 18.1)` section — table showing all 8 canonicals (phases 07.1/09/11/13/14/15/16/17; FIRST §9 REUSE at 18.1). ext_authz noted as FIRST §9 REUSE of 5th canonical (buffer phase-13/compressor phase-14/ext_authz phase-18.1 → three consumers). Includes authoritative check note: `grep -cE '^\*\*\(xiv\)\*\*' docs/envoy-go/DECISIONS.md = 0`.

**§13.7 — HTTP outbound auth-check framework note:** Created new `## HTTP outbound auth-check framework note (per phase 18.1 ADR-0159)` section between `## JWT verifier framework primitive` and `## Forward-pointer notes`. Documents: NOT a new shared cross-phase primitive (ADR-0159 disposition (b)); structural composition against phase-17 `internal/jwks/Fetcher`; per-request lifecycle (async goroutine, `StopIteration`, `OnDestroy` cancel, `context.Canceled` error mapping); `internal/httpclient/` generalization trigger (THIRD outbound-HTTP consumer = oauth2 → reconsidering with three consumers in view).

**D10 deviation recorded:** BEHAVIOR_CONTRACT.md's `## HTTP filter chain` subsections are landing-chronological, NOT alphabetical. The PLAN D10 spec called for alphabetical insertion after csrf; the actual convention is landing-chronological. ext_authz inserts AFTER jwt_authn@17 (landing-chronological position). The BEHAVIOR_CONTRACT.md insertion note documents this explicitly.

### ROADMAP row 18.1 flip

`in-progress` → `done` (2026-05-15). Summary sharpened with: 15-task PLAN notation; production LoC counts (`extauthz.go` 1087 + `check.go` 411 + `attributes.go` 435 + `doc.go` 192 = **2125 LoC production** + test-helper 217 LoC); final 7-ADR roster (ADR-0156/0157/0159/0160/0161/0162/0163); ADR-0044 escape-valve NOT triggered (ADR-0165 NOT authored; next-free ADR-0165 preserved); NO ADR-0125 amendment paragraph confirmed; 21/21 differential fixtures green; BEHAVIOR_CONTRACT 6-patch bundle noted.

### STATE.md advance

- `active-phase`: `18.2-ext-authz-grpc`
- `lifecycle-state`: `phase 18.1 done; phase 18.2 SPEC pending`
- `next-skill`: `superpowers:writing-plans` (phase-18.2 SPEC-authoring session)
- `last-commit`: `<TBD>` (SHA-fill follow-up per established convention)
- `next-free ADR`: `ADR-0165`
...
**GREEN.** (Lint errors addressed in Tasks 1–13 during active IMPL work: misspell `cancelled`→`canceled` in `check.go`/`extauthz.go`/`extauthz_test.go`/`extauthzhttp.go`; unused `buildDispatchFilter` removed from `extauthz_test.go`; `//nolint:staticcheck` on `ar.GetAllowedHeaders()` in `extauthz.go`/`extauthz_test.go` + `corev3.ApiVersion_V2` in `extauthz_test.go`/`fuzz_test.go`.)

### Gate B: race tests

```
go test -race -count=1 $(go list ./... | grep -v './test/differential')
(all packages including extauthzhttp test helper)   EXIT:0
```
...
**GREEN.** 53/53 PASS at ADR-0051 pin (`summerwind/h2spec@sha256:5f4a65c30cae8569558ced048b4bfe0dcf01a221e36767ae504ccd8348a7aeb0`).

### Gate D: 22 fuzzers × 30s each

```
PASS  FuzzAccessLogFormat
PASS  FuzzConfigDumpFormat
PASS  FuzzBootstrapLoad
PASS  FuzzDrainTransitions
PASS  FuzzHCMConfigParse
PASS  FuzzFrameStream
PASS  FuzzHPACKDecode
PASS  FuzzBandwidthLimitConfigParse
PASS  FuzzBufferConfigParse
PASS  FuzzCompressorConfigParse
PASS  FuzzCsrfPolicyConfigParse
PASS  FuzzExtAuthzConfigParse
PASS  FuzzFaultConfigParse
PASS  FuzzFilterChainParse
PASS  FuzzHeaderMutationConfigParse
PASS  FuzzJwtAuthnConfigParse
PASS  FuzzLocalRateLimitConfigParse
PASS  FuzzRBACConfigParse
PASS  FuzzTcpProxyFilter
PASS  FuzzFilterChainMatch
PASS  FuzzPromTextFormat
PASS  FuzzTLSContextParse
DONE
```

**GREEN.** 22/22 fuzzers PASS at 30s each (0 crashes).

### Gate E: 21 differential fixtures 0000–0020

```
go test -count=1 ./test/differential/ -run 'TestDifferential/000[0-9]'   ok (47.543s)  EXIT:0
go test -count=1 ./test/differential/ -run 'TestDifferential/001[0-9]'   ok (50.170s)  EXIT:0
go test -count=1 ./test/differential/ -run 'TestDifferential/0020'        ok (5.032s)   EXIT:0
```

**GREEN.** 21/21 fixtures PASS (0000–0020; fixture 0020 PASS: TestDifferential/0020-http-ext-authz-http).

### Gate F: BEHAVIOR_CONTRACT bundle verification

```
grep -c "envoy.filters.http.ext_authz" docs/envoy-go/BEHAVIOR_CONTRACT.md           → 4
grep -c "ext_authz\." docs/envoy-go/BEHAVIOR_CONTRACT.md                            → 11
...
grep -cE '^\*\*\(xiv\)\*\*' docs/envoy-go/DECISIONS.md                              → 0  [NO (xiv) amendment — CONFIRMED]
grep -E "71.*77|stat-table 71|71 → 77" docs/envoy-go/BEHAVIOR_CONTRACT.md           → "Total: 77 internal names (17+5+4+3+0+17+14+4+7+6)"
```

**GREEN.** All 6 patches verified; ADR-0125 no-§(xiv) confirmed; stat-table 71→77 confirmed.
...
**Goal:** Author the end-of-phase review document (`REVIEW.md`) following the phase-17 REVIEW.md structural template.  Append Task 15 entry to PROGRESS.md.  Commit both.

### Step 1: REVIEW.md authored

`docs/envoy-go/phases/18.1-ext-authz-http/REVIEW.md` written (~400 lines, 12 sections):

- §1 Phase summary — APPROVED verdict; architectural centerpiece description; ContinueDecoding discovery note; ADR-0044 escape-valve NOT triggered note
- §2 ADR roster — all 7 ADRs (ADR-0156/0157/0159/0160/0161/0162/0163) evaluated VALIDATED with evidence citations
- §3 Empirical pins outcome — all 10 18.1-load-bearing pins documented; 3 RATIFIED-PENDING-IMPL-TIME closures explicitly called out (§18.P6+§18.P7 at Task 8; §18.P11 at Task 13)
- §4 Gate-by-gate evidence — verbatim from PROGRESS.md Task 14 gate outputs
- §5 Acceptance checklist — SPEC §15 all 15 claims verified PASS
- §6 Divergence-window roster — 5 windows (a–e) enumerated
- §7 Framework-delta impact — 1 new primitive (ADR-0159) + 4 reuses (ADR-0076 async-resume; ADR-0128 body-buffering; ADR-0125 5th-canonical per-route; ADR-0085 SendLocalReply); NO new per-phase framework extensions required
- §8 Test counts + verification surface — 184 unit tests; 21/21 fixtures; 22/22 fuzzers; 53/53 h2spec
- §9 Deferred items — 11 items for future phases
- §10 Notable lessons / surprises — 5 lessons (ContinueDecoding bug; failure_mode_allow topology; buildAuthRequest call-site; path="" + headers_to_add bugs; escape-valve unused)
- §11 Parent-rollup note — parent row 18 stays in-progress; next session is 18.2 SPEC via superpowers:brainstorming
- §12 Sign-off — APPROVED; ready for master squash-merge
...
**Commit SHA:** (to be SHA-filled at post-squash follow-up)
