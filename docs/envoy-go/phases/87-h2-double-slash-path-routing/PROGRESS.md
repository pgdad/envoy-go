...
**PLAN** — decompose SPEC §9 (D-87-SEQ: ONE leg, one atomic commit) into ordered IMPL tasks with the RED anchors re-proven at the PLAN tip; draft the contract-edit text (`## HTTP/2` origin-form parity rule at `:2019`ff) and the ADR-0309 completion plan (guard 1 -> 0 at the IMPL); freeze the error-string constraint set (`bad :path` REUSED for the two `ParseRequestURI` rejects; `fragment in :path` NEW; zero landed strings change); re-derive every count.
