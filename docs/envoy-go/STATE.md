...
- **last-commit:** TBD — this brainstorm-merge commit's SHA fills in a follow-up SHA-fill commit per the phase-04..09 SHA-fill convention. Commit message preview: `phase 10 brainstorm: http-filter-header-mutation [ROADMAP row 10; BRAINSTORM.md]`. The brainstorm worktree branch `phase-10-http-filter-header-mutation-brainstorm` (branched from master tip `3066c72`) ff-merges into master at session exit; per the user's persistent preference recorded in `~/.claude/projects/-home-esa-git-envoy-go/memory/MEMORY.md`, master is pushed to origin after merge.
