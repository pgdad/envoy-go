...
- **last-commit:** `75fcfb4` (squash-merge of branch `phase-24-http-filter-global-ratelimit-brainstorm` from worktree `.worktrees/phase-24-http-filter-global-ratelimit-brainstorm/`; this SHA-fill follow-up backfills the `<<TBD-fill-post-squash>>` placeholder per the phase-09..23 BRAINSTORM-stage close pattern). Predecessor master tip: `88101a7` — `next-prompt.txt: advance to post-phase-23-IMPL cold-start`.
