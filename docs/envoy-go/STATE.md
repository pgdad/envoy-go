...
- **last-commit:** `(this 28.2-SPEC squash — SHA filled post-squash)` — `phase 28.2 SPEC: response decoder + latency-threshold counters + per-connection mutex + TCPZKResponder/0048 design + phase-28 rollup plan; ROADMAP 28.2 → in-progress; ADR-0223 §Context §AMEND`. Substantive predecessor on master: the 28.1b-IMPL squash `fdf40ea`.
