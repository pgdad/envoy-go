...
- **last-commit:** `fdf40ea` — `phase 28.1b IMPL: read-side seam (readChainConn + chainRuntime.replayRead + Buffer.TotalAppended re-base) under the shared R1 wrap predicate; 0046-zookeeper-requests re-enabled + cross-side GREEN all 7 arms + R4; 0047-zookeeper-boot-reject; completion bundle ADR-0221 both-seams + ADR-0222 bodies + BEHAVIOR_CONTRACT 136→337; six gates GREEN LIVE (49/49 differential incl. 47-dir R1 gate, h2spec 53/53, proxy-wasm 10/10) [ADR-0221,ADR-0222]`. Substantive predecessor on master: the 28.1b-PLAN squash `eff30e8`.
