...
- **NEW lb-policy reject text (envoy-go-strict DEPARTURE).** `RING_HASH`/`MAGLEV` are rejected at config-parse with `cluster: %q: unsupported lb_policy %s (supported: ROUND_ROBIN, LEAST_REQUEST, RANDOM)` — the supported-list extended at phase 35 (RANDOM retired from the rejected set → the THIRD accepted policy; see `### Load balancer — random (RANDOM)` below). This is a DEPARTURE: the reference parse-ACCEPTS `RING_HASH`/`MAGLEV` (and ACCEPTED RANDOM before phase 35 landed it). No fixture pins the text (it lands unit-level in `manager_test.go`).
...
- LB policies other than ROUND_ROBIN, LEAST_REQUEST, and RANDOM — load-balancing family (LEAST_REQUEST landed at phase 34; RANDOM landed at phase 35; the 6 remaining candidates {ring_hash, maglev, subset LB, locality-weighted LB, priority load balancing, panic thresholds} stay deferred — see `### Load balancer — least_request (LEAST_REQUEST)` + `### Load balancer — random (RANDOM)` below).
