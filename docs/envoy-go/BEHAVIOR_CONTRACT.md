...
#### Phase 25.1 forward-pointer notes
...
**Deferred items — 25.2 BRAINSTORM scope hand-off:**

- **Full advanced-bridge surface delta** (per parent §10 + phase-25.1 SPEC §2.11) — body+buffer hostcalls (4: `proxy_get_buffer_status` + `proxy_get_buffer_bytes` + `proxy_set_buffer_bytes` + the buffer-type roster for HttpRequestBody + HttpResponseBody + HttpCallResponseBody) + trailers callbacks (`proxy_on_request_trailers` / `proxy_on_response_trailers`) + timer dispatch (`proxy_set_tick_period_milliseconds` + `proxy_on_tick`) + metrics (`proxy_define_metric` / `proxy_increment_metric` / `proxy_record_metric` / `proxy_get_metric` — plugin-defined dynamic stats in `wasm.<plugin_name>.<custom_name>` namespace) + shared data (`proxy_set/get_shared_data` — cross-stream + cross-plugin within the same `vm_id`) + outbound HTTP dispatch (`proxy_dispatch_http_call` / `proxy_on_http_call_response` — RE-CONSUMES phase-20 `internal/httpclient/` per ADR-0177 third-or-later co-consumer; validates the phase-20 framework extraction) + foreign-function call (`proxy_call_foreign_function` with empty default registry per AMEND-A9) + full stream-info surface (`proxy_get_property` for upstreamHost / upstreamCluster / downstreamSslConnection / requestedServerName / filterState extensions). Anticipated 25.2 ADRs: ADR-0206 (`internal/wasm/` 25.2 ABI extensions — body-bridge + dispatch surface) + ADR-0207 (`internal/filter/http/wasm/` 25.2 package extensions + mixed-mode fixture discipline) + escape-valve ADRs (ADR-0208 + ADR-0209 if 2-slot buffer fires). Likely +4 envoy-go-strict counters (`tick_invocations` + `http_call_dispatched` + `http_call_response` + `foreign_function_denied`); project total advances 119 → ~123 at 25.2 phase-done.
- **Per-module wazero Runtime pool design (ADR-0205 escape-valve)** — D-P4 R8 STANDS WEAK-default at 25.1 IMPL (`ns/op = 61000` ~61µs/stream << 1ms threshold per parent §13-R8); ADR-0205 NOT consumed; carries forward to 25.2 BRAINSTORM as the 25.2 IMPL escape-valve slot. 25.2 may re-evaluate against the body+buffer + advanced bridge surface (which adds more bridge methods + more per-stream allocation); if the per-stream construction cost crosses the 1ms threshold there, ADR-0205 fires at 25.2 IMPL with the "per-module wazero Runtime pool with pre-instantiated entries" decision.
- **Foreign-function registry surface (per AMEND-A9)** — at 25.1 `proxy_call_foreign_function` is stub-Unimplemented; at 25.2 the host registration interface lands with EMPTY default registry per AMEND-A9 + ADR-0202 §Decision. Operators register host-side foreign functions via a NEW `wasm.RegisterForeignFunction(name string, fn ForeignFunctionFn)` API at boot; unregistered names return `WasmResult::NotFound` (=1) to the guest. Default-registry = empty = envoy-go-strict departure (upstream registers a default `compress`/`decompress`/`expr_create`/`expr_evaluate`/`expr_delete` set at `source/extensions/common/wasm/foreign.cc`).

**Deferred items — 25.3 BRAINSTORM scope hand-off:**

- **Per-route `typed_per_filter_config` shape (per AMEND-A3)** — at 25.1 `WasmPerRoute` is PARSE-REJECT-by-absence; at 25.3 the SPEC empirically pins against v1.37.2 + go-control-plane v1.32.4. **AMEND-A3 confirms ABSENCE-DEFINITIVE — ADR-0125 STAYS at 10 canonicals; NO §(xvi) AMENDMENT.** The 5th-canonical REUSE-by-absence pattern (mirrors phase-20 oauth2 + phase-21 adaptive_concurrency + phase-23 admission_control). If the SPEC scrape SURFACES a `WasmPerRoute` proto with novel shape (low probability per AMEND-A3 evidence), escalates to NEW 11th canonical + ADR-0125 §(xvi) AMENDMENT 10 → 11 + ADR-0210 anchor.
- **Multi-plugin-per-VM (`vm_id`-keyed VM sharing)** — at 25.1 each `PluginConfig` constructs a fresh per-stream VM; at 25.3 multiple PluginConfig entries with the same `vm_id` share a single VM instance, instantiating distinct plugin contexts via `proxy_on_context_create(streamCtxID, rootCtxID=<plugin-specific>)` (mirrors upstream Envoy's `Wasm Service` discipline scoped to the HTTP filter).
- **`VmConfig.environment_variables` activation** — at 25.1 the WASI `environ_get` / `environ_sizes_get` shims return zero entries; at 25.3 the SPEC settles whether to honor the env-var inject envelope at VM start (anticipated YES per parent §3.0).
- **`VmConfig.fail_open` semantics** — at 25.1 `fail_open` is DEFERRED + the failure_policy=FAIL_RELOAD reload disposition is also DEFERRED; at 25.3 the SPEC settles fail-open (plugin failure short-circuits to filter-bypass instead of 500 fail-closed) + the per-module VM-reload state machine that consumes the deferred `vm_reload_*` Group-C 3-counter triplet.
- **Conformance harness seed at `test/conformance/proxy-wasm/`** per AMEND-A8 + parent §7.3 — runs proxy-wasm spec v0.2.1 conformance bytecode against envoy-go subject-only at the AMEND-A8 starting threshold (62.5% — 10/16 test families per `proxy-wasm-cpp-host@da3ce05d:test/` empirical pin); pass threshold + gate disposition settled at 25.3 SPEC. INDEPENDENT of the differential harness.
