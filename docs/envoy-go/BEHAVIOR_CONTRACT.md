...
- **NEW lb-policy reject text (envoy-go-strict DEPARTURE).** `MAGLEV` (and the remaining `CLUSTER_PROVIDED`/`LOAD_BALANCING_POLICY_CONFIG`) is rejected at config-parse with `cluster: %q: unsupported lb_policy %s (supported: ROUND_ROBIN, LEAST_REQUEST, RANDOM, RING_HASH)` — the supported-list extended at phase 36.1 (RING_HASH retired from the rejected set → the FOURTH accepted policy; see `### Load balancer — ring_hash (RING_HASH)` below). `MAGLEV` is now the LONE recorded departure among the hash policies: the reference parse-ACCEPTS `MAGLEV` (the second consistent-hash policy — it reuses this phase's ADR-0235 PICK-INPUT seam unchanged when it lands), so envoy-go's continued rejection is the DEPARTURE. No fixture pins the text (it lands unit-level in `manager_test.go`, whose `TestManager_Error_UnsupportedLBPolicy` doubly-hit-RETARGETED its trigger from `Cluster_RING_HASH` — now accepted — to `Cluster_MAGLEV`, the phase-34/35 accept-of-N-forces-retarget-to-N+1 pattern).
...
- LB policies other than ROUND_ROBIN, LEAST_REQUEST, RANDOM, and RING_HASH — load-balancing family (LEAST_REQUEST landed at phase 34; RANDOM landed at phase 35; RING_HASH landed at phase 36.1; the **5** remaining candidates {maglev, subset LB, locality-weighted LB, priority load balancing, panic thresholds} stay deferred — maglev the cheap follow-on reusing the ADR-0235 hash-input seam unchanged — see `### Load balancer — least_request (LEAST_REQUEST)` + `### Load balancer — random (RANDOM)` + `### Load balancer — ring_hash (RING_HASH)` below). The 36.2 HTTP route `hash_policy` plane (header + connection-properties source_ip) is the second producer against the ADR-0235 seam and lands at the 36.2 IMPL.
