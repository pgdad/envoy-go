...
### 60-name table (introduced by phase 06.1; extended by phase 09; extended by phase 11; extended by phase 12; UNCHANGED in phase 13; extended by phase 14; extended by phase 15; extended by phase 20)
...
- **`Lua.SourceCodes` multi-script map activation** — arm 4 PARSE-REJECT lifts at 22.3; multi-script lookup via the `SourceCodes` map enables per-route delegation to named scripts. Stays PARSE-REJECT at 22.2 with the byte-stable wording UNCHANGED from 22.1 (`"lua: source_codes map is not yet supported (lands in phase 22.3)"`).
- **`LuaPerRoute` 3-arm oneof override** — arm 18 PARSE-REJECT lifts at 22.3; NEW 9th canonical per-route shape per ADR-0125 §(xiv) AMENDMENT body landing at 22.3 IMPL final Task (3-arm hybrid: `disabled-bool` + `string-reference-delegation` + `DataSource-wholesale-override`). Stays PARSE-REJECT at 22.2 with the byte-stable wording UNCHANGED from 22.1 (`"lua: per-route configuration is not yet supported (lands in phase 22.3)"`).
- **Per-route 3-tier dispatch** — listener-default → SourceCodes-named-script → per-route DataSource override; settled at 22.3 SPEC.
- **`*LState`-pool design (ADR-0193 escape-valve)** — D-P10 R6 STANDS WEAK-default at 22.2 IMPL Task 15 (`ns/op = 98157` ~98µs/stream at FULL bridge surface; only 1.4× the 22.1 baseline of ~70µs; well under 1ms threshold). ADR-0193 NOT consumed at 22.2 phase-done; carries forward to 22.3 BRAINSTORM as the 22.3 IMPL escape-valve slot. 22.3 may re-evaluate against the multi-script + per-route surfaces (which add per-route `*Chunk` lookup + per-route resolution); if the per-stream construction cost crosses the 1ms threshold there, ADR-0193 fires at 22.3 IMPL with the `*LState`-pool design.
- **Multi-decoder-filter "park-headers-iteration-pending-body" cooperative discipline** — the Task 19a "Continue on body-yield" trade-off documented above is a known limitation for multi-decoder-filter topologies that depend on body-after-yield header-mutation visibility. Future framework phase (or 22.3) introduces a cooperative discipline that parks headers iteration pending body-yield resume so subsequent decoders see the post-:body() mutated headers. NOT a 22.2 scope item (no in-tree multi-lua-decoder topology at 22.2).
...
## Per-route canonical patterns cross-reference (ADR-0125 roster; updated through phase 22.1; 9th canonical AMENDMENT-anticipation paragraph anchored at parent SPEC commit per ADR-0125 §(xiv) — body lands at 22.3 IMPL final Task)
