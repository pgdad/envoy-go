...
Phase 19.1 ships `envoy.filters.http.ext_proc` in **headers-stages-only mode** (Envoy v1.37.2 canonical external-processor filter — both-decode-and-encode-side; delegates per-stage mutation decisions for both the request AND the response to an external service over a bidirectional gRPC stream OR a JSON-transcoded HTTP POST per stage) per the canonical Envoy v1.37.2 filter spec under the 07.1 framework. envoy-go's MVP envelope is the TWELFTH §9 family-row (after cors @ 07.1, fault @ 09, header_mutation @ 10, local_ratelimit @ 11, csrf @ 12, buffer @ 13, compressor @ 14, bandwidth_limit @ 15, rbac @ 16, jwt_authn @ 17, ext_authz @ 18.1+18.2). It is the FIRST §9 family-row to ship BOTH `StreamDecoderFilter` AND `StreamEncoderFilter` participation in a single filter package (phase-14 compressor's encode-only is the only prior encode-side precedent; the BOTH-DECODE-AND-ENCODE shape is anchored at ADR-0167). Body-stage activation lands at phase 19.2 (sibling sub-phase); 19.1 PARSE-REJECTs `request_body_mode` / `response_body_mode` `!= NONE` (BUFFERED lifts at 19.2; STREAMED + BUFFERED_PARTIAL + FULL_DUPLEX_STREAMED PARSE-REJECT envoy-go-strict permanently). Trailer-modes `!= SKIP` PARSE-REJECT permanently. Three STREAMED-only top-level flags (`observability_mode`, `send_body_without_waiting_for_header_response`, `deferred_close_timeout != 0`) PARSE-REJECT permanently per parent §5.P10. **8 anchored ADRs** (ADR-0167..ADR-0174): ADR-0167 (package shape + BOTH-decode-encode filter + 9-counter filterStats + multi-stage `SendLocalReply`); ADR-0168 (compiledConfig + grpc_service-vs-http_service mutex + body-mode 19.1 PARSE-REJECT); ADR-0169 (`*ProcessorClient` bidi-stream wrapper EXTENDING the phase-18.2 `*Dialer` — FIRST cross-phase consumer of ADR-0158); ADR-0170 (filter-local `ProcessingRequest`/`ProcessingResponse` JSON codec); ADR-0171 (header-mode ProcessingMode state machine + mode-override + max_message_timeout); ADR-0172 (header_mutation + ImmediateResponse-at-headers + clear_route_cache/route_cache_action + grpc-status content-type sniff); ADR-0173 (per-route 5th-canonical REUSE + SHARED-stats + 9-counter stat surface; NO ADR-0125 amendment — SECOND CONSECUTIVE §9 REUSE after phase 18.1); ADR-0174 (symmetric `EncoderFilterCallbacks` extension — 6 new methods mirroring ADR-0165; reuses chain-field plumbing). **D12 hypothesis HELD** at 19.1 IMPL phase-done — NO impl-time-unanticipated ADR fired; ADR-0177 stays unconsumed (reserved for 19.2).
...
| `processing_mode` (#3) | CONSUMED per parent §5.P9. `HeaderSendMode.DEFAULT (0)` → SEND for header-modes; SKIP for trailer-modes. `BodySendMode != NONE` PARSE-REJECT in 19.1 (BUFFERED activates at 19.2 per ADR-0168 §Decision AMENDMENT). `HeaderSendMode != SKIP` on trailer-modes PARSE-REJECT permanently. |
...
| `failure_mode_allow` (#7) / `message_timeout` (#10) / `max_message_timeout` (#15) / `disable_immediate_response` (#16) | Error-posture surface per parent §5.P11. `failure_mode_allow:true` + processor error → stream continues with `HeaderContinue` + `failure_mode_allowed` counter increment (also increments `streams_failed`). `message_timeout` (proto default 200ms; PGV `[0s, 1h]`) — per-stage message timer per ADR-0171 §Decision (vi); `max_message_timeout` GATES `override_message_timeout` enablement (when 0, override API disabled; when ≥1ms, override is range-checked against `[1ms, max_message_timeout]`). `disable_immediate_response:true` → `ImmediateResponse` ProcessingResponses classified as `spurious_msgs_received` + dropped. **Per-message timer enforcement is STRUCTURAL-ONLY at 19.1** per ADR-0172 §Decision (vii) — `msgCtx` is built but NOT bound to `stream.Send`/`stream.Recv` (gRPC ClientStream contract); full pre-emption-on-timeout lands at 19.2. |
...
## Per-route canonical patterns cross-reference (ADR-0125 roster; updated through phase 19.1)

*This section summarizes the ADR-0125 per-route canonical pattern roster as of phase 19.1. ADR-0125 governs `typed_per_filter_config` resolution; each canonical was introduced or extended at a specific phase.*
