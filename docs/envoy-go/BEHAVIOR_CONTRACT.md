...
*Introduced by phase 32.1 (`network-filter-redis-upstream-pool-seam-and-codec`) — the first half of the SIXTH §9 Network-filters-family row (a parent pre-split row 32 with sub-rows 32.1 + 32.2 per ADR-0106(d); after the phase-26 family-parent, the phase-27 `sni_cluster` row, the phase-28 `zookeeper_proxy` parent, the phase-29 `mongo_proxy` parent, and the phase-31 `kafka_broker` flat row; the phase-30 pin-refresh was an infra row). The project's FIRST terminal routing proxy. Justified by ADR-0229 (the redis_proxy filter umbrella — 32.1 half ACCEPTED). Cross-side-PROVEN by the now-green `0055-redis-roundtrip` (PING local-reply arm + proxied SET/GET round-trip arm + downstream-RESP byte-equivalence prong + cross-side `StatsAsserter` on the flat `GET /stats` endpoint) + `0056-redis-boot-reject` fixtures.*
...
**Forward-pointers.** 32.2 (`network-filter-redis-commands-and-stats`) — PLANNED. Lands: the full single-key command set + the per-command/splitter/REDIS_CLUSTER_STATS stat roster (the EAGER ~180-command table + 2 `splitter.*` + 3 REDIS_CLUSTER_STATS names) + the 4 gauges' inc/dec + the `redis.` LABEL-HOISTED Prometheus tag-extractor arm at `internal/stats/name.go` + the differential command matrix (extending `0055`) + the 41st fuzzer `FuzzRESPDecode` + the parent-row-32 ROLLUP. Stat surface 546 → ~1091.
