...
*Introduced by phase 39.1 (active HTTP health checking; the keystone leg of the phase-39 by-checker split — 39.1 = the state+runtime+HTTP-checker, 39.2 = the TCP + gRPC checkers reusing the 39.1 substrate). Justified by ADR-0242 (the host health-state dimension + the cluster-level checker runtime) + ADR-0243 (the health-aware LB pick — Approach A, the build-time-injected health view + panic threshold). The project's FIRST per-host MUTABLE state + the FIRST cluster-level BACKGROUND runtime.*
...
**The 6-arm PARSE-REJECT roster (envoy-go's own, no PGV — the 38.2 precedent; byte-stable; all config-build/boot rejects; unit-level).** OUTCOME-parity with the reference per ADR-0080; the IMPL pins exact byte-stable strings:
...
- **`tcp_health_check` / `grpc_health_check` checker** — the 39.1 DEPARTURE-reject (see below).

**Departures + coverage boundaries:**
- **The +7 stats (emitted ONLY on clusters with `health_checks`; surface 1125 → 1132).** Four `cluster.<name>.health_check.{attempt,success,failure,network_failure}` (counters, Inc'd per probe per the ADR-0242 runtime) + `health_check.healthy` (gauge — count of healthy hosts per checker) + `membership_healthy` (gauge) + `lb_healthy_panic` (counter — Inc'd by the ADR-0243 health-aware pick when the healthy fraction is strictly `< healthy_panic_threshold`). Existing fixtures (no `health_checks`) are unaffected. The `0066` `StatsAsserter` cross-equals the deterministically-converging subset (`membership_healthy`, `health_check.attempt`/`success`/`failure`). See the `## Stat-name mapping` Phase 39.1 block.
- **The tcp/grpc-checker 39.1 DEPARTURE-reject.** A `tcp_health_check` / `grpc_health_check` checker is DEPARTURE-rejected at config-build as an unsupported checker in 39.1. The reference parse-ACCEPTS TCP (and gRPC, which additionally requires cluster H2 — `cluster must support HTTP/2 for gRPC healthchecking`); envoy-go over-rejects both vs the reference within the ADR-0080 OUTCOME framing (a boot-reject rather than runtime behavior). Recorded here; LIFTED at 39.2 (the TCP + gRPC checkers reuse the 39.1 substrate).
...
- **The poll-to-convergence differential posture.** The `0066-health-check-http` fixture (cross-side) uses a poll-to-converge + warmup determinism shape (poll `/stats` on both sides until `membership_healthy == 2` and `health_check.failure ≥ 1` — no fixed `time.Sleep`; then a load phase asserting 100% of traffic lands on the LIVE backends, the dead host filtered) over endpoints = {2 LIVE HTTP backends, 1 DEAD unbound endpoint} (66% healthy > 50% → asserts FILTERING, not panic). 20/20 flake-free.
- **NO new fuzzer / NO new BackendKind.** The health checker decodes no untrusted wire bytes (the probe targets are validated xDS config; the probe response is a status-code classification) → fuzzers STAY 42. `0066` reuses the HTTP backends → tail STAYS 33. ZERO new packages (`health.go` in the existing `internal/cluster`) + ZERO new go.mod deps (`HealthCheck` is in the pinned `/envoy v1.32.4`; the HTTP probe reuses the H1/H2 client).
- **Deferred surface (recorded departures).** TCP + gRPC checkers (→ 39.2); degraded / draining states (`HealthStatus` DEGRADED/DRAINING — MVP HEALTHY/UNHEALTHY only); jitter / interval variants (`no_traffic_interval`, the jitter fields); EDS `health_status`; health-check event logging; the deferred stats (`health_check.{passive_failure,verify_cluster,degraded}`, `membership_{change,degraded,excluded}`, `update_no_rebuild`). Outlier (passive) detection — a future Upstream-robustness row — will feed the SAME `clusterHealth` registry as a passive input. The 3 health-gated Load-balancing-family candidates (locality-weighted LB, priority load balancing, panic thresholds) are now UNBLOCKED — the health dimension they depend on exists.
...
- Health-check-driven endpoint selection — **PARTIALLY LANDED at phase 39.1** (active HTTP health checking; see `### Cluster — active health checks`). Active HC is cluster-scoped, so a tcp_proxy upstream cluster carrying `health_checks` now filters unhealthy hosts. Still deferred for the tcp_proxy plane specifically: nothing extra (the cluster LB pick is shared) — but the TCP + gRPC checker codecs land at 39.2.
