...
- **NEW lb-policy reject text (envoy-go-strict DEPARTURE).** `RANDOM`/`RING_HASH`/`MAGLEV` are rejected at config-parse with `cluster: %q: unsupported lb_policy %s (supported: ROUND_ROBIN, LEAST_REQUEST)` — the ONE deliberate byte-stable-reject-text change this phase (it replaces the phase-02 "only ROUND_ROBIN lb_policy supported" text). This is a DEPARTURE: the reference parse-ACCEPTS those three policies. No fixture pins the text (it lands unit-level in `manager_test.go`).
...
- LB policies other than ROUND_ROBIN and LEAST_REQUEST — load-balancing family (LEAST_REQUEST landed at phase 34; the 7 remaining candidates {random, ring_hash, maglev, subset LB, locality-weighted LB, priority load balancing, panic thresholds} stay deferred — see `### Load balancer — least_request (LEAST_REQUEST)` below).
