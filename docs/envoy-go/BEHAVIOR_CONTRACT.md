...
- **Stats:** 23 eager fixed stats (22 counters + the `op_query_active` gauge) + the dynamic `cmd.*`/`collection.*`/callsite families. Request-side counters increment at 29.1; the response-side `op_reply*`/`op_command_reply` + the gauge inc/dec/drain are increment-active at 29.2 (+0 creation); `cx_drain_close` + `delays_injected` + the `cx_destroy_*` close-direction value parity at 29.3. Project stat surface **337 → 360** at 29.1 (+23), then **360 → 360** at 29.2 (zero creation delta — increment-wiring only; see the `## Stat-name mapping` 337 → 360 + 360 → 360 blocks).
...
- **Forward-pointers.** **29.2 (ADR-0225) — DONE.** The OP_REPLY/OP_COMMANDREPLY response decoder in `OnWrite`, requestID↔responseTo correlation, the `op_query_active` gauge inc/dec/drain, and `emit_dynamic_metadata` all LANDED (see the response-side bullets above). **29.3 (ADR-0226) — forward:** the async halt/resume seam; fault-delay injection (`delays_injected`); the mongo access log; `cx_drain_close`; the DEFERRED `cx_destroy_*_with_active_rq` close-direction value parity (D-P4 — the close-direction accessor + the direction-keyed increment); fixture `0052-mongo-fault-delay`; the parent-row-29 ROLLUP.
