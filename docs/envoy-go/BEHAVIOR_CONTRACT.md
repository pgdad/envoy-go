...
- **Forward-pointers.** **29.2 (ADR-0225) — DONE.** The OP_REPLY/OP_COMMANDREPLY response decoder in `OnWrite`, requestID↔responseTo correlation, the `op_query_active` gauge inc/dec/drain, and `emit_dynamic_metadata` all LANDED. **29.3 (ADR-0226) — DONE.** The async halt/resume seam (see the NEW `### Network filter chain framework — async halt/resume (29.3 amendment)` subsection below), fault-delay injection (`delays_injected`), the mongo access log, `cx_drain_close`, and the `cx_destroy_*_with_active_rq` close-direction VALUE parity (D-P4 CLOSED) all LANDED (see the 29.3 bullets above); fixture `0052-mongo-fault-delay`; the parent-row-29 ROLLUP (the FOURTH §9 Network-filters family row CLOSED). **The §9 family stays OPEN — 3 candidates remain (`redis`/`kafka_broker`/`thrift`).**
