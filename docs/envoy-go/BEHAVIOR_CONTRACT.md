...
| `cluster.<n>.ring_hash_lb.size` | gauge | `envoy_cluster_ring_hash_lb_size{envoy_cluster_name="<n>"}` (RING_HASH-only; Set once at ring build, = total ring points) |
| `cluster.<n>.ring_hash_lb.min_hashes_per_host` | gauge | `envoy_cluster_ring_hash_lb_min_hashes_per_host{envoy_cluster_name="<n>"}` (RING_HASH-only; Set once at ring build, = smallest per-host ring-entry count) |
| `cluster.<n>.ring_hash_lb.max_hashes_per_host` | gauge | `envoy_cluster_ring_hash_lb_max_hashes_per_host{envoy_cluster_name="<n>"}` (RING_HASH-only; Set once at ring build, = largest per-host ring-entry count) |
| `cluster.<n>.maglev_lb.min_entries_per_host` | gauge | `envoy_cluster_maglev_lb_min_entries_per_host{envoy_cluster_name="<n>"}` (MAGLEV-only; Set once at table build, = smallest per-host table-entry count = `floor(M/N)`) |
| `cluster.<n>.maglev_lb.max_entries_per_host` | gauge | `envoy_cluster_maglev_lb_max_entries_per_host{envoy_cluster_name="<n>"}` (MAGLEV-only; Set once at table build, = largest per-host table-entry count = `ceil(M/N)`) |
| `cluster.<n>.lb_subsets_active` | gauge | `envoy_cluster_lb_subsets_active{envoy_cluster_name="<n>"}` (subset-only; Set once at build to the distinct-subset count — ×1, NOT the reference's 33× artifact) |
| `cluster.<n>.lb_subsets_created` | counter | `envoy_cluster_lb_subsets_created{envoy_cluster_name="<n>"}` (subset-only; Add once at build to the distinct-subset count — ×1, NOT the reference's 33× artifact) |
| `cluster.<n>.lb_subsets_selected` | counter | `envoy_cluster_lb_subsets_selected{envoy_cluster_name="<n>"}` (subset-only; +1 per subset hit from `subsetLB.Pick`) |
| `cluster.<n>.lb_subsets_fallback` | counter | `envoy_cluster_lb_subsets_fallback{envoy_cluster_name="<n>"}` (subset-only; +1 per fallback path from `subsetLB.Pick`) |

The three `ring_hash_lb.*` gauges are registered + Set ONCE at ring build, and ONLY when `lb_policy: RING_HASH` (the `registerClusterMetrics` type-asserts `*ringHashLB`; reference parity). They are STATIC (the ring is immutable) and CROSS-SIDE-EXACT — keyed only on `(minimum_ring_size, hash_function, host count, weights)`, NOT on endpoint addresses (e.g. 1026 / 342 / 342 for a 3-equal-host default cluster, identical on subject and reference). See `### Load balancer — ring_hash (RING_HASH)`.

The two `maglev_lb.*` gauges are registered + Set ONCE at table build, and ONLY when `lb_policy: MAGLEV` (the `registerClusterMetrics` type-asserts `*maglevLB`; reference parity). They are STATIC (the table is immutable) and CROSS-SIDE-EXACT — keyed only on `(table_size, host count, weights)`, NOT on endpoint addresses (`min = floor(M/N)`, `max = ceil(M/N)`; e.g. 21845 / 21846 for a 3-equal-host default-65537 cluster, identical on subject and reference). There is NO `maglev_lb.size` gauge (the table size is config-known, unlike ring_hash's computed `ring_hash_lb.size`). See `### Load balancer — maglev (MAGLEV)`.

The four `lb_subsets_*` stats are registered ONLY when `lb_subset_config` is present (the `registerClusterMetrics` type-asserts `*subsetLB`; reference parity). `lb_subsets_active` (gauge) + `lb_subsets_created` (counter) are Set / Add ONCE at build to the distinct-subset count (envoy-go pins the semantically-correct ×1 count — the reference's 33× magnitude artifact is NOT mirrored; a recorded departure, so these two are NOT cross-side-equal). `lb_subsets_selected` (counter) + `lb_subsets_fallback` (counter) are incremented from `subsetLB.Pick` (+1 per subset hit / +1 per fallback path respectively) — the FIRST stats a `loadBalancer` Inc's from its own `Pick` path; they are request-counted and CROSS-SIDE-EQUAL (the `0064` `StatsAsserter` cross-equal prong). See `### Load balancer — subset (lb_subset_config)`.
...
**Phase 32.2 extension — 546 → 1091 internal names:** phase 32.2 adds the **545** remaining `redis_proxy` stats (sub-row 32.2 of the parent row 32 pre-split — the consumer half on top of the 32.1 seam). The breakdown: **2 `splitter.*`** (`splitter.unsupported_command` + `splitter.invalid_request`) + **3 REDIS_CLUSTER_STATS** + **540 per-command** (180 commands × `command.<cmd>.{total,success,error}`), all created EAGERLY at config-parse under scope `redis.<stat_prefix>.` via `NewCounterIfAbsent`. The 180-command roster is the STATIC supported-commands table (transcribed from upstream Envoy v1.37.2 `supported_commands.h` + `command_splitter_impl.cc`); per-row enumeration is NOT reproduced here (the 25.x / 28.1 / 29.1 / 31 large-roster convention) — normatively defined by `internal/filter/network/redisproxy/commands.go::supportedCommandList` + `stats.go` + the golden tests `TestCommandRoster_MatchesUpstream` (180, strict-sorted) / `TestCommandRoster_AllValidNames` (IsValidName by construction) / `TestStatRoster32_2_*`. **The 180 `command.<cmd>.latency` histograms + the `*_fault` counters are DEFERRED per ADR-0060/faults and NOT created** (the +545 is counters ONLY). All 545 are upstream-parity. Phase 32.2 ALSO wires the 2 lifecycle gauges' inc/dec (`downstream_cx_active` MIRRORED + `downstream_rq_active` QUIESCED) + the `downstream_cx_protocol_error` increment — all created at 32.1, so +0 creation there. The 2 `*_bytes_buffered` gauges remain a SUBJECT-SIDE coverage boundary (subject pins 0; the reference legitimately tracks them). Phase 32.2 total: **546 → 1091 internal names** (546 from pre-32.2 + **545 from phase 32.2**). Prometheus rendering via the `redis.` LABEL-HOISTED tag-extractor arm landed THIS phase at `internal/stats/name.go` (`envoy_redis_<leaf>{envoy_redis_prefix="<sp>"}` — the mongo `.rbac.` single-label-hoist shape, NOT the kafka inline shape). **This LANDS the second half + closes the SIXTH §9 Network-filters-family row** (the parent-row-32 ROLLUP — parent row 32 + sub-row 32.2 flip `in-progress → done` atomically); **1 §9 family candidate remains after phase 32 (`thrift`).**

**Phase 33 extension — 1091 → 1116 internal names:** phase 33 adds the **25** `thrift_proxy` fixed stats (`envoy.extensions.filters.network.thrift_proxy.v3.ThriftProxy` — the SEVENTH §9 Network-filters-family row, a flat top-level row; the §9 FAMILY-CLOSING row). The roster = **24 counters + 1 GAUGE `request_active`** created EAGERLY at config-parse under scope `thrift.<stat_prefix>.` via `NewCounterIfAbsent` / `NewGaugeIfAbsent` (idempotent across listeners sharing a prefix — the redis/kafka/mongo precedent). The 24 counters: `request`, `request_call`, `request_oneway`, `request_passthrough`, `request_decoding_error`, `request_invalid_type`, `request_internal_error`, `response`, `response_reply`, `response_success`, `response_error`, `response_exception`, `response_passthrough`, `response_decoding_error`, `response_invalid_type`, `route_missing`, `unknown_cluster`, `no_healthy_upstream`, `shadow_request_submit_failure`, `upstream_rq_maintenance_mode`, `cx_destroy_local_with_active_rq`, `cx_destroy_remote_with_active_rq`, `downstream_cx_max_requests`, `downstream_response_drain_close`; the 1 gauge: `request_active` (the project's THIRD differentially-mirrored gauge family — after mongo's `op_query_active` + redis's `downstream_cx_active`). Normatively defined by `internal/filter/network/thriftproxy/stats.go::counterSuffixes` + `TestStatRoster33_*`. The roster is FIXED + table-bounded (no wire-derived segment → `IsValidName` satisfied by construction, D-T7). **The `request_time_ms` histogram is DEFERRED per ADR-0060 and NOT created** (the registry has no Histogram type — the +25 is 24 counters + 1 gauge ONLY). All 25 are upstream-parity. The upstream traffic stats (`upstream_cx_total`/`upstream_rq_total`) live under the existing `cluster.<name>.*` roster (the redis AMEND-R6 precedent) — NOT new thrift names; the subject's cluster package does NOT emit `upstream_cx_rx_bytes_total` (it reuses only `upstream_cx_total`/`upstream_rq_total`). **3 counters are created-but-NEVER-incremented (the `reference_close_direction_framework_gap` coverage boundary, AMEND-T6/D-T8):** `cx_destroy_local_with_active_rq` + `cx_destroy_remote_with_active_rq` (close-direction-keyed — the framework records close TYPE not DIRECTION) + `downstream_response_drain_close` (the reply-completion drain close) — the reference DOES fire `cx_destroy_local_with_active_rq` + `downstream_response_drain_close` on the live `0057` routing-MISS arm (where it `FlushWrite`-closes after the local exception), so they are asserted PER-SIDE there (subject pins 0; the reference moves them). Phase 33 total: **1091 → 1116 internal names** (1091 from pre-33 + **25 from phase 33**). Prometheus rendering via the NEW `thrift.` SINGLE-label-HOIST tag-extractor arm landed THIS phase at `internal/stats/name.go` (`envoy_thrift_<leaf>{envoy_thrift_prefix="<sp>"}` — the redis-32 / mongo `.rbac.` single-label-hoist shape, NOT the kafka inline shape). **This LANDS the SEVENTH §9 Network-filters-family row (a flat row, NO parent rollup per ADR-0106) + CLOSES the §9 Network-filters family — 0 candidates remain after phase 33.**

**Phase 32.1 extension — 536 → 546 internal names:** phase 32.1 adds the **10** `redis_proxy` fixed stats (`envoy.extensions.filters.network.redis_proxy.v3.RedisProxy` — the SIXTH §9 Network-filters-family sub-phase; sub-row 32.1 of the parent row 32 pre-split, seam-first). The roster = **6 counters + 4 gauges** created EAGERLY at config-parse under scope `redis.<stat_prefix>.` via `NewCounterIfAbsent`/`NewGaugeIfAbsent` (idempotent across listeners sharing a prefix — the kafka/mongo precedent). The 10 names: `downstream_cx_total` (counter), `downstream_cx_rx_bytes_total` (counter), `downstream_cx_tx_bytes_total` (counter), `downstream_rq_total` (counter), `downstream_cx_drain_close` (counter; created; incremented at 32.2), `downstream_cx_protocol_error` (counter; created; incremented at 32.2), `downstream_cx_active` (gauge; created; inc/dec at 32.2), `downstream_cx_rx_bytes_buffered` (gauge; created; inc/dec at 32.2), `downstream_cx_tx_bytes_buffered` (gauge; created; inc/dec at 32.2), `downstream_rq_active` (gauge; created; inc/dec at 32.2). Normatively defined by `internal/filter/network/redisproxy/stats.go::rosterSuffixes()` + `TestStatRoster32_1_MatchesUpstream`. All 10 are upstream-parity (a subset of `ALL_REDIS_PROXY_STATS`; the 2 `splitter.*` + 3 REDIS_CLUSTER_STATS = 5 remaining fixed names land at 32.2). The 4 gauges are the project's SECOND differentially-mirrored gauge family (after mongo's `op_query_active`); their inc/dec lands at 32.2. The upstream traffic stats (`upstream_cx_total`/`upstream_cx_active`/`upstream_rq_total`) live under the existing `cluster.<name>.*` roster (AMEND-R6) — NOT new redis names. Prometheus rendering is LABEL-HOISTED (the `redis.` tag-extractor arm at `internal/stats/name.go` — `envoy_redis_<leaf>{envoy_redis_prefix="<sp>"}`, the mongo `.rbac.` shape; the arm itself lands at 32.2; the 32.1 differential uses the flat `GET /stats` internal-name scrape). Phase 32.1 total: **536 → 546 internal names** (536 from pre-32.1 + **10 from phase 32.1**). **This LANDS the first half of the SIXTH §9 Network-filters-family row** (sub-row 32.1 of the parent row 32 pre-split; parent row 32 STAYS `in-progress`); **1 §9 family candidate remains after phase 32 (`thrift`).**

**Phase 31 extension — 360 → 536 internal names:** phase 31 adds the **176** `kafka_broker` eager counters (`envoy.extensions.filters.network.kafka_broker.v3.KafkaBroker` — the FIFTH §9 Network-filters-family row + the family's third stats-PRIMARY filter + consumer #3 of the ADR-0221 WriteFilter seam + the project's FIRST `/contrib v1.32.4` consumer). The roster = **86 per-key `request.<root>_request` + 86 per-key `response.<root>_response` + 4 fixed** (`request.unknown` / `request.failure` / `response.unknown` / `response.failure`), created EAGERLY at config-load under scope `kafka.<stat_prefix>.` via `NewCounterIfAbsent`. The 86-key roster is a STATIC Kafka 3.9.1 table (`api_keys` 71/72 telemetry-EXCLUDED → 86 of 88 keys); per-row enumeration is NOT reproduced here (the 25.x / 28.1 / 29.1 large-roster convention) — normatively defined by `internal/filter/network/kafkabroker/stats.go::newKafkaStats` + `apikeys.go::apiKeyRoster()` + the golden tests `TestStatRoster` / `TestApiKeyRoster_MatchesUpstream`. **The 86 response-duration histograms are DEFERRED per ADR-0060 and NOT created** (the +176 is counters ONLY, not +262). All 176 are upstream-parity (none envoy-go-strict); the envoy-go-strict departures are behavioral (the eager boot-window creation posture; the per-exception-reset decode posture; the `*.failure` cross-side abandon-at-close asymmetry — all recorded as coverage boundaries in the kafka_broker subsection, not as stat-table departure rows). Phase 31 total: **360 → 536 internal names** (360 from pre-31 + **176 from phase 31**). Prometheus rendering is FULLY INLINED with EMPTY labels (`envoy_kafka_<sp>_<rest>{}`) via the NEW `kafka.` INLINE-PREFIX arm at `internal/stats/name.go` (the zookeeper `.zookeeper.` INLINE shape generalized to a `kafka.` ROOT prefix — NOT the mongo label-hoist; AMEND-K2, REFUTING the BRAINSTORM label-hoist hypothesis). The phase-15 deferred histograms + the phase-16 per-policy counter family + the 8 reference-only ext_proc counters continue NOT counted in the base total per the long-standing convention. **This LANDS the FIFTH §9 Network-filters-family row** (a flat top-level row per ADR-0106); after phase 31, **2 §9 family candidates remain (`redis`/`thrift`).**

**Phase 30 (reference-image pin-refresh) note:** phase 30 refreshes the `ENVOY_TARGET.md` differential reference image from the standard Envoy variant `envoyproxy/envoy:v1.37.2` (ADR-0008) to the **contrib variant of the SAME upstream version**, `envoyproxy/envoy:contrib-v1.37.2` @ `sha256:7edd5b0fd763d32c3dfcfd0061f9c2ea63eebd8cdf7f88d974d3adfc99453be8` (ADR-0227, superseding ADR-0008) — the dedicated re-baselining phase mandated by doctrine D-3.7. The contrib image is a behavioral SUPERSET (identical Envoy v1.37.2 source plus extra compiled-in contrib extensions); the full 54-fixture differential suite re-baselined **byte-identical-PASS, ZERO divergence** against it, so this contract's surface is UNCHANGED (54 fixtures / 39 fuzzers / 360 stats / BackendKind tail 30). The two current-pin bullets above now carry the contrib digest + ADR-0227. ZERO production LoC, ZERO new go.mod deps; motivation is downstream (unblock the contrib-only `kafka_broker` §9 subject, phase 31).

**Phase 29.3 extension — 360 → 360 internal names (zero creation delta; the parent-row-29 ROLLUP):** phase 29.3 wires the increments of the LAST four `mongo_proxy` fixed counters that were created eagerly at 29.1 (D-P1) and stayed presence-only through 29.2: `delays_injected` (at fault-timer ARM — mirroring upstream `stats_.delays_injected_.inc()`), `cx_drain_close` (a correlated reply EMPTIES the active-query list while the connection is draining → +1 → `Connection().Close(FlushWrite)`), and `cx_destroy_local_with_active_rq` + `cx_destroy_remote_with_active_rq` (the D-P4 close-direction VALUE parity — keyed on which `tcp_proxy` pump EOF'd first, incremented at `OnDestroy` IFF the residual active-query list was non-empty). Zero new counters created; no `internal/stats/name.go` arm added (the 29.1 `mongo.` four-rule tag-extractor handles all four leaves AS-IS); no new built-in. Fixture `0052-mongo-fault-delay` (the `TCPMongoResponder` BackendKind 30 REUSED) is the cross-side proof — `delays_injected` VALUE parity (pre + post handoff), the `cx_destroy_*` LOCAL/REMOTE/all-answered VALUE parity, and the `cx_drain_close` PRESENCE-downgrade (D-S29.3-8 — the unit value proof is load-bearing); no new fuzzer (39 unchanged — the seam concurrency is proven by `-race` tests). The stat total stays **360**. **This closes the FOURTH §9 Network-filters-family row** (the parent-row-29 ROLLUP — `mongo_proxy` full request + response + correlation + gauge + fault-delay + access-log + drain + close-direction); after phase 29, **3 §9 family candidates remain (`redis`/`kafka_broker`/`thrift`).**

**Phase 29.2 extension — 360 → 360 internal names (zero creation delta):** phase 29.2 wires the response-side increments of the 5 `mongo_proxy` reply counters (`op_reply` / `op_reply_cursor_not_found` [responseFlags 0x01] / `op_reply_query_failure` [0x02] / `op_reply_valid_cursor` [cursorID ≠ 0] / `op_command_reply`) + the `op_query_active` GAUGE inc/dec/drain — all 6 created EAGERLY at 29.1 (exist-at-zero, creation parity D-P1). Zero new counters or gauges are created; no `internal/stats/name.go` arm added; no new built-in. The reply counters charge ONLY after a fully successful OP_REPLY/OP_COMMANDREPLY body decode; the gauge `Inc`s at OP_QUERY decode, `Dec`s at a correlated reply, and `Add(-n)`-drains the residual at connection destroy (nets to 0 per connection — the project's FIRST differentially-mirrored gauge). Fixture `0051-mongo-responses` (the NEW `TCPMongoResponder` BackendKind 30) is the cross-side proof; the 39th fuzzer `FuzzMongoDecode` is EXTENDED to feed both `decodeOnData` + `decodeOnWrite` (D-P6 — no 40th). The stat total stays **360**.
...
The envoy-go admin server is a single HTTP/1.1 plaintext bind allocated by `internal/admin.Server.Start()` (per phase 01 contract; reused unchanged in 06.1 and 08.1). Eight endpoints are registered on the same `*http.ServeMux`: `/ready` (phase 01), `/stats/prometheus` (phase 06.1), `/config_dump`, `/clusters`, `/listeners`, `/server_info` (phase 08.1), `POST /drain_listeners` (phase 08.2; NEW mutating endpoint), `GET /stats` (phase 32.1; NEW flat internal-name text endpoint). Phase 08.2 also extends `/ready` + `/server_info` for the DRAINING state.

**Framing deviation (all eight admin endpoints).** envoy-go's `net/http` server emits `Content-Length` (the body is buffered before write); upstream Envoy v1.37.2 emits `transfer-encoding: chunked`. The differential harness dechunks upstream responses before byte-comparing the body. This deviation was first documented for `/ready` at phase 01 (per ADR-0015 paragraph 3) and extends unchanged to all eight endpoints. No allow-list entry; the dechunk is structural.

**Header set (all eight admin endpoints, post-framing-normalization).** The lowercase wire-form header set is `content-type`, `cache-control: no-cache, max-age=0`, `x-content-type-options: nosniff`, `date: <IMF-fixdate>`, `server: envoy` (per ADR-0014). All eight endpoints emit this set via the shared `writeAdminHeaders` helper. The differential harness uses the existing case-insensitive header comparator (introduced for phase 01).
...
*Introduced by phase 32.1 (`network-filter-redis-upstream-pool-seam-and-codec`). Justified by D-S32.1-5 (the 32.1 `StatsAsserter` flat-`/stats` scrape requirement — the `redis.<stat_prefix>.` and `cluster.<name>.*` names not yet in `/stats/prometheus` pending the 32.2 `redis.` tag-extractor arm). Implemented in `internal/admin/stats.go`; routed through `writeAdminHeaders` (the shared 4-header helper).*

**Body shape.** `text/plain; charset=UTF-8`. One stat per line in `name: value\n` format (LF line endings, no trailing blank line), sorted ascending by internal stat name. All registered stats are emitted at their current integer value. This is the flat internal-name text form — the counterpart to `/stats/prometheus` (which applies the `name.go` tag-extractor arms and emits Prometheus text exposition format). envoy-go-strict endpoint (upstream Envoy's flat `/stats` text form differs in detail; this endpoint is introduced for the internal-name `StatsAsserter` scrape — not a byte-equivalence target against the reference).

**Equivalence claim.** NOT asserted byte-equal to reference Envoy's `/stats` (upstream Envoy's flat stat text uses a different format and namespace). The `0055` `StatsAsserter` scrapes THIS endpoint on BOTH sides (reference + subject) and compares the NAMED stat values by internal name (both sides use the same internal names for `redis.<sp>.*` + `cluster.<name>.*`), so the cross-side value equality is the meaningful assertion — not body byte-equality of the entire document.

**Header set.** Inherits the umbrella rules (all eight admin endpoints share the `writeAdminHeaders` 4-header set: `content-type: text/plain; charset=UTF-8`, `cache-control: no-cache, max-age=0`, `x-content-type-options: nosniff`, `date`, `server: envoy`).

### Applies to

- phase 08.1 and phase 08.2 envoy-go admin subsystem.
- all eight endpoints: `/ready`, `/stats/prometheus`, `/config_dump`, `/clusters`, `/listeners`, `/server_info`, `POST /drain_listeners` (08.2 NEW), `GET /stats` (32.1 NEW — flat internal-name text; `name: value\n`, sorted; introduced for the `0055` `StatsAsserter` scrape, D-S32.1-5).
...
### Load balancer — least_request (LEAST_REQUEST)

*Introduced by phase 34 (the Load-balancing family OPENS). Justified by ADR-0232 (the LB acquire/release seam) + ADR-0233 (the least_request policy).*

Phase 34 lands the project's first LB policy beyond phase-02 ROUND_ROBIN. `cluster.Manager` accepts `Cluster.LbPolicy LEAST_REQUEST` on the LEGACY enum path (the same path ROUND_ROBIN uses; the `Cluster.load_balancing_policy` extension point stays deferred).

**Acceptance + config (`Cluster.LeastRequestLbConfig`, the `lb_config` oneof member field 37).** `choice_count` defaults to **2** when the config is absent (or a mismatched oneof member is set — see below); when set it must be **`>= 2`** (a hand-rolled PGV-parity gate — the manager calls no PGV); a set value is used **verbatim with NO clamp** at `choice_count > len(endpoints)` (reference parity — a legal config, e.g. `choice_count: 10` over 3 endpoints, makes the policy quasi-deterministic).

**P2C selection semantics (the upstream v1.37.2 mirror).** Un-weighted power-of-two-choices: sample `choice_count` host indices **with replacement** (`rng() % n` per draw — duplicates possible, no clamp); pick the host with the **strictly fewest** outstanding active requests (strict `<` on the raw per-endpoint active count); **first-drawn wins ties**. The weighted `weight/(active+1)^bias` formula belongs to upstream's EDF path, which engages ONLY under unequal host weights or slow_start — the equal-weight MVP takes pure P2C, and `active_request_bias` never enters the computation. The RNG is a crypto-seeded mutex-guarded `math/rand/v2` PCG (injectable in tests). The per-endpoint active counters are LB-internal `atomic.Int64` state, NOT registry stats.

**cx-as-rq active-count semantics (the active counter's unit).** The per-endpoint counter counts **open connections attributable to a pick**: +1 at `Pick` (conn-producing paths), −1 at the connection's final Close (or immediately on dial failure / non-conn picks). For TCP-proxied traffic this is EXACTLY the reference's semantics (live-pinned: per-host `rq_active == cx_active` 1:1; tcp_proxy charges one rq per cx). For the H1 connection pool, an idle pooled connection RETAINS its +1 hold from dial until its FINAL close (caller close, `PutIdleH1` overflow drop, or `closePool` drain) — real upstream load, the documented HTTP idle-pooled-conn approximation boundary.

**Per-side RNG non-equivalence (NOT cross-side-exact; band-proven).** Both proxies' P2C selection is RNG-driven on independent streams (upstream's RNG is per-worker-thread crypto-seeded; the subject's is per-cluster crypto-seeded), so the absolute selection sequence is NEVER cross-side-equal. Fixture `0059-lb-least-request` proves the policy DISTRIBUTIONALLY via the FIRST band-based `AssertDistribution` use (a hold-4/burst-60 skew workload over 3 reused `TCPEcho` backends at `choice_count: 10`): asserted **PER SIDE** as {conservation `c1+c2+c3 == 64`, starvation `c1 <= 12`, concentration `c2 >= 16`} on the sorted per-backend accept counts. Each band leg is proven live by a deliberate-break under `-count=1` (an inverted comparison fails concentration; a no-op release fails starvation; a corrupt `StatsAsserter` want fails the stats prong).

**Departures + coverage boundaries (recorded here per the §9 byte-stable-reject lineage):**
- **NEW lb-policy reject text.** The remaining `CLUSTER_PROVIDED`/`LOAD_BALANCING_POLICY_CONFIG` are rejected at config-parse with `cluster: %q: unsupported lb_policy %s (supported: ROUND_ROBIN, LEAST_REQUEST, RANDOM, RING_HASH, MAGLEV)` — the supported-list extended at phase 37 (MAGLEV retired from the rejected set → the FIFTH accepted policy; see `### Load balancer — maglev (MAGLEV)` below). `CLUSTER_PROVIDED` is now the LONE recorded LB-policy departure / retarget trigger: the reference parse-ACCEPTS the five enum-path policies envoy-go accepts, so envoy-go's continued rejection of `CLUSTER_PROVIDED` (which needs a cluster-supplied LB the static path has no shape for) is the next departure. No fixture pins the text (it lands unit-level in `manager_test.go`, whose `TestManager_Error_UnsupportedLBPolicy` doubly-hit-RETARGETED its trigger from `Cluster_MAGLEV` — now accepted — to `Cluster_CLUSTER_PROVIDED`, the phase-34/35/36 accept-of-N-forces-retarget-to-N+1 pattern).
- **`active_request_bias` / `slow_start_config` DEPARTURE rejects.** Set under `lb_policy: LEAST_REQUEST` → rejected at config-parse (the reference parse-accepts them and they alter its picks; `slow_start_config` engages the reference's EDF path, and `active_request_bias` goes live the instant host weights become unequal — a dimension envoy-go silently ignores entirely). Fail-fast rather than silently diverging (the thrift AMEND-T7 lineage).
- **Mismatched `lb_config` oneof member — silent-ignore (PARITY).** A mismatched oneof member (e.g. `ring_hash_lb_config` under `LEAST_REQUEST`, or `least_request_lb_config` under `ROUND_ROBIN`) is silently ignored — reference parity (validate-accepts silently) AND behavior-inert (both sides default identically: `GetLeastRequestLbConfig()` nil → choice_count 2). The existing under-ROUND_ROBIN posture stays byte-stable.
- **Direct-pick consumers' load invisibility (coverage note).** The two non-conn `PickEndpoint()` consumers — httpclient's `request.URL.Host` rewrite + the thriftproxy pick-without-dial no-healthy-host probe — release IMMEDIATELY (they hold no connection), so their picks are invisible to least_request's active counters. A documented coverage boundary, not a bug.
- **`upstream_rq_total` per-side on the TCP path.** On `0059`'s stats prong: cross-equal `cluster.<name>.upstream_cx_total == 64` + `membership_total == 3` + `upstream_cx_active == 0` (quiesced); PER-SIDE `upstream_rq_total` (reference = 64 [tcp_proxy charges rq-per-cx]; subject = 0 [the tcpproxy path never calls `IncUpstreamRqTotal` — a pre-existing boundary this fixture pins]).
- **NO new fuzzer / NO new BackendKind (deliberate firsts).** Phase 34 decodes no wire bytes → NO new fuzzer (the FIRST no-fuzzer phase since the fuzzing regime began; a manufactured fuzzer over no decoder is a dead assertion). `0059` reuses `TCPEcho` → NO new BackendKind (tail stays 33).
- **Stat surface UNCHANGED at 1116.** ZERO lb-policy stat-name delta (live-confirmed empty `/stats` name-set diff between LEAST_REQUEST and ROUND_ROBIN; the reference's `cluster.<name>.lb_*` roster exists identically-at-0 under both policies and is NOT mirrored — unchanged posture). The FIRST zero-stat-delta phase since the surface began moving.

### Load balancer — random (RANDOM)

*Introduced by phase 35 (the Load-balancing family's SECOND row). Justified by ADR-0234 (the random policy); REUSES the ADR-0232 LB acquire/release seam UNCHANGED (the seam's FIRST reuse).*

Phase 35 lands the project's THIRD LB policy (after phase-02 ROUND_ROBIN + phase-34 LEAST_REQUEST). `cluster.Manager` accepts `Cluster.LbPolicy RANDOM` on the LEGACY enum path (the same path ROUND_ROBIN + LEAST_REQUEST use; the `Cluster.load_balancing_policy` extension point stays deferred).

**Acceptance (NO config message).** RANDOM has NO config message — `Cluster.RandomLbConfig` does not exist in the v1.32.4 surface (zero config knobs; the simplest family surface). The manager's `case clusterv3.Cluster_RANDOM` is a BARE construction: `newRandom(endpoints)` — NO config-parse arm, NO PGV gate. (RANDOM's locality-weighting config lives only on the deferred `load_balancing_policies.random.v3` extension-point proto.)

**Uniform-pick semantics (the upstream v1.37.2 mirror).** Stateless uniform random: ONE draw `i := int(rng() % uint64(n))`, return `endpoints[i]` — mirroring upstream `RandomLoadBalancer::peekOrChoose` EXACTLY. It consults NO active counters, runs NO power-of-two loop, holds NO per-pick state, and applies NO tie-break (the contrast with least_request's P2C, which draws `choice_count` times and picks the strictly-fewest-active host). The RNG is the SAME crypto-seeded mutex-guarded `math/rand/v2` PCG least_request uses (the package-private `newPCGRNG()` helper, REUSED verbatim — `random.go` calls it directly, no extraction; injectable via `newRandomWithRNG` in tests). `randomLB` returns the shared `noopRelease` (it holds no per-endpoint state — the seam's "zero cost" reuse).

**Healthy-set boundary (no health checking → all-hosts sampling).** Healthy-host filtering happens upstream of the pick in the reference's shared base (priority/healthy/panic); with no health checking, envoy-go's RANDOM degenerates to all-hosts sampling (the Upstream-robustness family's boundary — the same posture as least_request).

**Per-side RNG non-equivalence (NOT cross-side-exact; band-proven).** Both proxies' selection is RNG-driven on independent crypto-seeded streams, so the absolute selection sequence is NEVER cross-side-equal. Fixture `0060-lb-random` proves RANDOM STAYS uniform DISTRIBUTIONALLY via the SECOND band-based `AssertDistribution` use — the REUSED `0059` hold-4/burst-60 skew stimulus over 3 reused `TCPEcho` backends, with the assertion INVERTED (the contrapositive of least_request): asserted **PER SIDE** as {conservation `c1+c2+c3 == 64`, uniform-floor `each >= 6`, uniform-ceiling `each <= 40`} on the per-backend accept counts. RANDOM stays uniform-band DESPITE the held-conn load imbalance (it never consults the counters), where least_request would skew away from the held endpoint. Each band leg is proven live by a deliberate-break under `-count=1` (see below).

**Departures + coverage boundaries:**
- **Mismatched `lb_config` oneof member — silent-ignore (PARITY).** A mismatched oneof member (e.g. `least_request_lb_config` or `ring_hash_lb_config` set under `RANDOM`) is silently ignored — reference parity AND behavior-inert (the `lb_config` oneof has NO `random` member, so the manager never reads it on the RANDOM path). The construction is unconditionally bare.
- **D-S35-2 shared-RNG-message cosmetic note.** `newRandom` calls the shared `newPCGRNG()` directly and ACCEPTS its seed-error message verbatim — `"cluster: least_request: seed rng"`. This path is reachable ONLY on a `crypto/rand` read failure (effectively unreachable); the `least_request:` prefix under a RANDOM cluster is a recorded COSMETIC note, NOT a behavior divergence (a wrapper would double-wrap or force an edit to `newPCGRNG`, which the SPEC AMEND-R5 forbids).
- **The 0060 anti-skew band re-tune (IMPL-time empirical amendment to D-S35-3).** The SPEC anticipated `{floor 12, ceiling 32}` (~2.5σ), which empirically FLAKED 2/20 on the natural multinomial tail (`P(X<12) ≈ 0.0032`, `P(X>32) ≈ 0.0020` per sample; 6 samples/run). The PLAN's pinned PRINCIPLE {conservation + uniform-floor + uniform-ceiling} authorized widening within the band; the constants were re-tuned at IMPL Task 6 to `{floor 6, ceiling 40}` (~4–5σ; per-sample `P(X<6) ≈ 2e-6`, `P(X>40) ≈ 3e-7`) — proven flake-free over **60/60** runs (two `-count=20` batches + the 3 `-count=1` breaks; observed per-backend extremes 14–31, comfortably inside `[6,40]`). The wider band STILL bites all three deliberate breaks. Final constants: `K=4` held, `S=60` burst, `sum==64`, `uniformFloor=6`, `uniformCeiling=40`.
- **The 3 deliberate-break liveness proofs (`-count=1`, go-test caching defeated).** (i) `case Cluster_RANDOM`'s `newRandom(endpoints)` → `newLeastRequest(endpoints, …)` (the canonical anti-skew break — least_request consults the held-conn counters) fails the uniform FLOOR (`backend[1]=2 < 6`); (ii) `Pick`'s `rng() % n` → `0` (single-host pin) fails the uniform CEILING (`backend[0]=64 > 40`); (iii) the `StatsAsserter` `upstream_cx_total` want 64 → 99 fails the stats prong (both sides observe 64). Each: applied → `-count=1` FAIL → `git restore` → diff-clean; never committed broken.
- **NO new fuzzer / NO new BackendKind (now family expectations).** RANDOM decodes no wire bytes → NO new fuzzer (fuzzers stay **42**). `0060` reuses `TCPEcho` → NO new BackendKind (tail stays **33**). These are the phase-34 family-level firsts, now ESTABLISHED expectations.
- **Stat surface UNCHANGED at 1116 (the SECOND zero-delta phase).** ZERO lb-policy stat-name delta (live-confirmed empty `/stats` name-set diff between RANDOM and ROUND_ROBIN, 455 == 455). The `0060` `StatsAsserter` set: `upstream_cx_total`/`membership_total`/quiesced-active CROSS-EQUAL; `upstream_rq_total` PER-SIDE (reference = 64 [tcp_proxy charges rq-per-cx]; subject = 0 [the tcpproxy path never calls `IncUpstreamRqTotal`] — the same boundary `0059` pins).

### Load balancer — ring_hash (RING_HASH)

*Introduced by phase 36.1 (the Load-balancing family's THIRD row; the project's FIRST consistent-hashing policy). Justified by ADR-0236 (the ring_hash policy) + ADR-0235 (the LB hash-key seam extension — the seam's FIRST extension, the ADR-0232 PICK-INPUT half). The 36.1 leg lands the seam + the `ringHashLB` + the manager gate + the tcp_proxy `source_ip` plane; the 36.2 leg adds the HTTP route `hash_policy` plane.*

Phase 36.1 lands the project's FOURTH LB policy (after phase-02 ROUND_ROBIN + phase-34 LEAST_REQUEST + phase-35 RANDOM) and its FIRST consistent-hashing policy. `cluster.Manager` accepts `Cluster.LbPolicy RING_HASH` (`envoy.config.cluster.v3`, enum value 2) on the LEGACY enum path (the same path the other three use; the `Cluster.load_balancing_policy` extension point stays deferred).

**Acceptance + the `RingHashLbConfig` two-layer gate.** `lb_policy: RING_HASH` with no `ring_hash_lb_config` is accepted with the defaults {minimum_ring_size 1024, maximum_ring_size 8388608, hash_function XX_HASH} (the manager self-supplies the doc-comment defaults — the v1.32.4 zero value is nil, NOT generated). `ring_hash_lb_config` carries `minimum_ring_size` (proto field 1), `hash_function` (field 3 — `XX_HASH`=0 default / `MURMUR_HASH_2`=1), `maximum_ring_size` (field 4). The HAND-ROLLED gate (the manager calls no PGV — the phase-34 `choice_count` precedent) mirrors the reference's TWO validation layers byte-for-byte: (a) the PGV per-field bound — `minimum_ring_size`/`maximum_ring_size` `> 8388608` → `cluster: %q: ring_hash_lb_config.{minimum,maximum}_ring_size: value must be less than or equal to 8388608`; (b) the RUNTIME cross-field check — `minimum_ring_size > maximum_ring_size` (post-default) → `cluster: %q: ring hash: minimum_ring_size (%d) > maximum_ring_size (%d)` (distinct wording from the PGV layer; there is NO `min ≤ max` PGV rule — AMEND-RH1). All reject arms land UNIT-LEVEL in `manager_test.go` (NO cross-side boot-reject dir — no fixture pins them; both sides reject identically).

**Ketama consistent-hash ring semantics (the upstream v1.37.2 mirror).** The `ringHashLB` (`internal/cluster/ringhash.go`) builds an immutable ring ONCE at construction, mirroring `RingHashLoadBalancer` EXACTLY: for each endpoint, `ceil(minimum_ring_size / N)` ring points (the equal-weight MVP — the running-sum scale formula collapses to this under equal weights; weighted-ring richness deferred), each keyed `hash("<addr>:<port>_<i>")` (`_` literal + decimal index, no padding) — `xxHash64(key)` seed 0 for the default `XX_HASH`, `murmurHash2(key, 0xc70f6907)` for `MURMUR_HASH_2`; the points are sorted ascending by hash. `Pick(hashKey, hasHash)` binary-searches (`sort.Search`) for the first ring point `>= hashKey`, wrapping to index 0 past the end. The no-hash fallback (`hasHash == false`) draws a UNIFORM random ring position from the injected rng — Envoy's documented "effectively the random LB" fallback (`thread_aware_lb_impl.cc`). For the default 3-equal-host cluster: `size = 3 * ceil(1024/3) = 3 * 342 = 1026`, `min = max = 342` (matched against the live reference — D-S36-5). Both hash functions are IMPLEMENTED (parse + ring build); the differential fixtures use the `XX_HASH` default (cross-side-reproducible), `MURMUR_HASH_2` is unit-level.

**The seam EXTENSION (ADR-0235; the ADR-0232 PICK-INPUT half; the ctx-carried hash key).** The unexported `loadBalancer.Pick` widens from `Pick() (Endpoint, func(), error)` to `Pick(hashKey uint64, hasHash bool) (Endpoint, func(), error)` — the two-scalar `(value, ok)` form (chosen over `*uint64` and a `pickOptions` struct). The three incumbent policies (roundRobin/leastRequest/randomLB) add two IGNORED params — behavior-neutral, so all prior fixtures stay byte-identical. The hash key rides `context.Context`: an unexported `hashKeyCtxKey struct{}` + an EXPORTED additive `cluster.WithHashKey(ctx, key)` (the producers live in OTHER packages) + an unexported `hashKeyFrom(ctx) (uint64, bool)`, all in `cluster.go` (the ADR-0219 ctx-carry precedent). `Dial`/`AcquireH1` extract the key and thread it to `c.lb.Pick(hk, ok)` in 2 changed lines each; their EXPORTED signatures — and `PickEndpoint()` (which passes `(0, false)` → the no-hash fallback) — stay BYTE-STABLE (the OPTION-C discipline preserved). The TWO new exported symbols are the additive `cluster.WithHashKey` (the ctx stuffer) + `cluster.HashSourceIP(addr string) uint64` (the bare-client-IP `xxHash64` helper the tcp plane uses); NO existing method signature changes. The RELEASE half (ADR-0232) is REUSED unchanged (`ringHashLB` returns the shared `noopRelease` — the ring is immutable, no per-pick state). This is the seam's FIRST extension and the durable asset maglev (and any future hash policy) reuses unchanged — the analogue of ADR-0232 (the RELEASE half) for the PICK-INPUT half.

**The two data planes (tcp `source_ip` at 36.1; HTTP route `hash_policy` at 36.2).** The hash key is supplied by BOTH planes. The 36.1 tcp plane: `tcp_proxy` parses `hash_policy: [{source_ip: {}}]` and, per new connection, computes `cluster.HashSourceIP(downstream.RemoteAddr)` = `xxHash64(bare client IP, NO port)` seed 0 and stuffs it via `cluster.WithHashKey` before `Dial(ctx)` (a non-source_ip tcp `hash_policy` specifier — e.g. filter_state — is DEPARTURE-rejected). The 36.2 HTTP plane (deferred to the 36.2 IMPL): the router route-level `hash_policy` — HEADER (`xxHash64(value)`) + connection-properties `source_ip` (the tcp-identical compute), combined `hash = rotl64(prev, 1) ^ new_hash` (rotate-left-1 then XOR, policy-list order, first contributor seeded from 0) with a `terminal=true` short-circuit; cookie / query_parameter / filter_state DEPARTURE-rejected (they need cookie-generation callbacks / query-string parsing / filter-state plumbing — out of envelope).

**Per-side affinity (deterministic) vs cross-side host-identity NON-reproducibility (the documented harness boundary; AMEND-RH8).** Each side's ring is internally byte-exact (every ring input is data envoy-go has), so affinity is DETERMINISTIC: the same hash key → the same ring point → the same backend, ALWAYS (an equality, not a band). BUT cross-side host IDENTITY (does key X hit the "same" backend index on both sides?) does NOT hold — (1) the reference proxy runs Docker-mapped, so Docker NATs every client connection to ONE bridge-gateway source IP (a cross-side source_ip SPREAD is infeasible — N keys on the subject, 1 on the reference), and (2) the two sides build the ring over DIFFERENT endpoint address strings (subject STATIC `127.0.0.1:port`; reference STRICT_DNS container-IP:port) → the ring LAYOUTS differ. Therefore the differential asserts PER-SIDE affinity + spread + cross-side byte-equivalence + cross-side `StatsAsserter` (the 0059/0060 per-side precedent), NOT cross-side host identity. Fixture `0061-lb-ring-hash` (tcp source_ip; 36.1): the driver binds outgoing conns to source IPs `127.0.0.2..5` (16 conns each = 64 total) via `net.Dialer{LocalAddr}`; the SUBJECT-side affinity manifests in the aggregate per-backend counts as a MODULAR INVARIANT — every per-backend count `% 16 == 0` (one source IP → one key → one ring point → one backend, all-16-or-0; D-S36-4), the EXACT affinity leg; spread `>= 2` distinct nonzero backends; conservation `sum == 64`. The reference (Docker-NAT'd to one source IP → all 64 on ONE backend) is asserted on `sum == 64` + cross-side byte-equivalence + cross-side stats only. (The 36.2 `0062-lb-ring-hash-http` keys on a NAT-transparent REQUEST HEADER → a TRUE cross-side affinity+spread differential — both sides assert affinity.)

**Healthy-set boundary (no health checking → all-hosts ring).** Healthy-host filtering happens upstream of the pick in the reference's shared base (priority/healthy/panic); with no health checking, the ring is built over the full static endpoint set (the same Upstream-robustness-family boundary as least_request + random).

**Departures + coverage boundaries:**
- **The 3 mirrored `ring_hash_lb.*` gauges (the FIRST non-zero LB-stat delta; stat surface 1116 → 1119).** RING_HASH is the FIRST LB policy to move the stat surface (least_request + random were both zero-delta). Three new MIRRORED per-cluster gauges `cluster.<name>.ring_hash_lb.{size, min_hashes_per_host, max_hashes_per_host}` (registered + Set once at ring build via `registerClusterMetrics`'s `*ringHashLB` type-assert — ONLY on the RING_HASH path, reference parity; D-S36-6). They are STATIC (immutable ring) and CROSS-SIDE-EXACT — keyed only on `(minimum_ring_size, hash_function, host count, weights)`, NOT on endpoint addresses (1026 / 342 / 342 on BOTH sides) — the strong `0061` `StatsAsserter` cross-equal prong. Live-proven REFUTING the BRAINSTORM's anticipated zero delta (the `/stats` name-set diff RING_HASH-vs-ROUND_ROBIN is exactly these three names, 458 vs 455). See the `## Stat-name mapping` "Phase 36.1 extension" block.
- **`upstream_rq_total` per-side on the TCP path.** On `0061`'s stats prong: cross-equal `ring_hash_lb.{size=1026, min=342, max=342}` + `upstream_cx_total` + `membership_total == 3` + `upstream_cx_active == 0` (quiesced); PER-SIDE `upstream_rq_total` (reference = conn count; subject = 0 — the tcpproxy path never calls `IncUpstreamRqTotal`, the same boundary `0059`/`0060` pin; on the 36.2 HTTP plane the router DOES increment it → cross-equal there).
- **Mismatched `lb_config` oneof member — silent-ignore (PARITY).** A stray `ring_hash_lb_config` under a non-RING_HASH cluster, or a mismatched member (e.g. `least_request_lb_config`) under `RING_HASH`, is silently ignored — reference parity AND behavior-inert (the manager reads `GetRingHashLbConfig()` only on the RING_HASH path).
- **`MURMUR_HASH_2` / cookie / query_parameter / filter_state.** `MURMUR_HASH_2` is implemented (parse + ring build) but unit-level (the fixtures use the `XX_HASH` cross-side-reproducible default). The HTTP `hash_policy` cookie / query_parameter / filter_state sources are DEPARTURE-rejected at 36.2 (out of envelope); a non-source_ip tcp `hash_policy` specifier is DEPARTURE-rejected at 36.1.
- **D-S36-2 shared-RNG-message cosmetic note.** `newRingHash` calls the shared `newPCGRNG()` directly (for the no-hash fallback rng) and ACCEPTS its seed-error message verbatim — `"cluster: least_request: seed rng"`. This path is reachable ONLY on a `crypto/rand` read failure (effectively unreachable on Linux `getrandom`) AND only when a RING_HASH cluster receives a no-hash pick; the `least_request:` prefix under a RING_HASH cluster is a recorded COSMETIC note, NOT a behavior divergence (the random-35 D-S35-2 disposition; a wrapper would force editing `newPCGRNG`).
- **D-S36-4 modular-invariant coverage note.** The subject affinity leg asserts `count[i] % 16 == 0` (the all-16-or-0 invariant from binding exactly 16 conns per source IP). This is necessary-and-overwhelmingly-discriminating against the random-scatter break (a random multinomial(64, 1/3) landing all-multiples-of-16 is < 1%), NOT a tight proof against an adversarial even-split — a documented coverage boundary, since `TCPEcho` carries no backend identifier so per-source-IP→backend attribution is not directly observable.
- **The 4 deliberate-break liveness proofs (`-count=1`, go-test caching defeated).** (i) scatter the key (`Pick` draws random, ignores `hashKey`) → a source IP's 16 conns spread → the subject affinity (`% 16`) leg FAILS; (ii) collapse the ring (all picks → `ring[0]`) → the spread (`>= 2`) leg FAILS (affinity stays intact at 64 % 16 == 0 → spread bites independently, proving it non-vacuous); (iii) corrupt a `StatsAsserter` want (`upstream_cx_total` 64 → 99) → the stats cross-equal prong FAILS; (iv) corrupt the `ring_hash_lb.size` gauge (`rh.size + 1`) → BOTH the cross-side `ref != subj` mismatch AND the `subj != want` value check FAIL. Each: applied → `-count=1` FAIL → `git restore` → diff-clean; never committed broken. Flake check 20/20 (affinity is deterministic; spread `>= 2` over 4 keys / 3 backends is overwhelmingly stable).
- **NO new fuzzer / NO new BackendKind (family expectations).** RING_HASH decodes no untrusted wire bytes (the hash key derives from a source IP / a parsed header value, not a wire frame) → NO new fuzzer (a ring-LOOKUP property test is unit-level — fuzzers STAY 42). `0061` reuses `TCPEcho` → NO new BackendKind (tail STAYS 33). ZERO new packages (`ringhash.go` + `hash.go` in the existing `internal/cluster`) + ZERO new go.mod deps (`xxHash64`/`murmurHash2` hand-rolled — `cespare/xxhash` is absent from go.mod and a new dep for one call site is unwarranted; `go mod tidy -diff` EMPTY).

#### The HTTP route `hash_policy` producer plane (phase 36.2; the SECOND producer — ADR-0237)

*Added by phase 36.2 (the SECOND leg of the CONSUMED 36.1/36.2 by-plane split). The 36.1 leg above lands the seam + the `ringHashLB` + the manager gate + the tcp `source_ip` plane; this 36.2 leg adds the HTTP router's route-level `RouteAction.hash_policy` producer against that ALREADY-LANDED seam. Justified by ADR-0237. A PRODUCER-ONLY leg: NO LB policy / seam / manager / stat-name change — it only STUFFS a ctx-carried hash key before the existing dial.*

**Supported specifiers + the per-request fold (the upstream `HashPolicyImpl::generateHash` mirror).** Per matched route, if `RouteAction.hash_policy[]` (proto field 15) is non-empty, the router folds each supported policy's contribution into a `uint64` ring_hash key and threads it via the LANDED `cluster.WithHashKey(ctx, key)` into `AcquireH1`/`Dial`/`DialH2` → `ringHashLB.Pick`. TWO specifiers are SUPPORTED: **`header`** — `xxHash64` over the matched request-header value (the single-value case, the fixture path); the FULL multi-value case is the seed-chained `XXH64` fold over BYTE-SORTED values (`seed = XXH64(value_i, seed)`, NO separator — NOT a concatenation), via the additive `cluster.HashHeaderValues(values []string) uint64`. The producer shims feed a SINGLE header value (H1 `req.Header.Get`; H2 an hpack-field scan, both lowercased), so the producer plane is single-value; the digest SUPPORTS multi-value and is unit-tested. **`connection_properties.source_ip`** — REUSES the tcp plane's `cluster.HashSourceIP(remoteAddr)` VERBATIM (`xxHash64` of the bare client IP, no port). The multi-policy combine is `hash = rotl64(prev, 1) ^ new` (rotate-left-1 then XOR, policy-list order) with the FIRST contributor taken VERBATIM (`0 ^ new`, NOT rotated — the accumulator starts empty); a `nullopt` policy (absent header / no source IP) is SKIPPED ENTIRELY (no rotate, no XOR); a `terminal=true` policy SHORT-CIRCUITS once the accumulator is non-empty (it checks the ACCUMULATOR, so a terminal policy that itself contributes nothing does NOT short-circuit). A route with no `hash_policy` leaves `ctx` untouched → the LB sees `hasHash==false` → the ring_hash random fallback (byte-stable; every prior fixture unaffected).

**DEPARTURE-rejected specifiers + the empty-`header_name` parity reject.** Parsed + validated at config-build (`hcm/config.go parseRouteHashPolicies`, the tcp_proxy `NewFilter` fail-fast precedent — the proto-read stays in `hcm` so the `router` package stays proto-free; the lowered descriptor `router.HashPolicy` is router-owned). FIVE arms reject: `cookie` / `query_parameter` / `filter_state` (the reference validate-ACCEPTS all three — a RECORDED DEPARTURE, fail-fast: `router: hash_policy specifier %T is not supported (only header, connection_properties.source_ip)`); a configured `regex_rewrite` on a `header` (DEPARTURE — it alters the hashed value); a `source_ip==false` `connection_properties` (DEPARTURE — inert upstream, Envoy adds no hash method). An empty `header_name` PARITY-rejects mirroring the PGV `min_len=1` — `value length must be at least 1 runes` (the go-binding PGV form; envoy-go hand-rolls against the go module). All reject arms land UNIT-LEVEL in `hcm`/`router` config tests (NO cross-side boot-reject dir — the reference accepts, so a cross-side boot-reject is asymmetric; the `0062` fixture is an ACCEPT path).

**NAT-transparent TRUE cross-side affinity (`0062-lb-ring-hash-http`; vs the tcp plane's subject-side-only).** The `X-Hash` request-header key survives Docker's source-IP NAT UNTOUCHED → a SYMMETRIC cross-side proof, the TRUE cross-side affinity differential the tcp `source_ip` plane could NOT be (AMEND-RH8). `0062` routes N=16 distinct `X-Hash` values × K=16 repeats each (256 routed requests) to a 3-endpoint RING_HASH cluster (`ring_hash_lb_config: {}` defaults). The assertions: per-value affinity asserted on BOTH sides via the 36.1 aggregate-count MODULAR INVARIANT (each backend's per-value count is a multiple of K=16, so every per-backend aggregate `% 16 == 0`; a scattered key breaks it); spread `>= 2` distinct backends per side; cross-side byte-equivalence; a cross-side `StatsAsserter`. Cross-side host IDENTITY stays INFEASIBLE (the two sides build the ring over different endpoint address strings) — the pinned property is per-value affinity + spread + byte-equiv + cross-equal stats, NOT host identity. The N=16 (not N=4) workload is a flake-margin choice: with N=4 over 3 backends the values occasionally all land on one backend (spread collapses, `P ≈ 3.7%`/side); N=16 drops the per-side collapse probability to `≈ 7e-8` (past the 5σ-equivalent margin).

**`upstream_rq_total` is CROSS-EQUAL on the HTTP plane (the new prong vs `0061`).** The HTTP router calls `IncUpstreamRqTotal` → BOTH subject and reference increment `cluster.<name>.upstream_rq_total` (= N*K = 256), UNLIKE the tcp plane where the subject stays 0 (the `0059`/`0060`/`0061` boundary). The `0062` `StatsAsserter` reuses the 36.1 cross-equal prongs (the 3 `ring_hash_lb.{size=1026, min=342, max=342}` gauges + `membership_total=3` + quiesced `upstream_cx_active=0`) AND adds the `upstream_rq_total` cross-equal.

**ZERO new stat names — surface STAYS 1119 (the FIRST LB producer reusing the prior plane's stat surface entirely).** The 36.2 HTTP `hash_policy` plane adds NO new stat names — it reuses the 3 `ring_hash_lb.*` gauges (LANDED at 36.1) + the existing `cluster.<name>.*` cx/rq roster. Live-confirmed: the `/stats` name-set subset matching `hash_policy` is EMPTY (`grep -ic hash_policy /stats` = 0). The ONE candidate additive exported `cluster` symbol is `cluster.HashHeaderValues` (the header digest) — additive, no existing-symbol change, the exported `Cluster` surface stays BYTE-STABLE. NO new fuzzer (the HTTP hash key derives from a parsed header value / source IP, not an untrusted wire frame — a key-fold property test is unit-level; fuzzers STAY 42) + NO new BackendKind (`0062` reuses the existing HTTP backends — tail STAYS 33) + ZERO new packages + ZERO new go.mod deps (`RouteAction.HashPolicy` is already on the import graph — `go mod tidy -diff` EMPTY). The conformance gates RE-RAN green (h2spec 53/53 + proxy-wasm 10/10) — NOT zero-touch-by-construction like 36.1 (36.2 DOES touch the H1/H2 router path), but the request/response wire path is byte-identical when no `hash_policy` is configured (every conformance config); the full 64-dir differential is the real guard (the 63 prior dirs byte-exact through the constructor signature change).

### Load balancer — maglev (MAGLEV)

*Introduced by phase 37 (the Load-balancing family's FOURTH row; the project's SECOND consistent-hashing policy). Justified by ADR-0238 (the maglev policy). The seam's FIRST hash-policy REUSE: maglev consumes the ADR-0235 LB hash-key seam + BOTH ADR-0237 producer planes (tcp `source_ip` + HTTP route `hash_policy`) UNCHANGED — swapping the Ketama RING for a fixed-size Maglev lookup table behind the SAME `Pick(hashKey, hasHash)` seam (NO seam ADR, NO producer ADR — the phase-35 single-ADR-on-reuse shape).*

Phase 37 lands the project's FIFTH LB policy (after phase-02 ROUND_ROBIN + phase-34 LEAST_REQUEST + phase-35 RANDOM + phase-36 RING_HASH) and its SECOND consistent-hashing policy. `cluster.Manager` accepts `Cluster.LbPolicy MAGLEV` (`envoy.config.cluster.v3`, enum value 5) on the LEGACY enum path (the same path the other four use; the `Cluster.load_balancing_policy` extension point stays deferred).

**Acceptance + the `MaglevLbConfig` gate (cap + primality — both reference PARITY).** `lb_policy: MAGLEV` with no `maglev_lb_config` (or with `maglev_lb_config: {}` / `table_size` unset) is accepted with the default `table_size 65537` (the manager self-supplies the doc-comment default — the v1.32.4 zero value is nil, NOT generated; 65537 is the Fermat prime 2^16+1). `maglev_lb_config` carries a SINGLE field `table_size` (proto field 1, `UInt64Value`); there is NO `hash_function` field (maglev always uses xxHash64). The HAND-ROLLED gate (the manager calls no PGV — the ring_hash precedent) mirrors the reference's TWO validation layers: (a) the PGV cap — `table_size > 5000011` → `cluster: %q: maglev_lb_config.table_size: value must be less than or equal to 5000011` (the PGV-parity wording); (b) the PRIMALITY gate (checked AFTER the cap, mirroring the reference's PGV-fires-before-app-layer order) — a composite `table_size` (≤ cap, incl 0/1) → `cluster: %q: maglev_lb_config.table_size (%d) must be a prime number` (the house-prefixed form). The primality gate is reference PARITY, NOT an envoy-go-strict departure: the reference itself rejects a non-prime `table_size` with `The table size of maglev must be prime number` (a `--mode validate` matrix confirmed `table_size: 100` → REJECT). The maglev permutation REQUIRES `M` prime for a terminating full-cycle fill (`skip ∈ [1, M-1]` is coprime to a prime `M`, so every per-host permutation is a full `M`-cycle reaching every empty slot). All reject arms land UNIT-LEVEL in `manager_test.go` (NO cross-side boot-reject dir — no fixture pins them; both sides reject identically — a cross-side boot-reject dir is now FEASIBLE but NOT taken, the unit tests are sufficient and lower-cost).

**Maglev lookup-table semantics (the upstream v1.37.2 mirror).** The `maglevLB` (`internal/cluster/maglev.go`) builds an immutable fixed-size `table []int` (each slot an endpoint index) ONCE at construction, mirroring `MaglevTable` EXACTLY: endpoints SORTED by `Addr()` (`"IP:PORT"` = `host->address()->asString()`, port-inclusive) ascending BEFORE the populate loop (the Envoy `std::sort` pre-build — the one structural detail ring_hash did not need; the table layout is sort-order-sensitive); per host `offset = xxHash64(addr) % M` (seed 0) + `skip = (xxHash64Seed(addr,1) % (M-1)) + 1` (seed 1, ∈ `[1, M-1]`); the canonical populate loop fills `table[(offset + skip·next) % M]` host-by-host (in sorted order, one slot per host per iteration under equal weight) until all `M` slots fill (it TERMINATES because `M` prime → each `skip` coprime to `M` → a full `M`-cycle reaches every empty slot). `Pick(hashKey, hasHash)`: `hasHash == true` → `table[hashKey % M]`; `hasHash == false` → a uniform random table index from the injected rng (Envoy's documented no-hash fallback, "effectively the random LB"). For the default 3-equal-host 65537 cluster the per-host slot tallies are 21845 / 21846 (`floor(65537/3)` / `ceil(65537/3)`). The build reuses the existing `xxHash64`/`xxHash64Seed` digests in `hash.go` (ZERO new hash code — the only deviation from ring_hash's per-replica `_<i>`-suffixed key is that maglev hashes the bare `Addr()` with seeds 0 and 1). The table is immutable post-build → `Pick` is read-only / concurrent-safe.

**The seam + producer REUSE (ADR-0235 + ADR-0237 UNCHANGED).** `maglevLB` implements the existing unexported `loadBalancer` interface (`Pick(hashKey uint64, hasHash bool) (Endpoint, func(), error)`) and returns the shared `noopRelease` — exactly as `ringHashLB` does. NO interface change, NO `cluster.go` / pick-funnel / `Dial` / `AcquireH1` change, NO producer-site change. The ADR-0235 PICK-INPUT seam delivers the ctx-carried key to `maglevLB.Pick` IDENTICALLY to `ringHashLB.Pick`; the ADR-0237 HTTP `hash_policy` producer + the tcp `source_ip` producer are policy-AGNOSTIC and UNTOUCHED — they stuff a key into `ctx`; maglev indexes a different structure with it. This is the seam's FIRST hash-policy REUSE (the hash-policy analogue of phase-35 reusing the ADR-0232 release seam) — ZERO seam code/ADR, ZERO producer code/ADR; the single ADR is the policy (ADR-0238). It validates ADR-0235's "the durable asset maglev reuses unchanged" §Consequences claim.

**Healthy-set boundary (no health checking → all-hosts table).** Healthy-host filtering happens upstream of the table lookup in the reference's shared base (priority/healthy/panic); with no health checking, the table is built over the full static endpoint set (the same Upstream-robustness-family boundary as least_request + random + ring_hash).

**Departures + coverage boundaries:**
- **The 2 mirrored `maglev_lb.*` gauges (the SECOND non-zero LB-stat delta; stat surface 1119 → 1121).** MAGLEV is the SECOND LB policy to move the stat surface (after ring_hash; least_request + random were both zero-delta). Two new MIRRORED per-cluster gauges `cluster.<name>.maglev_lb.{min,max}_entries_per_host` (registered + Set once at table build via `registerClusterMetrics`'s `*maglevLB` type-assert — ONLY on the MAGLEV path, reference parity). They are STATIC (immutable table) and CROSS-SIDE-EXACT — `min = floor(M/N)`, `max = ceil(M/N)`, keyed only on `(table_size, host count, weights)`, NOT on endpoint addresses (21845 / 21846 on BOTH sides) — the strong `0063` `StatsAsserter` cross-equal prong (the strong maglev-specific prong). There is NO `size` gauge (the table size is config-known — confirmed absent live). See the `## Stat-name mapping` "Phase 37 extension" block.
- **`upstream_rq_total` cross-equal on the HTTP plane.** On `0063`'s stats prong: cross-equal `maglev_lb.{min=21845, max=21846}` + `upstream_cx_total` (= N·K = 256) + `upstream_rq_total` (= 256, the HTTP router increments it both sides) + `membership_total == 3` + `upstream_cx_active == 0` (quiesced post-drain).
- **Mismatched `lb_config` oneof member — silent-ignore (PARITY).** A stray `lb_config` oneof member under a MAGLEV cluster (e.g. `ring_hash_lb_config` or `least_request_lb_config`) is silently ignored — reference parity (the `--mode validate` matrix accepted `MAGLEV` + a stray `ring_hash_lb_config` SILENTLY, zero warnings) AND behavior-inert (the manager reads `GetMaglevLbConfig()` only on the MAGLEV path).
- **The unused `attempt` retry boundary.** The reference's `attempt > 0` retry-hash mutation (`hash ^= …`) is UNUSED — envoy-go's `Pick` has no `attempt` argument (the single-pick funnel; the ring_hash precedent).
- **The 3 deliberate-break liveness proofs (`-count=1`, go-test caching defeated).** (i) scatter the key (`Pick` draws random, ignores `hashKey`) → an `X-Hash` value's 16 requests spread → the affinity (`≡ 0 mod 16`) leg FAILS; (ii) collapse the BUILT table to host-0 AND corrupt the build gauge tallies (min→0, max→65537) → the spread (`≥ 2`) leg FAILS AND BOTH `maglev_lb.*` gauge cross-equal prongs FAIL — the gauges are the per-host SLOT tally over M slots (figures 65537/0, NOT per-request counts), so the break is deliberately BUILD-level (a weaker Pick-level `table[0]` short-circuit would bite spread ONLY and leave the gauges vacuously green); (iii) corrupt a `StatsAsserter` want (`upstream_cx_total` 256 → 257) → the stats cross-equal prong FAILS. Each: applied → `-count=1` FAIL → `git restore` → diff-clean; never committed broken. Flake check 20/20 (affinity is deterministic; spread `≥ 2` over 16 values / 3 backends collapses with `P ≈ 7e-8`/side).
- **Cross-side host IDENTITY is INFEASIBLE — affinity-provable (the `0062` posture).** The subject/reference tables are built over DIFFERENT endpoint address strings (`"IP:PORT"` differs cross-side), so a given `X-Hash` value may land on a DIFFERENT backend idx per side; the differential asserts the PER-SIDE modular invariant (each per-backend count `≡ 0 mod K`, BOTH sides — the `X-Hash` header is NAT-transparent) + spread (`≥ 2`) + conservation (256) + cross-side `/health` byte-equivalence + a cross-side `StatsAsserter`, NOT host identity. `0063-lb-maglev`: N=16 distinct `X-Hash` values × K=16 repeats (256 routed `GET /get` + 8 `/health`).
- **NO new fuzzer / NO new BackendKind (family expectations).** MAGLEV decodes no untrusted wire bytes (the hash key derives from a parsed header value / source IP, not a wire frame) → NO new fuzzer (a table-build/lookup property test is unit-level, FOLDED into `maglev_test.go` — fuzzers STAY 42). `0063` reuses the `0062` `HTTPEcho` → NO new BackendKind (tail STAYS 33). ZERO new packages (`maglev.go` in the existing `internal/cluster`) + ZERO new go.mod deps (the enum + config are in the pinned `/envoy v1.32.4`; ZERO new hash code — `xxHash64`/`xxHash64Seed` reused; `go mod tidy -diff` EMPTY).

### Load balancer — subset (lb_subset_config)

*Introduced by phase 38.1 (the Load-balancing family's FIFTH construct; the family's FIRST WRAPPER — NOT a `LbPolicy` enum value). Justified by ADR-0239 (the subset-match seam-input extension + the HTTP `RouteAction.metadata_match` producer) + ADR-0240 (the subset LB policy + the `Endpoint.Metadata` dimension). The seam's SECOND pick-input: subset matching widens `Pick(hashKey, hasHash)` → `Pick(hashKey, hasHash, match, hasMatch)`, validating ADR-0235's "future pick-inputs widen `Pick` behind a byte-stable surface" generality.*

Phase 38.1 lands `Cluster.lb_subset_config` (`envoy.config.cluster.v3`, proto field 22) — the project's FIFTH LB construct (after phase-02 ROUND_ROBIN + phase-34 LEAST_REQUEST + phase-35 RANDOM + phase-36 RING_HASH + phase-37 MAGLEV) and the family's FIRST wrapper.

**The wrapper-not-an-enum acceptance (wrap-after-switch, NOT a sixth `case`).** Unlike the five incumbent leaf `LbPolicy` enum values, `lb_subset_config` is an orthogonal `Cluster` field; its presence WRAPS whatever child the `lb_policy` switch produced. `cluster.Manager` builds the leaf LB via the EXTRACTED `buildLeafLB` factory (the `lb_policy` switch lifted to run over an arbitrary endpoint sub-slice), then — when `lb_subset_config` is present — wraps it in a `subsetLB` AFTER the switch (`manager.go buildCluster`). The `Cluster.LbPolicy` supported-list + `TestManager_Error_UnsupportedLBPolicy` are UNTOUCHED — subset is not an enum value, so the phase-34/35/36/37 doubly-hit reject-text-retarget lineage BREAKS (there is no enum value to retire from the rejected set).

**Subset enumeration semantics (the upstream v1.37.2 `SubsetLoadBalancer` mirror).** The `subsetLB` (`internal/cluster/subset.go`) enumerates subsets at construction: per `subset_selectors[].keys` × distinct `envoy.lb` endpoint-metadata value-tuple. Only hosts carrying ALL of a selector's keys participate in that selector's subsets; a host may appear in multiple subsets (it matches every selector whose keys it carries). Each distinct value-tuple under a selector becomes ONE subset; `subsetLB` builds a child `loadBalancer` over that subset's sub-slice via `buildLeafLB`. Scalar values only in the MVP (string / number / bool — list/struct DEPARTURE-rejected on the route side, silently dropped on the endpoint side).

**Value-equality (`ValueUtil::equal` mirror — double-valued numbers, EXACT key-set match).** Numbers are double-valued — int `1` ≡ `1.0` (the reference's `ValueUtil::equal` uses a SINGLE numeric kind, no int/float canonicalization step); bool and string compare by typed direct equality. A route's `metadata_match` resolves to a subset by an EXACT key-set match (the reference's `findSubset` tree walk — NO partial / longest-prefix / superset match): the match's key-set must equal a built subset's selector key-set AND every value must `valueEqual`.

**The 3-way cluster-level fallback.** On a resolution miss, `subsetLB.Pick` applies the cluster-level `fallback_policy`:
- **NO_FALLBACK** → `errNoEndpoints` → the existing 503 `no healthy upstream` / response-flag `UH`. A no-host outcome is NOT a fallback — `lb_subsets_fallback` STAYS 0 on this path (only an ANY_ENDPOINT/DEFAULT_SUBSET fallback host-selection increments it).
- **ANY_ENDPOINT** → a child `loadBalancer` over the FULL endpoint set (spread across all hosts).
- **DEFAULT_SUBSET** → a child over the `default_subset` match; DEGRADES to NO_FALLBACK when the `default_subset` matches zero hosts.

**The per-subset child honours `lb_policy` (all five children).** Because each subset's child is built by `buildLeafLB`, all five children (roundRobin / leastRequest / randomLB / ringHashLB / maglevLB) are exercisable over a subset's sub-slice. A ring_hash / maglev subset child builds its ring / table over JUST that subset's hosts, and the ctx-carried hashKey routes WITHIN the subset (the hashKey passes straight through `subsetLB.Pick` → `child.Pick(hashKey, hasHash, …)`).

**The seam + producer (ADR-0239).** `loadBalancer.Pick` widens to `Pick(hashKey uint64, hasHash bool, match SubsetMatch, hasMatch bool)` — the SECOND pick-input after the ADR-0235 hash key. The five leaf policies absorb `(match, hasMatch)` behaviour-neutrally; only `subsetLB` consumes it. The PRODUCER is the HTTP router's ROUTE-STATIC `RouteAction.metadata_match["envoy.lb"]`, parsed ONCE at config-build (`buildRouterAction` → `parseRouteSubsetMatch` in `hcm/config.go`) into a proto-free `cluster.SubsetMatch` stored on `clusterRouteAction.subsetMatch`. Because the match is ROUTE-STATIC (not request-derived like the `hash_policy` header fold), it is threaded VERBATIM per request via `cluster.WithSubsetMatch` with NO per-request computation — the contrast with ADR-0237's `applyHashKey` fold.

**Healthy-set boundary (no health checking → subsets over the full set).** Healthy-host filtering happens upstream of subset selection in the reference; with no health checking the subsets are computed over the full static endpoint set (the same Upstream-robustness-family boundary as least_request + random + ring_hash + maglev).

**Departures + coverage boundaries:**
- **The 4 mirrored `lb_subsets_*` stats (the FIRST LB stats Inc'd from a `loadBalancer`'s own `Pick`; stat surface 1121 → 1125).** Four per-cluster `cluster.<name>.lb_subsets_*`, subset-only (registered via the `*subsetLB` type-assert in `registerClusterMetrics`): `lb_subsets_active` (gauge) + `lb_subsets_created` (counter) — both Set / Add at build to the distinct-subset count (×1, NOT the reference's 33× artifact); `lb_subsets_selected` (counter, +1 per subset hit from `subsetLB.Pick`) + `lb_subsets_fallback` (counter, +1 per fallback path from `subsetLB.Pick`). The `selected` / `fallback` counters are the FIRST stats a `loadBalancer` increments from its OWN `Pick` path (the maglev/ring_hash gauges are Set at build, not per-pick). See the `## Stat-name mapping` "Phase 38.1 extension" block.
- **The scalar-value-only `metadata_match` MVP boundary.** `parseRouteSubsetMatch` REJECTS a `metadata_match envoy.lb` entry carrying a list / struct value with `router: metadata_match envoy.lb key %q: only scalar values (string, number, bool) are supported` (the `router-metadata-match-nonscalar` arm — unit-level). List / struct values + `list_as_any` are DEFERRED. On the endpoint side `extractEndpoints` silently DROPS non-scalar `envoy.lb` values (a non-scalar endpoint metadata entry simply does not participate in any value-tuple).
- **The `active` / `created` 33×-reference-artifact-vs-×1 magnitude DEPARTURE.** The reference reports `lb_subsets_active` / `lb_subsets_created` with a 33× magnitude artifact; envoy-go pins the semantically-correct ×1 distinct-subset count. This is a RECORDED departure — envoy-go matches the subset SEMANTICS (the distinct-subset count), NOT the reference's 33× figure. Consequently `active` / `created` are NOT cross-side-equal in `0064` (only the request-counted `selected` / `fallback` are asserted cross-side-equal).
- **`CLUSTER_PROVIDED + lb_subset_config` — parity in OUTCOME, ZERO new reject arm.** The reference rejects this combination with `cluster: LB policy CLUSTER_PROVIDED cannot be combined with lb_subset_config`. envoy-go achieves PARITY IN OUTCOME via the pre-existing unsupported-policy gate — CLUSTER_PROVIDED already rejects in `buildLeafLB` (the `default` arm, `unsupported lb_policy …`) BEFORE the wrap is reached. ZERO new reject arm; the message diverges (the policy being unimplemented), the outcome (boot-reject) matches.
- **The NEW `Endpoint.Metadata` dimension.** `Endpoint` grows a `Metadata` field — the parsed `envoy.lb` scalar key→value map captured at `extractEndpoints` — the FIRST per-endpoint dimension beyond `Host:Port`. Locality / weight / priority stay discarded (the locality-weighted / priority candidates' territory). `Addr()` is UNCHANGED, so ring_hash / maglev table keys stay `"IP:PORT"` — a subset child's ring / table is over the same address strings, keyed identically.
- **The TRUE cross-side SET-membership affinity posture (cleaner than maglev/ring_hash).** The `0064-lb-subset` differential is the static-config NAT-transparent TRUE cross-side subset-affinity SET-membership arm: HTTP routes carrying `metadata_match` over `envoy.lb`-tagged endpoints, asserting every served backend ∈ the route's expected subset on BOTH sides + a fallback arm (ANY_ENDPOINT) + cross-side byte-equivalence + the cross-side `selected` / `fallback` stats prong. Because the `metadata_match` is STATIC config (identical on both sides), it is NAT-transparent → the SET-membership property holds TRUE cross-side — strictly CLEANER than maglev/ring_hash's host-identity-infeasible modular-invariant posture (`reference_differential_hash_key_cross_side_infeasible` is INVERTED here — no host-identity obstacle).
- **NO new fuzzer / NO new BackendKind.** Subset decodes no untrusted wire bytes (the metadata derives from validated xDS config) → NO new fuzzer (a subset-resolution property test is unit-level, FOLDED into `subset_test.go` — fuzzers STAY 42). `0064` reuses the `0063` `HTTPEcho` → NO new BackendKind (tail STAYS 33). ZERO new packages (`subset.go` in the existing `internal/cluster`) + ZERO new go.mod deps (the proto fields are in the pinned `/envoy v1.32.4`; `go mod tidy -diff` EMPTY).
- **Deferred surface.** The per-selector fallback machinery (`KEYS_SUBSET` / `fallback_keys_subset`), `single_host_per_subset`, `locality_weight_aware` / `scale_locality_weight`, `panic_mode_any`, `list_as_any`, `metadata_fallback_policy`; the deferred stats `lb_subsets_removed` (no churn) / `fallback_panic` (panic deferred) / `single_host_per_subset_duplicate` (single_host deferred); the `RouteAction.weighted_clusters` + `WeightedCluster.ClusterWeight.metadata_match` producer plane (landed at 38.2 — see `### Route action — weighted clusters` below).

### Route action — weighted clusters (weighted_clusters)

*Introduced by phase 38.2 (the pre-authorized SECOND leg of the phase-38 by-plane split; the 36.1/36.2 precedent). Justified by ADR-0241 (the weighted-cluster routing primitive + the `ClusterWeight.metadata_match` merge composition). The project's FIRST per-request cluster-SELECTION primitive: the SECOND `RouteAction.ClusterSpecifier` arm (after `RouteAction_Cluster`); the FIRST router-layer use of the `randomLB` per-request RNG model OUTSIDE the cluster LB.*

Phase 38.2 lands `RouteAction.weighted_clusters` routing — a NEW per-request weighted-random cluster-SELECTION primitive — plus `WeightedCluster.ClusterWeight.metadata_match`, the per-weighted-cluster subset match that COMPOSES with the 38.1 subset LB (ADR-0239/ADR-0240). Where every prior route resolves a SINGLE `*cluster.Cluster` at config-build (`buildRouterAction` accepted ONLY `RouteAction_Cluster`; `clusterRouteAction` held one cluster), 38.2 adds the SECOND `ClusterSpecifier` arm: a route fans across N weighted clusters, ONE chosen per request by integer weight.

**The SECOND `ClusterSpecifier` arm (a route fans across N weighted clusters, ONE chosen per request by integer weight).** The `RouteAction_WeightedClusters` oneof member → `*WeightedCluster` (`Clusters []*WeightedCluster_ClusterWeight`). Parsed ONCE at config-build by `buildWeightedRouterAction` (the `buildRouterAction` `RouteAction_WeightedClusters` switch arm) into a `weightedClusterRouteAction` (the `clusterRouteAction` sibling): N entries `{cluster *cluster.Cluster, weight uint32, subsetMatch cluster.SubsetMatch}` + the route-level `hashPolicies` (shared across entries) + a per-action `weightedSelector`.

**The selection semantics — weighted-RANDOM (per-request random in `[0, Σweight)` + cumulative walk; `total_weight` deprecated/ignored; explicit `weight: 0` never selected; per-action crypto-seeded PCG RNG — the `randomLB` model).** Each request the selector draws a random value in `[0, Σweight)` and walks the cumulative-weight array to find the chosen entry index. `total_weight` (`WeightedCluster.TotalWeight`, a deprecated `UInt32Value` field) is IGNORED — the client uses the SUM of per-cluster weights (LIVE-confirmed: a non-matching `total_weight` value is accepted-and-ignored; envoy-go matches this outcome). An explicit `weight: 0` on an entry is a valid presence (the `weight` field is a `UInt32Value` PRESENCE-required — total OMISSION rejects; explicit `{value:0}` ACCEPTS) but it can never win the walk while the sum is positive. envoy-go realizes the draw with a `weightedSelector` mirroring `randomLB` (`internal/cluster/random.go`): a per-action mutex-guarded `newPCGRNG()` (`math/rand/v2` PCG seeded from `crypto/rand`, mutex-guarded; boot-fail on the crypto/rand error — the `randomLB` precedent): `idx = walk(cumulativeWeights, uint32(rng() % uint64(Σweight)))`. This is the FIRST use of the per-request RNG model OUTSIDE the cluster LB — a router-layer RNG.

**The `ClusterWeight.metadata_match` merge (merged over route-level `RouteAction.metadata_match`, ENTRY precedence; composes with the 38.1 subset LB via the unchanged ADR-0239 ctx seam; behavior-neutral on a non-subset cluster).** Each `ClusterWeight.metadata_match` (proto field 3 — `*core.Metadata`) MERGES over the route-level `RouteAction.metadata_match` with ENTRY values taking precedence (LIVE-confirmed from the proto semantics). Both are STATIC route config → the merge is ONCE at config-build per entry: `routeScalars ⊕ entryScalars (entry wins on key collision)` → `cluster.NewSubsetMatch(merged)` stored on the entry. Per request, the chosen entry's merged `SubsetMatch` rides the EXISTING ADR-0239 `cluster.WithSubsetMatch` ctx seam UNCHANGED into the chosen cluster's subset selector — NO new seam, NO `loadBalancer.Pick` change, NO per-request fold. On a non-subset cluster the merged match is absorbed behaviour-neutrally (the 38.1 leaf widening ignores the ctx match — the five incumbent leaf policies absorb `(match, hasMatch)` per ADR-0239). On a subset cluster the merged match selects the subset WITHIN the chosen cluster — the full 38.1+38.2 composition. Non-scalar `envoy.lb` values DEPARTURE-reject via the existing `parseRouteSubsetMatch` path (the 38.1 `router-metadata-match-nonscalar` arm, reused per entry + route-level).

**The SIX-arm reject roster (empty-clusters / name-required / cluster_header-unsupported / weight-required / sum-0 / dangling).** envoy-go authors ALL producer rejects (it does NOT run PGV beyond the `clusters min_items:1` already expressed). The rejects (OUTCOME-parity with the reference per ADR-0080; message wording diverges):
- **empty clusters** — `wc.GetClusters()` is empty (the PGV `value must contain at least 1 item(s)` gate is mirrored in OUTCOME).
- **name required** — `ClusterWeight.Name` is empty AND `ClusterHeader` is also empty (covers both the neither-set and the `name: ""` cases; reference: `At least one of name or cluster_header need to be specified`).
- **cluster_header unsupported** — `ClusterWeight.ClusterHeader` is non-empty: REJECTED as unsupported (`cluster_header` is DEFERRED in 38.2; reference: `Only one of name or cluster_header can be specified` when both set; envoy-go rejects `cluster_header` whether or not `name` is also set — a departure-reject).
- **weight required** — `ClusterWeight.Weight` (`*wrapperspb.UInt32Value`) is nil (total omission — a presence check; an explicit `{value:0}` ACCEPTS).
- **sum-0** — the sum of all entry weights equals zero (reference: `Sum of weights in the weighted_cluster must be greater than 0.`).
- **dangling** — a `ClusterWeight.Name` that is not found in the cluster manager (reference: `route: unknown weighted cluster '<name>'`; caught at config-init, NOT deferred to a runtime 404/503).

Reject PRECEDENCE: name/header presence check → weight-required → cluster-exists → sum>0.

**The stat surface is UNCHANGED (1125): weighted selection introduces NO new stat names.** The reference emits NO `*weighted*` stat anywhere (LIVE-confirmed: a `/stats` scrape of a 70/30 weighted-route bootstrap found zero `*weighted*` hits; `grep -i weight` matched ONLY `cluster.<name>.max_host_weight` — an ENDPOINT host-weight gauge present on EVERY cluster, value 1, UNRELATED to the route weight; NOT reflecting the 70/30 config). The per-request cluster selection is observable purely through the PRE-EXISTING per-cluster `upstream_rq_total` counters (which envoy-go already emits per phase 06.1); the split across clusters tracks the configured weights. **38.2 adds ZERO new stat names; surface STAYS 1125.** The `max_host_weight` gauge is NOT mirrored (it is an endpoint host-weight gauge present on every cluster regardless of routing policy — not route-weight-related; unasserted).

**The deferred surface (cluster_header / header-mutation / typed_per_filter_config / host_rewrite / runtime_key_prefix / random_value_specifier.header_name / tcp_proxy plane).**
- `ClusterWeight.cluster_header` (header-named cluster; field 12) — DEFERRED; a `cluster_header`-bearing entry REJECTS as unsupported.
- `ClusterWeight.request_headers_to_add` / `request_headers_to_remove` / `response_headers_to_add` / `response_headers_to_remove` — per-weighted header mutation; DEFERRED; silently IGNORED (accepted-but-inert, orthogonal to the selection primitive).
- `ClusterWeight.typed_per_filter_config` — per-weighted-cluster filter config override; DEFERRED; silently IGNORED.
- `ClusterWeight.host_rewrite_specifier` (`host_rewrite_literal`) — per-weighted host rewrite; DEFERRED; silently IGNORED.
- `WeightedCluster.runtime_key_prefix` — runtime-overridable per-cluster weights (the RTDS/runtime layer); DEFERRED.
- `WeightedCluster.random_value_specifier` (`header_name`) — header-seeded selection RNG (deterministic-by-header selection); DEFERRED; the MVP uses the per-action PCG RNG (per-request random, the `randomLB` model).
- The `TcpProxy.weighted_clusters` plane — weighted clusters on the L4 tcp_proxy; DEFERRED entirely (the HTTP-router plane is the 38.2 leg).

**Departures + coverage boundaries:**
- **The `cluster_header`-unsupported departure-reject.** `ClusterWeight.cluster_header` is ACCEPTED by the reference; envoy-go REJECTS it as unsupported (38.2 does not implement the header-named cluster path). This is a DEPARTURE-reject (the reference accepts; envoy-go over-rejects vs the reference but within the ADR-0080 OUTCOME-parity framing — the outcome on a `cluster_header` route is a boot-reject rather than runtime behavior). Recorded here; to be lifted when `cluster_header` is implemented.
- **Header-mutation / `typed_per_filter_config` / `host_rewrite` silent-ignore.** The reference ACCEPTS entries carrying `request/response_headers_to_*` / `typed_per_filter_config` / `host_rewrite` and applies them; envoy-go ACCEPTS but IGNORES them silently (the accepted-but-inert posture — orthogonal to the selection primitive; no mutation is applied). A narrow coverage boundary: the selection + routing behavior is unaffected; the suppressed mutation is a known gap, to be lifted when per-weighted mutation is implemented.
- **`total_weight` ignored.** `WeightedCluster.TotalWeight` is DEPRECATED upstream; the client uses the SUM of cluster weights. envoy-go IGNORES it unconditionally (no cross-check against the sum, no reject on mismatch). OUTCOME-parity with the reference (the reference deprecation-warns but also ignores it for routing math).
- **The scalar-only `metadata_match` boundary (reusing the 38.1 reject).** `parseRouteSubsetMatch` (reused per entry + route-level) REJECTS a non-scalar `envoy.lb` value (list/struct) with the `router-metadata-match-nonscalar` arm. ZERO new reject arm — the 38.1 boundary extends naturally to the per-entry merge.
- **The per-side-distribution-band differential posture.** The `0065-weighted-clusters` fixture asserts a PER-SIDE distribution band (each cluster's `upstream_rq_total` within ~4–5σ of `N · weight/Σweight`) NOT cross-side per-request equality — because the independent per-request RNG makes cross-side per-request identity infeasible for a randomized policy (the `0060-lb-random` lineage; `reference_differential_band_sigma_margin` discipline). Cross-side per-request identity is infeasible and is NOT asserted.
- **ZERO new stat names.** The FIRST LB-family leg to add NO stat surface (38.2 contributes nothing to the stat surface; 38.1 added the 4 `lb_subsets_*` stats). Observable only through the pre-existing per-cluster `upstream_rq_*`. See the `## Stat-name mapping` Phase 38.2 note.
- **NO new fuzzer.** Weighted-cluster selection decodes no untrusted wire bytes (the weights + cluster names derive from validated xDS config); a selection-distribution property test is unit-level, FOLDED into `router_weighted_test.go` — fuzzers STAY 42.
- **NO new BackendKind.** `0065` reuses the `0063`/`0064` `HTTPEcho` — tail STAYS 33.
- **The FIRST per-request cluster-selection primitive + the FIRST router-layer per-request RNG.** Every prior route resolved a SINGLE cluster at config-build; 38.2 is the FIRST per-request choice among clusters (a new SELECT-then-DISPATCH model). The `weightedSelector`'s `newPCGRNG()` is the FIRST use of the per-request RNG model OUTSIDE the cluster LB.

### Cluster — active health checks

*Introduced by phase 39.1 (active HTTP health checking; the keystone leg of the phase-39 by-checker split — 39.1 = the state+runtime+HTTP-checker, 39.2 = the TCP + gRPC checkers reusing the 39.1 substrate). Extended by phase 39.2 (TCP + gRPC checker codecs; the pre-authorized second leg; the active-HC checker roster HTTP+TCP+gRPC is NOW COMPLETE). Justified by ADR-0242 (the host health-state dimension + the cluster-level checker runtime) + ADR-0243 (the health-aware LB pick — Approach A, the build-time-injected health view + panic threshold) + ADR-0244 (the multi-codec prober dispatch; this phase). The project's FIRST per-host MUTABLE state + the FIRST cluster-level BACKGROUND runtime.*

Phase 39.1 lands `Cluster.health_checks` (`envoy.config.cluster.v3`, proto field 8) — active HTTP health checking. Where `Endpoint` was the project's only per-host construct (immutable; `cluster.go:33`) and `internal/cluster` had no background runtime, 39.1 adds BOTH a mutable per-host health dimension and a timer-driven cluster subsystem, then makes the six LB constructs route only to healthy hosts.

**The host health-state model (start-healthy, first-check-immediate, threshold-gated).** A per-cluster `clusterHealth` registry holds a `map[string]*hostHealth` keyed by `Endpoint.Addr()`; each `hostHealth` carries an atomic `healthy` bit + `consecutiveFailures`/`consecutiveSuccesses` counters. `Endpoint` stays an immutable value type — health is CLUSTER state, NOT a field on the picked endpoint copy; the registry is seeded at `extractEndpoints` (one `hostHealth` per endpoint). **Hosts START healthy.** The FIRST check applies its result IMMEDIATELY (no threshold gating on the first observation — healthy on first success, unhealthy on first failure); thereafter the thresholds gate transitions: `consecutiveFailures == unhealthy_threshold` flips healthy → unhealthy (resets the success counter), `consecutiveSuccesses == healthy_threshold` flips unhealthy → healthy (resets the failure counter).

**The HTTP checker (GET path; expected_statuses default 200; interval/timeout; the project's FIRST cluster-level background runtime).** Per cluster × per configured `health_check`, a background goroutine (`healthChecker` in `internal/cluster/health.go`): a ticker at `interval` (after the ~immediate first check); each tick probes every host with a `timeout`-bounded context, classifies the result (success / failure / network_failure), applies the transition, and Inc's the stats. The HTTP probe sends `GET <path>` over a fresh dial (the configured `host` header; default H1), succeeding iff the response status ∈ `expected_statuses` (default `[200,200]`); a connection failure is a `network_failure` (a sub-class of `failure`). The lifecycle: `Manager.StartHealthChecks(ctx)` is called from `cmd/envoy-go/main.go` AFTER `Freeze` (so the host set + stats are immutable/allocated) — every existing unit/differential test that does NOT call it gets NO checkers (zero behavior change); `Manager.Drain()` cancels the root context and WaitGroup-waits for the goroutines to exit (no goroutine leak).

**The TCP checker — connect-only (phase 39.2; ADR-0244).** `tcp_health_check{}` (empty = connect-only; the MVP liveness signal) uses a `net.Dialer{Timeout}.Dial("tcp", addr)` then immediate close. A successful TCP connect ⇒ `(ok=true)` — the `health_check.success` counter increments and the transition model records a healthy result. A refused or timed-out connect ⇒ `(failure, network_failure)` — BOTH `health_check.failure` AND `health_check.network_failure` increment, matching the HTTP checker's dial-failure classification (L4 unreachability is a `network_failure` in both codecs). The result flows through the UNCHANGED `applyResult`/`recordResult` transition model. **`send`/`receive` payload matching is NOT supported** — a `tcp_health_check` that sets `send` or `receive` is DEPARTURE-rejected at config-build: `cluster: %q: health_check: tcp_health_check send/receive payload matching is not supported` (the connect-only MVP boundary; payload send/receive is deferred). `proxy_protocol_config` is also deferred (silently ignored).

**The gRPC checker — `grpc.health.v1.Health/Check` over H2 (phase 39.2; ADR-0244).** `grpc_health_check` issues a unary `grpc.health.v1.Health/Check` RPC over HTTP/2 (insecure h2c; a fresh `grpc.ClientConn` per probe). Classification (the application-vs-network `network_failure` discriminator): a transport/RPC error (`err != nil` — e.g. `codes.Unavailable` on a refused/dead port) ⇒ BOTH `health_check.failure` AND `health_check.network_failure` increment; a completed `Check` with `resp.Status == SERVING` ⇒ `health_check.success` increments (healthy); a completed `Check` with `resp.Status != SERVING` (NOT_SERVING / SERVICE_UNKNOWN / UNKNOWN) ⇒ only `health_check.failure` increments (`network_failure` stays 0 — the backend is reachable but reports unhealthy: an APPLICATION failure, not a network failure). `service_name` (proto field 1) IS SUPPORTED: empty = the overall `""` service; a non-empty `service_name` is sent as `HealthCheckRequest.Service`. `authority` (field 2) and `initial_metadata` (field 3) are DEFERRED — both are silently ignored (no reject). **The cluster MUST declare HTTP/2 support**: a `grpc_health_check` on a cluster WITHOUT HTTP/2 is rejected at config-build: `cluster: %q: grpc_health_check requires the cluster to support HTTP/2` (a config-init reject, NOT PGV — it fires under `--mode validate` as a plain error string).

**The health-aware pick + the panic threshold (default 50%, strict `<`; build-time-injected — NOT a `Pick`-signature widening).** Each of the six LB constructs (`roundRobin`/`randomLB`/`leastRequest`/`ringHashLB`/`maglevLB`; `subsetLB` inherits via its children) carries a build-time-injected `*clusterHealth` view consulted at `Pick`. A construct picks a candidate by its normal rule; if unhealthy it advances (round_robin: next index; least_request: choose the least-loaded HEALTHY host; random: re-draw; ring_hash/maglev: WALK FORWARD on the ring/table to the next healthy host — preserving the key→host stability that defines those policies). When the healthy FRACTION is strictly `< healthy_panic_threshold` (`Cluster.common_lb_config.healthy_panic_threshold`, default 50%) the construct enters PANIC mode: it routes to ALL hosts (better to spread load over a possibly-degraded set than to overload the few healthy ones) and increments `lb_healthy_panic`. The comparison is STRICT `<` (exactly-50% healthy does NOT panic). The health view is build-time-injected — NOT a per-request value — because host health is CLUSTER state: the `loadBalancer.Pick(hashKey, hasHash, match, hasMatch)` signature + the exported `Cluster` surface stay BYTE-STABLE. This is the deliberate CONTRAST with the ADR-0235 hash-key + ADR-0239 subset-match per-request ctx-carry pick-inputs (those WIDENED the pick path; health does NOT). **Behavior-neutral when `health == nil`** (a cluster with no `health_checks`) → the skip loop never fires, panic never triggers; every existing fixture + h2spec 53/53 + proxy-wasm 10/10 stays byte-identical.

**The parse-reject roster (envoy-go's own, no PGV — the 38.2 precedent; byte-stable; all config-build/boot rejects; unit-level).** OUTCOME-parity with the reference per ADR-0080; the IMPL pins exact byte-stable strings. The 39.1 `only http_health_check is supported` DEPARTURE-reject is LIFTED at 39.2 — `tcp_health_check` and `grpc_health_check` are now ACCEPTED:
- **health_checker oneof missing** (reference: `field: "health_checker", reason: is required`).
- **HTTP empty `path`** (reference: `HttpHealthCheckValidationError.Path: value length must be at least 1 characters`).
- **`interval` missing** (reference: `HealthCheckValidationError.Interval: value is required`).
- **`timeout` missing** (reference: `HealthCheckValidationError.Timeout: value is required`).
- **`unhealthy_threshold` (and `healthy_threshold`) missing** (reference: `HealthCheckValidationError.UnhealthyThreshold: value is required`).
- **`tcp_health_check` with `send`/`receive` set** (phase 39.2 DEPARTURE-reject — the deferred-feature boundary): `cluster: %q: health_check: tcp_health_check send/receive payload matching is not supported`.
- **`grpc_health_check` on a non-H2 cluster** (phase 39.2 config-init reject): `cluster: %q: grpc_health_check requires the cluster to support HTTP/2`.

**Departures + coverage boundaries:**
- **The +7 stats (emitted ONLY on clusters with `health_checks`; surface 1125 → 1132; 39.2 adds ZERO new stats — surface STAYS 1132).** Four `cluster.<name>.health_check.{attempt,success,failure,network_failure}` (counters, Inc'd per probe per the ADR-0242 runtime) + `health_check.healthy` (gauge — count of healthy hosts per checker) + `membership_healthy` (gauge) + `lb_healthy_panic` (counter — Inc'd by the ADR-0243 health-aware pick when the healthy fraction is strictly `< healthy_panic_threshold`). Existing fixtures (no `health_checks`) are unaffected. The `0066`/`0067`/`0068` `StatsAsserter` cross-equals the deterministically-converging subset (`membership_healthy`, `health_check.attempt`/`success`/`failure`). The stat set is CODEC-AGNOSTIC — the TCP and gRPC checkers (phase 39.2) add ZERO new stat names; the same seven-name set is emitted regardless of checker codec; stat surface 1132 → 1132 (AMEND-TG4). See the `## Stat-name mapping` Phase 39.1 block.
- **The 39.1 tcp/grpc-checker DEPARTURE-reject is LIFTED at 39.2 (ADR-0244).** In 39.1 a `tcp_health_check` / `grpc_health_check` checker was DEPARTURE-rejected with `only http_health_check is supported`. That reject is LIFTED at 39.2 — both checkers now ACCEPTED (tcp: connect-only; grpc: `grpc.health.v1.Health/Check` over H2). The remaining rejects for these codecs are the 39.2-specific ones listed in the reject roster above.
- **D-S39.2-3 recorded departure (gRPC `err != nil` MVP — every transport error maps to `network_failure`).** The `grpcProber` MVP classifies ALL `err != nil` results as `networkFailure=true`. A codes-based refinement (transport/`codes.Unavailable` ⇒ `network_failure`; a returned gRPC status e.g. `codes.Unimplemented` ⇒ application failure, `networkFailure=false`) is deferred. The `0068` fixture exercises only connect-refused + SERVING; the `codes.Unimplemented` edge is not exercised. Recorded here per ADR-0244 §Consequences.
- **The build-time-injected health view (NOT a `Pick`-signature widening) — the deliberate CONTRAST with ADR-0235/0239.** Host health is injected at BUILD because it is cluster state, NOT a per-request ctx value; the `Pick` signature + the exported `Cluster` surface stay byte-stable. This deliberately CONTRASTS with the ADR-0235 hash-key + ADR-0239 subset-match per-request pick-inputs (which widened `Pick`). The axes compose: a ring_hash/maglev pick resolves the hash key then the walk-to-next-healthy filters the result; a subset child applies the subset match then its injected view filters within the subset.
- **The first per-host mutable state + the first cluster-level background runtime.** `Endpoint` stays immutable (health lives off-`Endpoint` on the `clusterHealth` registry). The `Start.../Drain` runtime lifecycle (post-`Freeze` start; context-cancel + WaitGroup-wait drain) is the TEMPLATE for future timer-driven cluster subsystems.
- **The poll-to-convergence differential posture.** The `0066-health-check-http` / `0067-health-check-tcp` / `0068-health-check-grpc` fixtures (cross-side) each use a poll-to-converge + warmup determinism shape (poll `/stats` on both sides until `membership_healthy == 2` and `health_check.failure ≥ 1`; then a load phase asserting 100% of traffic lands on LIVE backends, the dead host filtered) over endpoints = {2 LIVE backends, 1 DEAD unbound endpoint} (66% healthy > 50% → asserts FILTERING, not panic). 20/20 flake-free each.
- **NO new fuzzer.** The health checker decodes no untrusted wire bytes — the probe targets are validated xDS config; the HTTP + TCP probes classify status/connect results; the gRPC probe decodes via the `grpc_health_v1` library (not hand-rolled). Fuzzers STAY 42. ZERO new packages (`health.go` in the existing `internal/cluster`) + ZERO new go.mod deps (`HealthCheck` in the pinned `/envoy v1.32.4`; `grpc_health_v1` a subpackage of the already-direct `google.golang.org/grpc v1.70.0`).
- **+1 BackendKind at 39.2 (GRPCHealthResponder = 34).** `0068-health-check-grpc` introduces the `GRPCHealthResponder` backend — an h2c server that answers `grpc.health.v1.Health/Check` ⇒ SERVING AND returns 200 to plain-H2 data-plane requests. BackendKind tail 33 → 34. (`0067-health-check-tcp` reuses the existing `HTTPEcho` backend — a TCP connect to its port succeeds; tail STAYS 33 at `0067`.)
- **Deferred surface (recorded departures).** TCP `send`/`receive` payload matching + `proxy_protocol_config`; gRPC `authority` + `initial_metadata` (both silently ignored) + the codes-based `network_failure` refinement (D-S39.2-3); degraded / draining states (`HealthStatus` DEGRADED/DRAINING — MVP HEALTHY/UNHEALTHY only); jitter / interval variants (`no_traffic_interval`, the jitter fields); EDS `health_status`; health-check event logging; the deferred stats (`health_check.{passive_failure,verify_cluster,degraded}`, `membership_{change,degraded,excluded}`, `update_no_rebuild`). Outlier (passive) detection — a future Upstream-robustness row — will feed the SAME `clusterHealth` registry as a passive input. The 3 health-gated Load-balancing-family candidates (locality-weighted LB, priority load balancing, panic thresholds) are UNBLOCKED — the health dimension they depend on exists.

### Cluster — passive health (outlier detection)

*Introduced by phase 40.1 (`consecutive_5xx` keystone — the first leg of the pre-authorized phase-40 by-detector-class split). Justified by ADR-0245 (the `RecordUpstreamResult` seam + the per-host ejection dimension + the `available` LB-pick predicate + the `consecutive_5xx` detector). Builds over the SAME `clusterHealth`/`hostHealth` registry (ADR-0242) + the SAME build-time-injected health-aware LB pick (ADR-0243) established at phase 39. The project's FIRST passive upstream-health construct + the FIRST per-request router→cluster health callback.*

Phase 40.1 lands `Cluster.outlier_detection` (`envoy.config.cluster.v3`, proto field 19) — passive ejection of hosts that return consecutive 5xx responses on real traffic. Where active health checking (phase 39) uses out-of-band probes to drive the `healthy` bit, outlier detection is driven by in-band request outcomes. The two dimensions compose independently: a host is available to the LB only if BOTH `isHealthy` AND NOT `isEjected`.

**The `RecordUpstreamResult` seam (the per-request router→cluster health callback).** `Cluster.RecordUpstreamResult(ep Endpoint, r UpstreamResult)` is called on EVERY upstream response from the TWO LIVE router success sites — `doH1ClusterAction` (`router.go`) and `doH2ClusterAction` (`router_h2.go`), beside the existing `IncStatusClass` call — where `picked` is non-zero and the upstream status code is known. The legacy test-only `(*routerAction).do` / `(*routerActionH2).doH2` drivers are NOT called. Weighted-cluster routes reuse the same `do*ClusterAction` → covered for free. `RecordUpstreamResult` is a **no-op when `c.outlier == nil`** — the hot path for clusters without `outlier_detection` is a single nil check (byte-identical to pre-40.1). `UpstreamResult.LocalOriginErr` (a connect/reset pre-pick failure) is tracked but UNREAD at 40.1 — reserved for the 40.2 local-origin detector.

**The `consecutive_5xx` detector — eject on N consecutive 5xx; reset only by a 2xx from that host.** On each `RecordUpstreamResult` when `c.outlier != nil`: (a) if 5xx (`StatusCode >= 500 && StatusCode < 600`): `consec5xx++`; if `consec5xx >= consecutive_5xx` → a candidate ejection fires (see §cap + §enforce below); (b) if non-5xx (2xx/3xx/4xx): `consec5xx = 0` (reset; ONLY a response FROM THIS HOST resets it — responses from other hosts have no effect). `consecutive_5xx == 0` (or absent when `outlier_detection` is set without the field; the proto default is 5 when the block is present) disables the detector. The per-host counter accumulates across all downstream connections — there is no per-connection isolation.

**The `max_ejection_percent` cap + `ejections_overflow`.** A candidate ejection fires the cap check: `(currentlyEjected + 1) × 100 / totalHosts <= max_ejection_percent` (default 10%). If the cap PERMITS: the host is ejected, the five `ejections_*` stats update (see §stats below). If the cap BLOCKS: `ejections_overflow` increments and the host STAYS in rotation (the counter-reset/threshold side of the detector continues, but no actual ejection occurs). Boundary pinned LIVE: for 1-of-3 hosts (33.33%) a cap of ≥ 34 ejects; cap 33 → overflow. `max_ejection_percent > 100` is PARSE-REJECTED.

**The `enforcing_consecutive_5xx` roll.** Default 100 (always enforce). A value of 0 detects but never enforces (detect-only: `ejections_detected_consecutive_5xx` increments but the cap check and eject are skipped). Intermediate values are a percentage roll. `enforcing_consecutive_5xx > 100` is PARSE-REJECTED.

**The lazy time-based un-eject (recorded departure from the reference sweep).** `clusterHealth.isEjected(ep)` checks the ejected state lazily: if `ejected` is set and `now >= unejectAtNanos`, it atomically clears `ejected`, decrements `ejections_active`, and returns false (un-eject); else returns `ejected.Load()`. Because `isEjected` is called on the pick path for every candidate the LB scans (via `available`), an ejected host automatically rejoins on the next LB scan past it once its `base_ejection_time` elapses — even with NO further traffic to that host (an ejected host gets no requests, so the pick-path check is the load-bearing un-eject path). The ejection duration is `now + base_ejection_time` (flat; NO backoff — AMEND-OD2). **Recorded departure:** the reference un-ejects via a periodic `interval` sweep (measured un-eject ≈ `base_ejection_time + ≤1×interval`); envoy-go uses a lazy, observation-driven un-eject — goroutine-free at 40.1 (the first new outlier goroutine is the 40.3 statistical sweep). `interval` is PARSE-ACCEPTED + validated (`> 0s`) but its sweep-cadence role is DEFERRED. `base_ejection_time` set and `<= 0s` is PARSE-REJECTED.

**The `available = isHealthy && !isEjected` LB predicate (the byte-stable extension).** The five leaf LB consult sites (`loadbalancer.go` round_robin, `random.go`, `ringhash.go`, `maglev.go`, `leastrequest.go`) move their per-pick filter from `isHealthy → available`; `subsetLB` inherits the move via its children. The **panic-fraction denominator** moves from `healthyCount` to `availableCount` (so an all-ejected cluster panics → routes to all, matching the reference); the **`membership_healthy` gauge STAYS `healthyCount`** (active-HC-only — outlier ejection is NOT reflected in `membership_healthy`, matching the reference). When no `outlier_detection` is configured `isEjected` is always false ⇒ `available == isHealthy`, `availableCount == healthyCount`, the panic denominator is unchanged — **byte-identical to pre-40.1** (every pre-40.1 fixture stays green).

**The +5 stats — emitted ONLY on clusters with `outlier_detection`; surface 1132 → 1137.** Scoped `cluster.<name>.outlier_detection.`:
- `ejections_active` — **gauge** (current ejected host count; 0→1 on eject, →0 on un-eject).
- `ejections_enforced_total` — counter (total enforced ejections across all detectors).
- `ejections_overflow` — counter (cap-blocked ejections — the host stays in rotation; these do NOT increment `ejections_enforced_total`).
- `ejections_detected_consecutive_5xx` — counter (threshold crossings, regardless of cap/enforcement).
- `ejections_enforced_consecutive_5xx` — counter (actual 5xx-triggered ejections).

The **double-count** is REPLICATED: a single consecutive-5xx ejection increments BOTH `ejections_enforced_total` AND `ejections_enforced_consecutive_5xx`. Existing fixtures (no `outlier_detection`) are unaffected. The `0069-outlier-detection-consecutive-5xx` `StatsAsserter` cross-equals `{ejections_active, ejections_enforced_total, ejections_enforced_consecutive_5xx, ejections_detected_consecutive_5xx}` (NOT the gateway-detected counter — no gateway detector at 40.1; AMEND-OD4).

**Departures + coverage boundaries:**
- **The lazy un-eject vs the reference interval-sweep (AMEND-OD1).** The reference un-ejects via a periodic `interval` sweep; envoy-go uses a lazy observation-driven un-eject (pick-path check). Under the `0069` continuous-traffic fixture the two are observationally equivalent. The un-eject/recovery arm is NOT cross-side-asserted at 40.1 (timing diverges). `interval` is parse-accepted + validated.
- **Flat `base_ejection_time` vs the reference's backoff×decay (AMEND-OD2).** The reference's `base_ejection_time × num_ejections` linear backoff + its per-non-ejected-`interval`-sweep DECAY are DEFERRED (they require the `interval` sweep). envoy-go 40.1 always un-ejects at `now + base_ejection_time` (flat). `ejectCount` is tracked for stats + future backoff.
- **The gateway-detected-on-503 counter not replicated (AMEND-OD4).** The reference trips `ejections_detected_consecutive_gateway_failure` on 503 (a gateway-failure detector — 503 ∈ {502,503,504}); envoy-go 40.1 has NO gateway detector so it does NOT emit that counter. The `0069` StatsAsserter excludes it.
- **Deferred stat departures.** The legacy `ejections_total`/`ejections_consecutive_5xx`/`ejections_success_rate` + the 14 other-variant `detected_`/`enforced_` counters (gateway/local-origin/failure-percentage/success-rate) are DEFERRED per 40.2/40.3.
- **Deferred ejection-lifecycle features.** The `interval`-driven un-eject sweep (AMEND-OD1); the `base_ejection_time × num_ejections` linear backoff + its decay (AMEND-OD2); `max_ejection_time` + `max_ejection_time_jitter`; `successful_active_health_check_uneject_host`; `always_eject_one_host`; outlier event logging; the `/clusters` per-host ejection readout. All PARSE-ACCEPTED-and-IGNORED at 40.1.
- **The `consecutive_5xx` detector is the ONLY active detector at 40.1.** `consecutive_gateway_failure` + `consecutive_local_origin_failure` + `split_external_local_origin_errors` (which threads `UpstreamResult.LocalOriginErr` through the pre-pick failure sites) are 40.2. The `success_rate_*` + `failure_percentage_*` statistical detectors + the per-interval aggregation runtime are 40.3.
- **`+1 BackendKind (HTTP503Responder = 35)`.** `0069` needs a BackendKind that returns 5xx unconditionally — no existing kind did (AMEND-OD6). `HTTP503Responder` is an in-process always-503 H/1.1 responder (the `acceptHTTPEchoCounting` precedent; writes `backend-<idx>:` body for attribution). BackendKind tail 34 → 35.
- **NO new fuzzer.** Outlier detection reads the existing HTTP status code — no new wire decoder. Fuzzers STAY 42.
...
- LB policies other than ROUND_ROBIN, LEAST_REQUEST, RANDOM, RING_HASH, and MAGLEV — load-balancing family (LEAST_REQUEST landed at phase 34; RANDOM landed at phase 35; RING_HASH landed at phase 36.1; MAGLEV landed at phase 37 — the cheap follow-on reusing the ADR-0235 hash-input seam + the ADR-0237 producers unchanged; the **4** remaining candidates {subset LB, locality-weighted LB, priority load balancing, panic thresholds} stay deferred — see `### Load balancer — least_request (LEAST_REQUEST)` + `### Load balancer — random (RANDOM)` + `### Load balancer — ring_hash (RING_HASH)` + `### Load balancer — maglev (MAGLEV)` below). The 36.2 HTTP route `hash_policy` plane (header + connection-properties source_ip) is the second producer against the ADR-0235 seam (REUSED by MAGLEV unchanged).
- Cluster types other than STATIC (subject side) / STRICT_DNS (reference side, per ADR-0010) — later phases.
- Health-check-driven endpoint selection — **LANDED at phase 39.1 (HTTP) + phase 39.2 (TCP + gRPC); checker roster COMPLETE** (see `### Cluster — active health checks`). Active HC is cluster-scoped, so any upstream cluster carrying `health_checks` filters unhealthy hosts via the shared cluster LB pick path. The three checker codecs (HTTP, TCP connect-only, gRPC `Health/Check`) are all implemented; the Upstream-robustness family STAYS open (outlier detection, circuit breakers, retries + hedging, per-protocol connection pooling remain deferred).
...
*Introduced by phase 29.1 (`network-filter-mongo-wire-and-requests`) — the FOURTH §9 Network-filters-family row (a flat top-level row per ADR-0106) + the family's second stats-PRIMARY filter + consumer #2 of the ADR-0221 `network.WriteFilter` conn-wrap seam (exactly as that ADR's §Consequences anticipated). Justified by ADR-0224 (the `mongo_proxy` filter, request side) + ADR-0225 (the response side + correlation + the `op_query_active` gauge). Cross-side-PROVEN by the now-green `0049-mongo-requests` (request) + `0051-mongo-responses` (response/gauge) fixtures. The response side (OP_REPLY/OP_COMMANDREPLY decode + correlation + the gauge increments + `emit_dynamic_metadata`) LANDED at 29.2 (ADR-0225); fault-delay + access-log + drain + the `cx_destroy_*` close-direction value parity land at 29.3 (ADR-0226).*
...
- **The OP_REPLY/OP_COMMANDREPLY recognized-not-decoded 29.1 boundary is now CLOSED (29.2, ADR-0225).** Reply(1)/CommandReply(2011) were recognized-not-decoded at 29.1 (valid envelopes, consumed, NOT a `decoding_error`, but bodies undecoded + counters dormant). At 29.2 their bodies are decoded and the 5 response counters (`op_reply`, `op_reply_cursor_not_found`, `op_reply_query_failure`, `op_reply_valid_cursor`, `op_command_reply`) are increment-active. The 29.1 posture was a sub-phase staging artifact, not an envoy-go departure — it is now resolved.
- **Response-side decode semantics (29.2, §3.1–§3.2).** `OnWrite` replaces the 29.1 no-op stub: it feeds the write-direction chain bytes into a PRIVATE write-side `writeBuf` (`decodeOnWrite` → `nextWriteMessage` 16-byte LE MsgHeader framing → `decodeResponseMessage` dispatch). The `writeBuf` carries **NO high-water mark** — `writeChainConn.Write` allocates a fresh per-`Write` `Buffer` (the 28.2 read/write asymmetry; `decodeOnWrite` appends `buf.Bytes()` directly), the inverse of the read side's `chainConsumed`-against-`TotalAppended()` mechanism. The write-side envelope is EXACTLY-2-opcode: OP_REPLY(1) + OP_COMMANDREPLY(2011) bodies decoded; ANY OTHER opcode on the response stream → the `decoding_error` path (an OP_QUERY/request-opcode on the write side is as invalid as OP_MSG on the read side — `reference_wire_format_both_sides_see_same_bytes`). OP_REPLY body: `responseFlags`/`cursorID`/`startingFrom`/`numberReturned` + reply docs (walked via `bson.go`); OP_COMMANDREPLY: the upstream-minimal metadata/commandReply pair. The 5 counters charge ONLY after a fully successful body decode (malformed → `decoding_error`, charges nothing). Reply-frame byte minimums (D-S29.2-1, verified vs upstream `codec_impl.cc`/`bson_impl.cc`): empty OP_REPLY = 20 body bytes, empty OP_COMMANDREPLY = 10.
- **requestID↔responseTo correlation + the per-connection-mutex concurrency note (29.2, §3.3–§3.5).** A decoded OP_REPLY correlates by `reply.responseTo == query.requestID` against the 29.1 active-query list via `takeQuery` (FIRST-match-erase). ONLY OP_QUERY created entries (GetMore/Insert/KillCursors/Command did not — so OP_GET_MORE replies are intrinsically uncorrelated, D-S29.2-2); an uncorrelated reply charges only the fixed `op_reply*` counters (touches neither the list nor the gauge); OP_COMMANDREPLY does NOT correlate (it only charges `op_command_reply`). **Concurrency:** the per-connection `dec.mu` (ADR-0223, NARROWED to mongo's single list — the three lockers are `appendQuery`/`takeQuery`/`onDestroy`) guards EXACTLY `dec.queries`; entries copied out by value under the lock, with all gauge/counter math OUTSIDE it. The 29.1 `sniffing bool` is converted to `atomic.Bool` with a CAS-based at-most-once `decoding_error` (`CompareAndSwap(true, false)` in `decoderError()`; D-S29.2-4) so the mutex stays STRICTLY on the list. Proven load-bearing by `TestDecoderConcurrentRequestResponseRace` (`-race -count=5`) + the R9 deliberate-break (removing the locks trips a DATA RACE under `-count=1` — `reference_differential_break_protocol_count1`).
- **The `op_query_active` gauge inc/dec/drain lifecycle (29.2, §3.4) — the project's FIRST differentially-mirrored gauge.** `Inc` at OP_QUERY decode (in `appendQuery`, under the mutex — the list-size↔gauge invariant); `Dec` at a correlated reply (on a `takeQuery` hit, OUTSIDE the lock); `Add(-n)` residual drain at connection destroy (`onDestroy` snapshots the remaining list length under the lock, `Add(-n)` outside; D-S29.2-5). Nets to 0 per connection. The `internal/stats/gauge.go` primitive existed since 06.1 but no differential fixture ever compared a gauge cross-side; 29.2's QUIESCED-POINT assertion design (the `0051` driver waits for the reply round-trip → `op_query_active == 0` both sides at rest; the unanswered → 1-while-open arm is unit-covered, D-P9) establishes the cross-side gauge-comparison pattern.
- **Dynamic-metadata emission (29.2, §3.7; D-S29.2-3 / D-P11 — differential-INVISIBLE).** When `emit_dynamic_metadata` is configured, the decoder ACCUMULATES this pass's collection→operation-strings in a `dynMeta` map; the filter writes ONE `StructValue` via a SINGLE `Bucket.Set("envoy.filters.network.mongo_proxy", "operations", sv)` (the single-Set model — not per-collection Sets), OVERWRITE-cleared per pass, gated (off → no Set). The ADR-0217 Bucket gains its THIRD production writer (after ext_authz + rbac) with ZERO `internal/dynamicmetadata/` change. The emission has zero cross-side stat/body surface → it is proven by UNIT TEST ONLY (no fixture arm) and recorded here as a coverage boundary (AMEND-B11).
- **The dynamic-HISTOGRAM family stays DEFERRED (ADR-0060) — recorded HERE at 29.2.** The per-command/per-collection `reply_num_docs`/`reply_size`/`reply_time_ms` histogram families are unmirrored; 29.2 is the response side that WOULD populate them, so the coverage boundary is recorded at this sub-phase (the parent §15 / project-wide histogram-deferral convention).
- **The `cx_destroy_*` close-direction boundary (D-P4 — value parity DEFERS to 29.3).** `cx_destroy_local_with_active_rq` / `cx_destroy_remote_with_active_rq` get NO increment at 29.2 — they stay exist-at-zero / PRESENCE-ONLY in `0051` (extending the 29.1 AMEND-C2 posture). The network framework records close TYPE, not DIRECTION (`OnDestroy()` carries none; the post-handoff close is detected inside `tcp_proxy`'s two pump goroutines via EOF, recording neither which side closed nor exposing it — `reference_close_direction_framework_gap`); threading direction requires a `tcp_proxy` + `chain.go` touch, kept OUT of 29.2 to preserve framework-ZERO-touch (the 28.2 precedent) + honor ADR-0219 (one ripple). The minimal close-direction accessor + the direction-keyed increment + the `cx_destroy_*` VALUE parity MOVE to 29.3 (ADR-0226), where the async halt/resume seam already opens the pump/terminal/`chain.go` area.
...
- **Stats:** 23 eager fixed stats (22 counters + the `op_query_active` gauge) + the dynamic `cmd.*`/`collection.*`/callsite families. Request-side counters increment at 29.1; the response-side `op_reply*`/`op_command_reply` + the gauge inc/dec/drain are increment-active at 29.2 (+0 creation); `delays_injected` (fault-timer ARM) + `cx_drain_close` (reply-completion drain close) + `cx_destroy_local/remote_with_active_rq` (the D-P4 close-direction VALUE parity) go increment-active at 29.3 (+0 creation — an increment-wiring delta only; no `NewCounterIfAbsent` call added; no `name.go` arm added). Project stat surface **337 → 360** at 29.1 (+23), then **360 → 360** at 29.2 AND **360 → 360** at 29.3 (zero creation delta — increment-wiring only; see the `## Stat-name mapping` 337 → 360 + 360 → 360 [29.2] + 360 → 360 [29.3] blocks).
- **Differential:** `0049-mongo-requests` (9 arms; two listeners `l_default`/mongo_a + `l_commands`/mongo_b; cross-side label-aware `StatsAsserter`; the EXISTING `TCPSink` backend BackendKind = 28 per AMEND-C4 — NO new BackendKind at 29.1; the gauge `# TYPE … gauge` line asserted present; `cx_destroy_*` presence-only per AMEND-C2; the `$comment` callsite AMEND-C3 double-count; R4 deliberate-break recorded with `-count=1`) + `0050-mongo-boot-reject` (the `stat_prefix`-required PARSE-REJECT, symmetric `BootRejectFixture`; reference PGV `MongoProxyValidationError.StatPrefix`; envoy-go `mongo_proxy: stat_prefix is required`). At 29.2 the 39th fuzzer (`FuzzMongoDecode`) is EXTENDED to feed BOTH `decodeOnData` + `decodeOnWrite` (D-P6 — the decoder is direction-agnostic → NO 40th fuzzer); it covers the request + response decoders (no-panic + chain-bytes-unmutated R3 + sniffing-off idempotence AMEND-B6 + bounded readBuf/writeBuf).
- **Differential (29.2):** `0051-mongo-responses` (cross-side; 6 arms; the NEW `TCPMongoResponder` BackendKind = **30** — `acceptMongoResponder`/`mongoRespondLoop`, the `acceptZKResponder` sibling, emitting correlated OP_REPLY/OP_COMMANDREPLY frames whose `responseTo` echoes the request `requestID`; the `0049` label-aware `StatsAsserter` + `scrapeMongoStats`/`scrapeTypeLine` helpers reused as-is). The gauge is asserted at QUIESCED points (`op_query_active == 0` both sides at rest); `cx_destroy_*` PRESENCE-ONLY (D-P4); the R4 deliberate-break recorded with `-count=1`. Live-verified arm totals: `op_query` = 7, `op_reply` = 5, `op_command` = 1, `op_command_reply` = 1, `decoding_error` = 1, the 3 flag counters = 1 each, `op_query_active` == 0 at rest.
- **Differential (29.3):** `0052-mongo-fault-delay` (cross-side; the load-bearing fixture; the `TCPMongoResponder` BackendKind **30** REUSED — no new backend; the `MultiListenerDriver` carries a delayed listener `mongo_d` [`delay: {fixed_delay: 0.100s, percentage: {numerator: 100, denominator: HUNDRED}}`] + a no-delay listener `mongo_nd`). Arms: (1) **fault-delay round-trip PRE + POST handoff** — ≥2 messages on one connection so delay 1 fires pre-handoff (read loop) AND delay 2 post-handoff (`replayRead`, the load-bearing path); `delays_injected{mongo_d} == 2` BOTH sides; both replies received (passthrough-not-broken); timing NEVER compared. (2) **seam non-perturbation (R1)** — `mongo_nd` plain OP_QUERY → reply; `delays_injected{mongo_nd} == 0` BOTH sides (the live R1 equivalence proof, complementing the 53-dir back-compat gate). (3) **`cx_drain_close` PRESENCE-DOWNGRADED** (D-S29.3-8 — present + ==0 both sides; the unit value proof is load-bearing). (4) **`cx_destroy_*` VALUE parity** (D-P4 CLOSED) on `mongo_nd`: (i) LOCAL — driver closes with a query outstanding → `cx_destroy_local_with_active_rq{mongo_nd} == 1` both sides; (ii) REMOTE — `mongoMarkerRemoteClose` triggers the responder to close without replying → `cx_destroy_remote_with_active_rq{mongo_nd} == 1` both sides; (iii) all-answered → neither increments. (5) **all-quiesced roster** — `op_query_active == 0` both prefixes both sides. The R4 deliberate-break recorded with `-count=1` (the `delays_injected` over-assert + the `cx_destroy_*` switch-off + the `cx_drain_close` unit break). Live stat dump (ref + subj BYTE-IDENTICAL): `delays_injected{mongo_d}=2 / {mongo_nd}=0`; `cx_destroy_local_with_active_rq{mongo_nd}=1`; `cx_destroy_remote_with_active_rq{mongo_nd}=1`; `cx_drain_close{*}=0`; `op_query_active{*}=0`.
- **Fault-delay injection (29.3, §3.2; ADR-0226).** The `delay` FaultDelay parsed at 29.1 is consumed mirroring upstream `tryInjectDelay`. **Per-request-message eval at the AS-BUILT FIVE request callbacks** (`decodeQuery`/`decodeInsert`/`decodeGetMore`/`decodeKillCursors`/`decodeCommand` each call `maybeInjectDelay()` at entry) — reconciled from the parent §11.6 upstream SIX (which named a request-path `decodeCommandReply`); `decodeReply`/`decodeCommandReply` are RESPONSE-side and excluded, matching upstream's "NOT decodeReply" (D-S29.3-5). The decoder DECIDES (roll+duration; deterministic `rollPercent` — `>=100%` always, `<=0%` never, a per-instance `*rand.Rand` for intermediate; the phase-09 precedent) at most once per pass; the filter ARMS the timer (`takePendingDelay` once-per-decided → `delays_injected` +1 at ARM → `time.AfterFunc(d, onDelayTimer)`) and returns **StopIteration while the timer pends**. The `delayPending atomic.Bool` re-entrancy guard (set at arm on the read goroutine, cleared at fire on the timer goroutine) suppresses re-evaluation while a delay pends — a multi-message buffer arms at most one delay per pass. `onDelayTimer` clears the guard, nils the timer, and calls `ContinueReading()` (the §async-halt seam resume). `OnDestroy` best-effort `timer.Stop()`s (the at-most-once timer↔destroy discipline). Absent a configured `delay`, mongo's `MayHalt()` is false → the chain is non-haltable → byte-identical (R1). The `0052` differential arms use 100%-probability fixed delays so `delays_injected` is exactly-once-per-delayed-message both sides; the delay DURATION is NEVER compared (timing discipline).
- **The mongo access log (29.3, §3.3; ADR-0226 — differential-INVISIBLE, AMEND-B10).** When `access_log` is configured, one JSON line per decoded message BOTH directions: `{"time":..., "message":..., "upstream_host":...}` (request-direction `full=true` → documents dumped; replies `full=false` → counts). The seam is a pluggable `Formatter func(any)` on `AsyncFileSink` (default `Default(*Record)` → every existing HCM caller byte-identical; `*Record` satisfies `any`) + a `MongoRecord` carrier + a `MongoFormat` formatter. **The timing-bearing differential-invisibility boundary (AMEND-B10):** the `time` field is per-message wall clock → cross-side log comparison is NOT viable → the proof is formatter unit goldens (the timestamp asserted by SHAPE, not value) + this coverage boundary, **NO fixture dir** (the tail stays `0052`). **The upstream-host boundary:** the L4 network filter has no upstream-host handle at the mongo decode point, so the record records the downstream-remote address (or `-` when unavailable) — a recorded coverage boundary, not a silent gap. The HTTP `Default` byte path is the regression gate (the existing accesslog + HCM tests stay byte-identical).
- **`cx_drain_close` reply-completion drain semantics (29.3, §3.4; ADR-0226).** On a correlated reply that EMPTIES the active-query list WHILE the connection is draining → `cx_drain_close` +1 → `Connection().Close(FlushWrite)` (the reply is flushed first; close BETWEEN operations, never mid-query). The drain signal is a minimal structural `DrainState` interface (`Draining() bool`) on `ReadFilterCallbacks`, backed by the phase-08.2 `*drain.Manager` (at the listener `rt.dm`) — NOT a `*drain.Manager` leak; threaded via `SetDrainState(rt.dm)` guarded `if rt.dm != nil` (a typed-nil `*drain.Manager` boxed in the interface is non-nil and would panic). **The `0052` cx_drain_close arm is PRESENCE-DOWNGRADED (D-S29.3-8):** driving a reply-completion into the narrow draining window cross-side is not deterministically reproducible (the docker reference + the subprocess subject; the driver's drive phase has no admin addr) → asserted present + ==0 both sides; the LOAD-BEARING increment-liveness proof is the deterministic UNIT value test (`TestFilter_DrainCloseOnEmptyListWhenDraining` ==1 / `TestFilter_NoDrainCloseWhenNotDraining` ==0).
- **`cx_destroy_*` close-direction VALUE parity — D-P4 CLOSED (29.3, §3.5; ADR-0226).** The 29.2 presence-only boundary is RESOLVED: 29.3 records which `tcp_proxy` pump EOF'd FIRST (downstream→upstream first = LOCAL; upstream→downstream first = REMOTE) into `chainRuntime.closeDir` (atomic first-wins CAS), exposed via `CloseDirection()` on the callbacks; mongo `OnDestroy` reads it and increments `cx_destroy_local_with_active_rq` / `cx_destroy_remote_with_active_rq` IFF the residual active-query list was NON-EMPTY at close (empty → neither, upstream parity). **The non-promoting forwarding trap (`reference_conn_wrap_method_no_promote`):** `prefixConn`/`writeChainConn` embed `net.Conn` as an INTERFACE so `readChainConn.SetCloseDirection` does NOT auto-promote — EXPLICIT forwarding methods on `prefixconn.go`+`writeconn.go` are MANDATORY (`TestCloseDirectionThroughWrapStack` is the guard; without them the recording is silently dead). `0052` re-asserts LOCAL/REMOTE/all-answered VALUE parity cross-side; the pre-handoff LOCAL-keying path is unit-only (the mongo production chain is always terminal — a recorded completeness boundary).
- **Runtime-key gating fully recorded (29.3 — envoy-go-strict; the parent §7.5 / §2.6 boundary).** `mongo.drain_close_enabled` / `mongo.fault.*` / `mongo.logging_enabled` etc. are unmirrored; the filter behaves at the proto-configured values. Recorded HERE at 29.3 (the natural home — fault + drain + logging all consult these keys upstream); supersedes the 29.1-subset runtime-key bullet above.
- **Forward-pointers.** **29.2 (ADR-0225) — DONE.** The OP_REPLY/OP_COMMANDREPLY response decoder in `OnWrite`, requestID↔responseTo correlation, the `op_query_active` gauge inc/dec/drain, and `emit_dynamic_metadata` all LANDED. **29.3 (ADR-0226) — DONE.** The async halt/resume seam (see the NEW `### Network filter chain framework — async halt/resume (29.3 amendment)` subsection below), fault-delay injection (`delays_injected`), the mongo access log, `cx_drain_close`, and the `cx_destroy_*_with_active_rq` close-direction VALUE parity (D-P4 CLOSED) all LANDED (see the 29.3 bullets above); fixture `0052-mongo-fault-delay`; the parent-row-29 ROLLUP (the FOURTH §9 Network-filters family row CLOSED). **The §9 family stays OPEN — after phase 29, 3 candidates remained (`redis`/`kafka_broker`/`thrift`); `kafka_broker` LANDED at phase 31, leaving 2 (`redis`/`thrift`).**

### envoy.filters.network.kafka_broker

*Introduced by phase 31 (`network-filter-kafka-broker`) — the FIFTH §9 Network-filters-family row (a flat top-level row per ADR-0106; after the phase-26 family-parent, the phase-27 `sni_cluster` row, the phase-28 `zookeeper_proxy` parent, the phase-29 `mongo_proxy` parent; the phase-30 pin-refresh was an infra row, not a family member) + the family's third stats-PRIMARY both-direction sniffer + consumer #3 of the ADR-0221 `network.WriteFilter` conn-wrap seam (after zookeeper_proxy #1 + mongo_proxy #2). The project's FIRST `github.com/envoyproxy/go-control-plane/contrib v1.32.4` go.mod consumer (the dep added WITH its first consumer; phase 30 / ADR-0227 was the standalone contrib-image pin-refresh prerequisite that unblocked the cross-side differential). Justified by ADR-0228 (the `kafka_broker` filter). Cross-side-PROVEN by the now-green `0053-kafka-requests` (request + response + correlation + unknown + failure arms) + `0054-kafka-boot-reject` fixtures.*

`kafka_broker` (`@type` `type.googleapis.com/envoy.extensions.filters.network.kafka_broker.v3.KafkaBroker` — type URL DERIVED via `proto.MessageName` on `kafka_brokerv3.KafkaBroker`, NOT hand-typed; the `extensions.` segment is intrinsic per `reference_network_filter_typeurl_extensions`) is a passive observability sniffer that decodes the Kafka request/response wire HEADER (generic across all API keys) flowing through a `[kafka_broker, tcp_proxy]` chain and emits FULL per-API-key request/response counter parity in BOTH directions. It NEVER mutates bytes, never halts iteration (no fault-delay / async-halt analog — ZERO-touch on the ADR-0226 seam; `MayHalt()` not implemented → byte-identical R1), never closes — its entire operator-visible surface IS its stats (the body differential is intrinsically vacuous; the cross-side label-aware `StatsAsserter` is the load-bearing proof). It is the 9th built-in network filter and implements BOTH `ReadFilter` and `WriteFilter` as ONE instance (consumer #3 of the ADR-0221 seam — the `OnWrite` response decoder is what qualifies the chain for the 28.1b post-handoff read seam per `reference_network_chain_terminal_handoff_ends_ondata`).

- **The HEADER-ONLY decode envelope, both directions (§3; AMEND-K5 / D31-3).** Each Kafka message on the wire is a 4-byte INT32 length prefix followed by the message bytes. The **request header** (decoded on the read goroutine in `OnData`) carries `api_key` INT16 / `api_version` INT16 / `correlation_id` INT32 / `client_id` NULLABLE_STRING / tagged-fields (the tagged-fields present iff the header is flexible — see `flexibleVersions` below). The **response header** (decoded on the write goroutine in `OnWrite`) carries `correlation_id` INT32 / tagged-fields ONLY — it is COUNT-AND-FRAME-SKIP: the response side reads only the `correlation_id` (recovering the api_key by correlation), then skips the rest of the frame by its length prefix. Per-API-key BODY decode is NOT performed (the header carries everything the per-key counters need — the api_key IS in the request header; the response side recovers it by correlation). Wire framing is adopted VERBATIM from upstream (`reference_wire_format_both_sides_see_same_bytes`). Partial frames are reassembled in per-direction private buffers (a trailing partial message is never an error — upstream parity).
- **The `flexibleVersions(api_key)` tagged-fields-in-header predicate + the ApiVersions(18) response special-case (AMEND-K5 / D31-3; Kafka source 3.9.1).** The request header is "flexible" (carries UNSIGNED_VARINT tagged-fields after `client_id`) iff `api_version ∈ flexibleVersions(api_key)` — a static per-key predicate transcribed from the Kafka 3.9.1 message JSON (SPEC Appendix C). The response header carries tagged-fields under the same predicate WITH ONE SPECIAL CASE: the **ApiVersions(18) response header suppresses tagged fields** regardless of version (upstream's `ApiVersionsResponse` quirk). Because the response decoder is count-and-frame-skip on `correlation_id` ONLY (it never reads response tagged-fields), the ApiVersions(18) special-case is IMMATERIAL to the count path — it is nonetheless implemented + table-tested for completeness (D-PLAN-3).
- **The `correlation_id → (api_key, api_version)` per-connection correlation map (§3; ADR-0223 mutex; the mongo-29.2 precedent).** The request decoder REGISTERS `correlation_id → (api_key, api_version)` after reading the (api_key, api_version, correlation_id) triple but BEFORE the `client_id` NULLABLE_STRING attempt (the upstream `expectResponse`-on-`onFailedParse` parity — a malformed `client_id` length is still a registered, expected response; D-PLAN-3). The response decoder RECOVERS + ERASES the entry by `correlation_id`; an unregistered `correlation_id` on the response stream → `response.failure` (upstream's `getResponseSpec` throws). The map is shared across the read goroutine (request decode in `OnData`) and the write goroutine (response decode in `OnWrite`), guarded by a per-connection mutex (ADR-0223, NARROWED to the single correlation map) — proven load-bearing by the `-race` concurrent test (Task 8).
- **The EAGER 176-counter roster under `kafka.<stat_prefix>.` (AMEND-K1/K3/K7; D31-2; the 9th built-in).** All 176 counters are created EAGERLY at config parse under scope `kafka.<stat_prefix>.` via `NewCounterIfAbsent` (post-Freeze-permitted, idempotent across listeners sharing a prefix — the mongo `newMongoStats` precedent). The roster = **86 per-key `request.<root>_request` + 86 per-key `response.<root>_response` + 4 fixed** (`request.unknown` / `request.failure` / `response.unknown` / `response.failure`). The per-key segment is the `_request`/`_response`-SUFFIXED message-name snake-case (`request.produce_request`, NOT `request.produce` — AMEND-K1). The 86-key roster is a STATIC table (Kafka 3.9.1; `api_keys` 71/72 telemetry-EXCLUDED → 86 of the 88 keys; SPEC Appendix B + the D-PLAN-1 per-key `maxVersion` column) — normatively defined by `internal/filter/network/kafkabroker/apikeys.go::apiKeyRoster()` + the golden test `TestApiKeyRoster_MatchesUpstream`. The roster is normatively defined by `internal/filter/network/kafkabroker/stats.go::newKafkaStats` + `TestStatRoster`. The 86 response-duration histograms are DEFERRED per ADR-0060 and NOT created (NOT counted). **The `stats.IsValidName` charset guard (AMEND-K7 / D31-5; `reference_dynamic_stat_name_charset_guard`):** the per-key stat segment is table-bounded (looked up from the static 86-key table, never a raw wire string) so the guard is satisfied BY CONSTRUCTION; only the config-derived `stat_prefix` needs the config-boundary `stats.IsValidName` guard (the rbac/mongo precedent, applied at config parse).
- **`request.unknown` / `response.unknown` semantics — keyed on (api_key, api_version) (AMEND-K4 / D-PLAN-1).** `*.unknown` charges when the decoded (api_key, api_version) does NOT resolve to a roster key — an unknown api_key OR an unknown VERSION of a known key (`api_version > maxVersion(key)`). `minVersion` is treated as 0 (the deprecated-low-version → unknown sub-case is NOT modeled — a recorded coverage boundary; the `0053` unknown-version arm tests the HIGH side with `api_version = 0x7FFF`). The response side routes to `response.unknown` when the recovered api_key is unknown.
- **`request.failure` / `response.failure` semantics — a decode exception per offending frame + decoder RESET (AMEND-K4 / D-PLAN-2; matches upstream `broker/filter.cc` `decoder->reset()`).** A malformed request header (e.g. a bad `client_id` NULLABLE_STRING length) → `request.failure`; an unregistered-`correlation_id` response → `response.failure`. The posture is per-exception `*.failure` + decoder RESET (resync the direction's private buffer) + Continue — NOT mongo-style lifetime sniffing-off. The correlation map is NOT cleared on a decode reset.
- **Prometheus exposition is FULLY INLINED with EMPTY labels (AMEND-K2 / D31-2 — REFUTING the BRAINSTORM label-hoist hypothesis).** The NEW `kafka.` INLINE-PREFIX arm at `internal/stats/name.go` (`strings.CutPrefix(internal, "kafka.")` → `envoy_kafka_` + dots→underscores) renders `kafka.<sp>.<rest>` as `envoy_kafka_<sp>_<rest flattened>{}` with an EMPTY label set — the zookeeper `.zookeeper.` INLINE shape generalized to a `kafka.` ROOT prefix, NOT the mongo `.rbac.`-style label-hoist tag-extractor. `prom.go` stays BYTE-UNTOUCHED; `name.go` is the only `internal/stats/` file changed.
- **The four active features PARSE-ACCEPTED-behavior-DEFERRED (D31-7; the mongo `header_delay` precedent).** `force_response_rewrite`, `id_based_broker_address_rewrite_spec`, `api_keys_allowed`, `api_keys_denied` are parsed + PGV-validated at config-load but NOT consumed at runtime (the filter only OBSERVES). ⚠️ **The broker-address-rewrite write-mutation is a FUTURE FRAMEWORK-SURGERY sub-phase boundary** — it would MUTATE the write buffer, a NEW framework capability never built (every prior §9 sniffer only OBSERVED bytes); deferred per the parent design, recorded here as a coverage boundary.
- **Boot-reject (D31-7).** `stat_prefix` is PGV-required (min 1 rune → boot-reject; byte-stable `kafka_broker: stat_prefix is required` per ADR-0080); `api_keys_allowed`/`api_keys_denied` each ∈ [0, 32767]; the `broker_address_rewrite_spec` oneof typed-nil arm + the nested `IdBasedBrokerRewriteRule` (`host` required, `port` ≤ 65535) are PGV-validated. `0054-kafka-boot-reject` carries the `stat_prefix`-required arm (both sides reject at boot); the api_keys-range / nested-rule arms are unit-test-only (D-P3).
- **ZERO framework-seam extension + close-direction (D31-6).** Phase 31 is a pure consumer of the as-built §9 framework (ReadFilter + WriteFilter chains, the 28.1b post-handoff read seam, the ADR-0223 per-connection mutex, the eager-roster + `name.go` tag-extractor machinery). NONE of the kafka counters is close-direction-keyed (no `cx_destroy_*` analog cross-confirmed live + source; `reference_close_direction_framework_gap` does not bite) → kafka_broker stays framework-ZERO-touch.
- **Runtime-key gating unmirrored (envoy-go-strict).** The filter behaves at the proto-configured values; runtime keys are at defaults (the §9 family runtime-key boundary).
- **Stats:** 176 eager counters (86 `request.<root>_request` + 86 `response.<root>_response` + 4 fixed). Project stat surface **360 → 536** (+176; the 86 response-duration histograms DEFERRED per ADR-0060 are NOT counted — see the `## Stat-name mapping` 360 → 536 block).
- **Differential:** `0053-kafka-requests` (cross-side `StatsAsserter`, 6 arms — request / unknown-key / unknown-version / request-failure / response / unregistered-correlation-response-failure; the NEW `TCPKafkaResponder` BackendKind = **31**, a correlation-id-echoing Kafka responder; R4 deliberate-break liveness recorded with `-count=1`) + `0054-kafka-boot-reject` (the `stat_prefix`-required PARSE-REJECT, symmetric `BootRejectFixture`). The 40th fuzzer `FuzzKafkaDecode` covers both decode directions (no-panic + no-mutation R3 + bounded-buffer).

**The `*.failure` cross-side asymmetry — a documented departure/coverage boundary (the abandon-at-close model; discovered at the phase-31 IMPL Task 13).** The two `*.failure` roots (`request.failure`, `response.failure`) are deterministically UNEQUAL cross-side. For BOTH the malformed request (`0053` arm a4) and the unregistered response (arm a6): the **SUBJECT** (header-only sniffer) counts exactly ONE failure per offending complete frame; the **REFERENCE** (full version-specific parser) counts the offending frame as +1 AND THEN enters a broken decoder state that ABANDONS the buffered bytes at connection teardown → +1 more → **2**. So `request.failure` = ref 2 / subj 1 and `response.failure` = ref 2 / subj 1, each per-side DETERMINISTIC (single offending frame, many runs). The `0053` fixture therefore pins the EXACT per-side value via `AssertStats` (ref=2, subj=1) — non-vacuous and R4-breakable on each side — rather than asserting a fragile cross-side equality. The +1 reference excess is the broken-stream abandon-at-close, a reference-only failure the header-only subject does not model — analogous to `reference_close_direction_framework_gap` (a framework-level coverage boundary, pinned to exact per-side values here rather than presence-only). This is the one genuine cross-side behavioral departure of the phase; every other `0053` arm is exact-value cross-side parity.

**Coverage boundaries / departures (phase 31):**
- **The 86 response-duration histograms are UNMIRRORED (ADR-0060).** The per-API-key `<root>_response` duration histogram family is deferred under the project-wide histogram deferral; only the 176 counters are created (NOT the 86 histograms) → stat surface 360 → 536 (NOT +262).
- **The four active features are PARSE-ACCEPTED-behavior-DEFERRED.** `force_response_rewrite` / `id_based_broker_address_rewrite_spec` / `api_keys_allowed` / `api_keys_denied` are parsed but inert; the broker-address-rewrite WRITE-MUTATION is a future framework-surgery sub-phase (it would mutate the write buffer — a capability never built).
- **The "leftover-body-bytes → unknown" sub-case is UNMIRRORED.** A request whose header parses but whose body has trailing bytes upstream routes to `unknown`; the header-only sniffer does not body-decode, so it cannot detect this (the one true body-decode coverage boundary; D31-4).
- **The deprecated-low-version → unknown sub-case is UNMIRRORED (D-PLAN-1).** `minVersion` is treated as 0; an `api_version` below a key's real minVersion is counted as the key (not `unknown`). The `0053` unknown-version arm tests the HIGH side (`0x7FFF`) only.
- **The eager-vs-per-connection boot-window difference.** envoy-go creates the 176-counter roster EAGERLY at config-load (exist-at-zero from boot); upstream creates per-connection. UNOBSERVABLE to the differential (every `0053` assertion runs post-first-connection) — a recorded boundary, not a silent gap (the mongo D-P1 precedent).
- **The per-exception-reset decode posture (D-PLAN-2).** A decode exception resets the offending direction's buffer (resync) + Continue — NOT lifetime sniffing-off (the mongo posture); matches upstream `broker/filter.cc` `decoder->reset()`. The correlation map survives a reset.
- **The response-side tagged-fields no-op (D-PLAN-3).** The response decoder reads `correlation_id` ONLY (count-and-frame-skip) → never reads response tagged-fields → the ApiVersions(18) response special-case is immaterial to the count path (implemented + table-tested for completeness).
- **The `*.failure` cross-side asymmetry** (ref=2 / subj=1 abandon-at-close; see the dedicated paragraph above).
- **Runtime-key gating unmirrored; api_keys 71/72 telemetry-EXCLUDED** (86 of 88 keys carry counters — upstream parity; the two excluded keys are upstream's telemetry/push-telemetry pair).

### envoy.filters.network.redis_proxy

*Introduced by phase 32.1 (`network-filter-redis-upstream-pool-seam-and-codec`) — the first half of the SIXTH §9 Network-filters-family row (a parent pre-split row 32 with sub-rows 32.1 + 32.2 per ADR-0106(d); after the phase-26 family-parent, the phase-27 `sni_cluster` row, the phase-28 `zookeeper_proxy` parent, the phase-29 `mongo_proxy` parent, and the phase-31 `kafka_broker` flat row; the phase-30 pin-refresh was an infra row). EXTENDED by phase 32.2 (`network-filter-redis-commands-and-stats`) — the second half: the full command set + the per-command/splitter/cluster roster + the `redis.` Prometheus arm + the 2 lifecycle gauges' inc/dec + the differential command matrix + the 41st fuzzer + the parent-row-32 ROLLUP. The project's FIRST terminal routing proxy. Justified by ADR-0229 (the redis_proxy filter umbrella — ACCEPTED; both halves landed). Cross-side-PROVEN by the now-green `0055-redis-roundtrip` (the 32.1 PING/SET/GET round-trip arms + the 32.2 command matrix [GET-miss/INCR/DEL/ECHO/QUIT/HELLO-err/UNKNOWN/PING-arg] + the per-command/splitter `/stats/prometheus` assertions + the `cx_active` held-open gauge arm + the downstream-RESP byte-equivalence prong) + `0056-redis-boot-reject` fixtures.*

`redis_proxy` (`@type` `type.googleapis.com/envoy.extensions.filters.network.redis_proxy.v3.RedisProxy` — type URL DERIVED via `proto.MessageName` on `redis_proxyv3.RedisProxy`, NOT hand-typed; the `extensions.` segment is intrinsic per `reference_network_filter_typeurl_extensions`) is the project's FIRST **terminal routing proxy**. It IS the connection terminator — implements `network.TerminalFilter` and owns the downstream `net.Conn` via `Handle(ctx, conn)`. There is NO `tcp_proxy` behind it. Every prior §9 network filter either OBSERVED a `tcp_proxy`-owned connection (zookeeper/mongo/kafka_broker) or published an override the `tcp_proxy` consumed (sni_cluster); redis_proxy IS the terminator.

- **Terminal-routing-proxy semantics.** `Handle` owns the downstream `net.Conn` for its lifetime. It reads RESP request frames from a `bufio.Reader` over the downstream conn (blocking on partial frames — the STREAMING-READER model, contrasted with the mongo/kafka observer PRIVATE-BUFFER model; D-P32-4), dispatches each command to either a local-reply path (PING/AUTH) or the upstream-pool seam (all other commands), and writes replies back to the downstream conn. On `io.EOF` → clean close. On decode error → close (the `downstream_cx_protocol_error` counter increment path is 32.2). The async halt/resume seam (ADR-0226) is NOT consumed — redis_proxy injects no fault delay at 32.1 (the MVP has no `MayHalt()` implementation → never-halting → byte-identical R1).

- **The RESP codec (SPEC §3.5; `resp.go`).** Request decode recognizes two frame shapes: INLINE (`<cmd>\r\n` — e.g. `PING\r\n`) and ARRAY-OF-BULK (`*<n>\r\n$<len>\r\n<bytes>\r\n...` — the canonical command encoding). Reply decode reads one frame keyed on the RESP value-type first byte: `+` (simple string — framing-only; whole line), `-` (error — framing-only), `:` (integer — `strconv.Atoi`-validated; `errProtocol` on non-numeric — the Task 6 deviation from a strict framing-only parse), `$<len>` (bulk string + null sentinel `$-1\r\n`), `*<n>` (array, recursive). Wire bytes are forwarded VERBATIM to downstream (`reference_wire_format_both_sides_see_same_bytes`, EXTENDED to the downstream RESPONSE the proxy generates — the §9 FIRST). The inline request path guards against reply-type first bytes (`+`/`-`/`:`/`$`/`@`) → `errProtocol` (a downstream reply cannot be parsed as a command — the Task 5 inline guard; controller-approved deviation).

- **`catch_all` single-cluster routing + unknown-cluster tolerance (§3.3; §4.3).** `prefix_routes.catch_all_route.cluster` is the ONLY routing target at 32.1 (`prefix_routes.routes[]` longest-prefix multi-cluster routing is 32.2). The cluster name is stored at config-parse time; on the FIRST proxied command `Handle` resolves it lazily via `cm.Get(name)` → a miss (unknown cluster at runtime) → graceful downstream close, NO `-ERR` reply (the unknown-cluster-tolerance posture; D-S32.1-6). This contrasts with `tcp_proxy`, which rejects an unknown cluster reference AT BOOT (the `tcpproxy: cluster %q not found` reject). The dial closure over the resolved `cluster.Cluster.Dial` is built at first-proxied-command time and passed to the upstream-pool seam (`network.NewUpstreamConn`).

- **PING/AUTH local-reply set (AMEND-R5; `commands.go`).** PING → `+PONG\r\n` (7 bytes); AUTH with no configured password → `-ERR Client sent AUTH, but no password is set\r\n`. Both answered LOCALLY — ZERO upstream traffic (the seam is never dialed for PING/AUTH-only connections). Both increment `downstream_rq_total` + the rx/tx byte counters. The 32.1 local-reply set is PING + AUTH; ECHO/TIME/QUIT/HELLO are 32.2 (proxied through the seam at 32.1 if a client issues them — they reach the backend).

- **Config parse + PGV/runtime reject arms (ADR-0080 byte-stable; SPEC §6).** THREE PGV hard rejects: (a) `stat_prefix` min-1 → `redis_proxy: stat_prefix is required`; (b) `settings` nil → `redis_proxy: settings is required`; (c) `settings.op_timeout` nil → `redis_proxy: settings.op_timeout is required`. ONE runtime reject: `catch_all_route.cluster` absent AND no `routes[]` → `redis_proxy: cannot configure a redis-proxy without any upstream`. The `stats.IsValidName(stat_prefix)` guard fires before stat creation (the `reference_dynamic_stat_name_charset_guard` applied at the config-parse boundary). `0056-redis-boot-reject` carries the `stat_prefix`-required arm (both sides reject at boot with a common substring `redis_proxy` in their real boot stderr — liveness proven by R6 break-liveness with `-count=1`); the other arms are unit-test-only (the `0047`/`0050`/`0054` boot-reject precedent).

- **Stats: 10 fixed downstream cx/rq counters + gauges (SPEC §7.2).** All 10 created EAGERLY at config-parse under `redis.<stat_prefix>.` via `NewCounterIfAbsent`/`NewGaugeIfAbsent`. **4 counters increment at 32.1:** `downstream_cx_total` (+1 per accepted downstream connection), `downstream_cx_rx_bytes_total` (+= raw request bytes read from downstream), `downstream_cx_tx_bytes_total` (+= reply bytes written to downstream), `downstream_rq_total` (+1 per decoded request, including PING/AUTH). **2 counters created-not-incremented (32.2):** `downstream_cx_drain_close` (drain path), `downstream_cx_protocol_error` (decode-error path). **4 gauges created-not-incremented (32.2):** `downstream_cx_active`, `downstream_cx_rx_bytes_buffered`, `downstream_cx_tx_bytes_buffered`, `downstream_rq_active` — the project's SECOND differentially-mirrored gauge family (after mongo's `op_query_active`); their inc/dec lands at 32.2. Normatively defined by `internal/filter/network/redisproxy/stats.go::rosterSuffixes()` + `TestStatRoster32_1_MatchesUpstream`. Project stat surface **536 → 546** (+10).

- **Upstream traffic stats via the EXISTING cluster roster (AMEND-R6).** `upstream_cx_total`/`upstream_cx_active`/`upstream_rq_total` are the cluster's OWN traffic stats under `cluster.<name>.*` (`internal/cluster/manager.go` — the per-cluster roster, NOT a new redis namespace). The seam reuses them: `Cluster.Dial` Incs `upstream_cx_total`/`active` on the lazy dial; the per-request hook (`Cluster.IncUpstreamRqTotal`) Incs `upstream_rq_total` per proxied command. The `0055` proxied arm asserts `cluster.redis_cluster.upstream_cx_total == 1` (one lazy dial) + `upstream_rq_total == 2` (SET + GET) cross-side EQUAL (single-conn scenario; the multi-conn divergence is 32.2, D-P32-9).

- **Differential: `0055-redis-roundtrip` (cross-side) + `0056-redis-boot-reject` (boot-reject).** `0055`: chain `[redis_proxy]` as TERMINAL on BOTH sides (no `tcp_proxy` behind it); `TCPRedisResponder` BackendKind **32** (FIFO/positional canned RESP replies: SET→`+OK\r\n`, GET→`$3\r\nbar\r\n`). Implements `fixture.StatsAsserter` (the `0053`/`0053-kafka-requests` precedent — cross-side dispatch per `reference_differential_asserter_dispatch`) scraping the flat `GET /stats` admin endpoint on both sides. 32.1 arms: (a) PING local-reply: `+PONG\r\n` byte-identical cross-side + `downstream_rq_total` +1 + `upstream_cx_total == 0`; (b) proxied SET→`+OK\r\n`, GET→`$3\r\nbar\r\n` byte-identical cross-side + stats cross-side equal. The downstream-RESP byte-equivalence prong is load-bearing (the §9 FIRST — redis_proxy GENERATES the downstream response; `reference_wire_format_both_sides_see_same_bytes`, extended). R4 deliberate-break liveness recorded with `-count=1` (5 breaks: counter==99, wrong-byte PONG, wrong-byte OK, wrong-byte GET reply, upstream_cx_total==2 — each FAIL then revert→PASS). 32.2 extends `0055` with the command matrix + splitter arms (same dir). `0056`: symmetric `BootRejectFixture` (`stat_prefix` missing → both sides reject at boot).

- **The 10th built-in + ZERO new go.mod dep (AMEND-R1 / D32-1).** Registered in `internal/filter/network/builtins/builtins.go` (doc "nine" → "ten") + `main.go` parity; the `redis_proxy/v3` blank-import in `internal/bootstrap/bootstrap.go` holds the TypeURL in the proto registry. redis_proxy/v3 is a subpackage of the ALREADY-DIRECT `/envoy v1.32.4` module → `go mod tidy` adds NOTHING (the opposite of kafka_broker's first-`/contrib` dep).

- **Close-direction-zero-touch (AMEND-R9).** NO close-direction-keyed counters (only `downstream_cx_drain_close` — a drain counter — NOT a `cx_destroy_*` keyed on LOCAL/REMOTE). The 29.3 `CloseDirection()` machinery is NOT consumed at 32.1 (`reference_close_direction_framework_gap` does not bite).

**Coverage boundaries / departures (phase 32.1):**
- **`op_timeout` parsed-not-consumed.** PGV-REQUIRED at boot; not wired to a dial/write deadline at 32.1.
- **4 gauges created-not-incremented (32.2).** `downstream_cx_active` / `downstream_cx_rx_bytes_buffered` / `downstream_cx_tx_bytes_buffered` / `downstream_rq_active` exist-at-zero from boot; their inc/dec paths are 32.2.
- **2 counters created-not-incremented (32.2).** `downstream_cx_drain_close` + `downstream_cx_protocol_error` exist-at-zero; increment wiring is 32.2.
- **Boot-window eager-vs-per-connection difference (unobservable).** envoy-go creates the 10 stats eagerly at config-load; upstream creates per-connection. UNOBSERVABLE to the differential (every `0055` assertion is post-connection — the D-P32-1 precedent).
- **Runtime-keys-at-defaults.** The filter behaves at the proto-configured values; runtime keys are at defaults (the §9 family boundary).
- **Close-direction-zero-touch (AMEND-R9).** No `cx_destroy_*` close-direction counters → framework ZERO-touch (the `reference_close_direction_framework_gap` does not bite).
- **The `redis.` LABEL-HOISTED Prometheus tag-extractor arm is 32.2.** The 32.1 differential uses the flat `GET /stats` internal-name scrape; `/stats/prometheus` omits `redis.*` until the arm lands.
- **Unknown-cluster graceful-close-no-`-ERR` (D-S32.1-6).** A runtime cluster-miss on `cm.Get(catchAllCluster)` → clean downstream TCP close, no `-ERR` reply.
- **One-conn-per-downstream pooling divergence (D-P32-9, forward-pointer 32.2).** The reference multiplexes all downstream conns onto one upstream conn per host; envoy-go creates one upstream conn per downstream conn. Unobservable in the single-connection 32.1 differential; pinned per-side at the 32.2 SPEC.
- **The HTTP-shaped `cluster.<name>.upstream_rq_2xx..5xx` have no RESP analog** — they stay at 0 per-side (not asserted in `0055`).
- **`prefix_routes.routes[]` longest-prefix multi-cluster routing is 32.2+.** Only `catch_all_route` is consumed at 32.1.
- **The deferred active features are PARSE-ACCEPTED-behavior-DEFERRED.** `prefix_routes` multi-cluster, hash-ring sharding + `enable_redirection`, multi-key fragmentation, `downstream_auth_*`, `faults`, request mirroring, replica `read_policy`, `custom_commands`, the full `ConnPoolSettings` surface — all parse-accepted at boot, not consumed at runtime.

**Phase 32.2 extension — the full command set, the per-command/splitter/cluster roster, the gauges' inc/dec, the `redis.` Prometheus arm, and the command matrix.**

- **The full single-route command set + the unknown/bad-arity dispatch (`commands.go`, `filter.go`).** A static 180-command supported-commands table (`supportedCommandList`/`supportedCommands`; lower-cased once at dispatch — the table member IS the `command.<cmd>.*` stat segment, so `IsValidName` holds by construction). `classify(cmd, args)` returns a `commandVerdict{action, reply, closeAfter, statCmd}` driving: an UNKNOWN command (not in the table) → `splitter.unsupported_command` + a byte-stable `-ERR unknown command…` reply; a bad-arity command → `splitter.invalid_request` + a byte-stable `-invalid request` reply; a valid command → proxied through the seam.

- **Per-command `command.<cmd>.{total,success,error}` + single-server semantics.** For each proxied command: `command.<cmd>.total` increments at dispatch; `command.<cmd>.success` on ANY decoded reply (including `-ERR`, `$-1`, `*-1` — a redis-server-level error is still a successfully round-tripped reply); `command.<cmd>.error` ONLY on a transport-or-pool failure (Send/decode error, conn closed before reply, unresolvable cluster). This is the single-server success-on-any-reply / error-on-transport-failure model (multi-key fragmentation is DEFERRED — MGET/MSET/DEL proxy single-server).

- **The extended local-reply set ECHO/TIME/QUIT/HELLO (`commands.go`).** ECHO echoes its arg (arity≠2 → `splitter.invalid_request`); QUIT writes `+OK\r\n` then closes (the `commandVerdict.closeAfter` signal); TIME returns a wall-clock 2-element array (NON-deterministic → UNIT-test-only, NOT a differential arm); HELLO is SPLIT — `>2` args → an options-error, a non-"2" protocol arg (incl. "3") → NOPROTO, BUT `HELLO 2`/no-arg is PROXIED (`command.hello.*` + forwarded). Because HELLO with no args is a valid proxied command, `commandsWithoutMandatoryArgs = {hello}` (HELLO must NOT be a bad-arity reject). PING/AUTH/ECHO/TIME/QUIT emit NO `command.*`; HELLO error-paths emit no `command.hello.*`. **The UNKNOWN-command error wording APPENDS the request args** (NOT an empty suffix — the reference is the source of truth; `unknownCommandError` joins `args[1:]` with `", "`); byte-exact confirmed cross-side for `BOGUSCMD x`: `-ERR unknown command 'BOGUSCMD', with args beginning with: x\r\n`.

- **The 2 lifecycle gauges' inc/dec + the `downstream_cx_protocol_error` wiring.** `downstream_cx_active` is inc'd at `Handle` entry and dec'd at exit (MIRRORED — the `0055` held-open arm asserts == 1 live on both sides); `downstream_rq_active` is per-request defer-balanced in `serveRequest` (QUIESCED — asserted == 0 at every measured point). `downstream_cx_protocol_error` increments on a non-EOF decode error (a clean EOF close leaves it at 0). The 2 `*_bytes_buffered` gauges remain a SUBJECT-SIDE coverage boundary (see below).

- **The EAGER 545 new stats.** Created at config-parse under `redis.<stat_prefix>.` via `NewCounterIfAbsent`: 180 × 3 = 540 per-command `command.<cmd>.{total,success,error}` + 2 `splitter.{unsupported_command,invalid_request}` + 3 `REDIS_CLUSTER_STATS`. Project stat surface **546 → 1091** (+545: 2 splitter + 3 cluster + 540 per-command; the 180 `command.<cmd>.latency` histograms + the `*_fault` counters are NOT counted per ADR-0060).

#### Prometheus exposition — the `redis.` tag-extractor arm

A `redis.`-prefixed internal name renders with a SINGLE-label hoist: `envoy_redis_<leaf>{envoy_redis_prefix="<stat_prefix>"}`. The `<stat_prefix>` segment is HOISTED into one `envoy_redis_prefix` label; the command name STAYS in the metric leaf (e.g. `redis.default.command.get.total` → `envoy_redis_command_get_total{envoy_redis_prefix="default"}`). Shape-detected via `strings.CutPrefix(internal, "redis.")` at `internal/stats/name.go`. Contrast: the kafka arm is INLINE (fully inlined `envoy_kafka_<sp>_<rest>{}`, EMPTY labels); the mongo arm is MULTI-label (multiple tag-extracted dimensions). The `redis.` arm is the mongo `.rbac.` SINGLE-label-hoist shape.

**Coverage boundaries / departures (phase 32.2):**
- **The 180 `command.<cmd>.latency` histograms are UNMIRRORED (ADR-0060).** The project-wide histogram deferral; only the 540 counters are created.
- **The `*_fault` counters are DEFERRED.** Faults are parse-accepted-behavior-deferred; their counter family is not created.
- **The 2 `*_bytes_buffered` gauges are a SUBJECT-SIDE coverage boundary.** The reference legitimately tracks them (`downstream_cx_rx_bytes_buffered == 14` — the held PING request frame parked unconsumed); the subject never wires them. Asserted SUBJECT == 0 only (non-vacuous — proves rendered + not spuriously incremented); the reference is not asserted.
- **The 3 REDIS_CLUSTER_STATS exist-at-0.** Created eagerly; the cluster-mode redirection paths that would increment them are deferred (single-route MVP).
- **TIME is unit-test-only** (wall-clock NON-deterministic — shape-only goldens, no differential arm).
- **HELLO 2/no-arg is PROXIED** (`command.hello.*` + forwarded); only the HELLO error-paths are local.
- **Multi-key fragmentation DEFERRED** (MGET/MSET/DEL proxied single-server).
- **One-conn-per-downstream per-side `upstream_cx_total` divergence (D-P32-9).** The `0055` command matrix exposes the documented seam divergence: the reference POOLS the upstream (`cluster.redis_cluster.upstream_cx_total == 1`); the subject lazy-dials one upstream conn per proxied-command downstream conn (`== 4`: SET/GET + GET-miss + INCR + DEL). PINNED per-side (ref=1/subj=4). The pooling-independent `upstream_rq_total == 5` stays cross-side EQUAL.
- **`op_timeout` parsed-not-consumed** (PGV-REQUIRED at boot; still not wired to a deadline at 32.2).
- **`enable_command_stats` is a no-op** (gates neither presence nor increment in v1.37.2 — the eager roster + increments are unconditional).
- **Close-direction-zero-touch (AMEND-R9).** Still no close-direction-keyed counters → framework ZERO-touch (the `reference_close_direction_framework_gap` does not bite).

### envoy.filters.network.thrift_proxy

*Introduced by phase 33 (`network-filter-thrift-proxy`) — the SEVENTH §9 Network-filters-family row (a flat top-level row per ADR-0106; after the phase-26 family-parent, the phase-27 `sni_cluster` row, the phase-28 `zookeeper_proxy` parent, the phase-29 `mongo_proxy` parent, the phase-31 `kafka_broker` flat row, and the phase-32 `redis_proxy` parent). The project's SECOND terminal routing proxy (after redis_proxy at phase 32) and the LAST §9 candidate — the §9 Network-filters family CLOSES after this phase. Justified by ADR-0231 (the `thrift_proxy` filter umbrella — ACCEPTED; §Decision/§Consequences body landed at this IMPL per ADR-0044). The FIRST row to REUSE a prior framework seam UNCHANGED (the ADR-0230 upstream connection-pool / cluster-routing seam — thrift is its anticipated SECOND consumer, validating the redis-scoped YAGNI sizing). Cross-side-PROVEN by the now-green `0057-thrift-roundtrip` (route-HIT round-trip + route-MISS local-exception + reply-EXCEPTION arms; downstream-Thrift-response byte-equivalence + `StatsAsserter`) + `0058-thrift-boot-reject` fixtures.*

`thrift_proxy` (`@type` `type.googleapis.com/envoy.extensions.filters.network.thrift_proxy.v3.ThriftProxy` — type URL DERIVED via `proto.MessageName` on `thrift_proxyv3.ThriftProxy`, NOT hand-typed; the `extensions.` segment is intrinsic per `reference_network_filter_typeurl_extensions`, D-T1) is the project's SECOND **terminal routing proxy**. It IS the connection terminator — implements `network.TerminalFilter` and owns the downstream `net.Conn` via `Handle(ctx, conn)`. There is NO `tcp_proxy` behind it (the redis_proxy precedent).

- **Single-pair single-route terminal envelope (SPEC §3; D-T2).** The MVP decodes ONE transport×protocol pair — **framed × binary** (pinned live: framed = 4-byte BE frame-len prefix; binary strict message-begin `0x8001 00 <msgtype>` + i32 name-len + name + i32 `seq_id`) — under `payload_passthrough` (route on the message-begin envelope, forward the struct body opaquely → `request_passthrough`/`response_passthrough` increment). AUTO_TRANSPORT/AUTO_PROTOCOL auto-detect framed-binary identically. The `Handle` pump reads message-begin frames from a `bufio.Reader` over the downstream conn (the streaming-reader model — the redis precedent).

- **`route_config` method-routing (`route.go`; SPEC §4).** A single exact-match (or match-all) method route → ONE upstream cluster. First-match wins; on a method routing MISS → a LOCAL `UnknownMethod` Thrift EXCEPTION (zero upstream — the redis-PING analogue, D-T5; `encodeUnknownMethod(method, seqID)` builds the byte-identical reference miss-path reply; the MVP keeps the connection open). `route_config` is NOT a hard PGV gate (absent → all methods miss → all local exceptions).

- **The REUSED ADR-0230 upstream round-trip (`filter.go`; Q-seam).** Each `Handle` builds a `network.NewUpstreamConn(dial, cl.IncUpstreamRqTotal)` over a dial closure (`Cluster.Dial`, keeping `internal/filter/network` free of an `internal/cluster` import); the synchronous single-flight pump reads one request frame → `up.Send` → reads one reply frame from `up.Reader()` → classifies (success/error first-field peek) → writes the reply downstream. ZERO new seam code; the seam is UNCHANGED. `seq_id` is parse-validated + passed-through (NOT demux-load-bearing — the reference remaps the upstream `seq_id` to 0 + restores the original downstream, so downstream byte-equivalence holds under pass-through; the upstream `seq_id` is a per-side boundary). A defensive cross-cluster re-dial branch (a second route resolving a different cluster mid-connection) is unit-tested live (the Task-8-followup test).

- **Config parse + PGV/departure reject arms (ADR-0080 byte-stable; SPEC §6; D-T4/AMEND-T7).** ONE hard PGV gate: `stat_prefix` min-1 → `thrift_proxy: stat_prefix is required` (+ the `stats.IsValidName(stat_prefix)` guard at the config boundary per `reference_dynamic_stat_name_charset_guard`). The un-chosen transport/protocol enum values are **envoy-go-strict DEPARTURE rejects** (the reference parse-ACCEPTS them — so these are unit-tested, NOT cross-side boot-reject arms): `thrift_proxy: unsupported transport (only AUTO_TRANSPORT and FRAMED are supported)` + `thrift_proxy: unsupported protocol (only AUTO_PROTOCOL and BINARY are supported)`. `0058-thrift-boot-reject` carries the `stat_prefix`-required arm (both sides reject at boot with a common substring in their real boot stderr; liveness proven by R6 break-liveness with `-count=1`).

- **The malformed-frame silent-close departure (D-S33-6).** A framing error (`errDecode`) counts `request_decoding_error` then SILENTLY CLOSES — NO local `ProtocolError` exception (a documented coverage-boundary departure). A distinct `errInvalidType` sentinel splits invalid-msgtype → `request_invalid_type`.

- **Stats: the EAGER 25-name roster under `thrift.<stat_prefix>.` (SPEC §7; D-T3).** 24 counters + 1 GAUGE `request_active` created EAGERLY at config-parse via `NewCounterIfAbsent`/`NewGaugeIfAbsent`. The 24 counters: `request`, `request_call`, `request_oneway`, `request_passthrough`, `request_decoding_error`, `request_invalid_type`, `request_internal_error`, `response`, `response_reply`, `response_success`, `response_error`, `response_exception`, `response_passthrough`, `response_decoding_error`, `response_invalid_type`, `route_missing`, `unknown_cluster`, `no_healthy_upstream`, `shadow_request_submit_failure`, `upstream_rq_maintenance_mode`, `cx_destroy_local_with_active_rq`, `cx_destroy_remote_with_active_rq`, `downstream_cx_max_requests`, `downstream_response_drain_close`. The gauge `request_active` is the project's THIRD differentially-mirrored gauge family (after mongo's `op_query_active` + redis's `downstream_cx_active`); QUIESCED-to-0 post-workload (D-S33-3 — no held-open arm). Normatively defined by `internal/filter/network/thriftproxy/stats.go::counterSuffixes`. Project stat surface **1091 → 1116** (+25). The `request_time_ms` histogram is DEFERRED (ADR-0060 — the registry has no Histogram type).

- **The 11th built-in + ZERO new go.mod dep (D-T1).** Registered in `internal/filter/network/builtins/builtins.go` (the two-dep factory — `ClusterManager` + `StatsRegistry`) + `main.go` parity; the `thrift_proxy/v3` blank-import in `internal/bootstrap/bootstrap.go` holds the TypeURL. thrift_proxy/v3 is a subpackage of the ALREADY-DIRECT `/envoy v1.32.4` module → `go mod tidy` adds NOTHING (the redis D32-1 posture — the OPPOSITE of kafka_broker's `/contrib` dep).

- **Differential: `0057-thrift-roundtrip` (cross-side) + `0058-thrift-boot-reject` (boot-reject).** `0057`: chain `[thrift_proxy]` as TERMINAL on BOTH sides; `TCPThriftResponder` BackendKind **33** (framed-binary canned REPLY echoing method + received `seq_id`, seq_id-agnostic; optional exception mode). Implements `fixture.StatsAsserter` scraping the flat `GET /stats` admin endpoint cross-side. Three arms: (a) route-HIT round-trip (Ping) — downstream byte-identical + `request`/`request_call`/`request_passthrough`/`response`/`response_reply`/`response_success`/`response_passthrough` +1 + `cluster.<name>.upstream_cx_total`/`upstream_rq_total` +1; (b) route-MISS (Pong) — the local `UnknownMethod` EXCEPTION bytes byte-identical cross-side + `route_missing`/`response_exception` +1; (c) reply-EXCEPTION (boom) — a BACKEND exception, `response_exception` +1 + a second upstream cx/rq. The downstream-Thrift-response byte-equivalence prong is load-bearing (the §9 SECOND after redis-32 — thrift_proxy GENERATES the downstream response). R4 deliberate-break liveness recorded with `-count=1`.

#### Prometheus exposition — the `thrift.` tag-extractor arm

A `thrift.`-prefixed internal name renders with a SINGLE-label hoist: `envoy_thrift_<leaf>{envoy_thrift_prefix="<stat_prefix>"}`. The `<stat_prefix>` segment is HOISTED into one `envoy_thrift_prefix` label; the remaining leaf STAYS in the metric name (e.g. `thrift.default.request_call` → `envoy_thrift_request_call{envoy_thrift_prefix="default"}`). Shape-detected via `strings.CutPrefix(internal, "thrift.")` at `internal/stats/name.go`. The redis-32 / mongo `.rbac.` SINGLE-label-hoist shape — NOT the kafka inline (empty-label) shape.

**Coverage boundaries / departures (phase 33):**
- **The `request_time_ms` histogram is UNMIRRORED (ADR-0060).** The project-wide histogram deferral; only the 24 counters + 1 gauge are created.
- **The 2 close-direction counters + `downstream_response_drain_close` are created-but-NEVER-incremented (the `reference_close_direction_framework_gap` boundary, AMEND-T6/D-T8).** `cx_destroy_local_with_active_rq` + `cx_destroy_remote_with_active_rq` (the framework records close TYPE not DIRECTION) + `downstream_response_drain_close` (the reply-completion drain close). SHARPER than redis-32: the reference DOES fire `cx_destroy_local_with_active_rq` + `downstream_response_drain_close` on the live `0057` routing-MISS arm (where it `FlushWrite`-closes after the local exception) → asserted PER-SIDE on the miss arm (the subject pins 0; the reference moves them). The framework-surgery sub-phase that would key close counters by direction stays deferred project-wide.
- **Per-side `request*` on a routing MISS (a documented behavioral divergence; PINNED per-side in `0057`).** The subject's pump increments `request`/`request_call`/`request_passthrough` at the TOP of the pump — BEFORE the route match — so a routing MISS counts them (count-before-match: Ping + Pong + boom → **subj=3** across the 3-arm workload). The C++ reference does NOT count `request*` on a miss (the miss is accounted ONLY via `route_missing`+`response_exception` per SPEC §7.3 / D-T5, live-confirmed: "request* do NOT move" on the miss → **ref=2**, HIT-only: Ping+boom). Asserted PER-SIDE in `0057` (ref=2/subj=3, NOT cross-equal) — a documented per-side divergence, NOT a subject bug.
- **The subject's cluster package does NOT emit `upstream_cx_rx_bytes_total` (SPEC §7.5).** It reuses ONLY `upstream_cx_total`/`upstream_rq_total` (the redis AMEND-R6 precedent). The decode-ran byte-witness is reference-side only (`cluster.<name>.upstream_cx_rx_bytes_total > 0`, ref=73 observed); the subject witness is `thrift.<sp>.request_call > 0` (both sides asserted). `upstream_cx_total` is PER-SIDE (ref=1 pooled — one upstream conn serves both proxied calls / subj=2 — one-conn-per-downstream, each HIT downstream dials 1 upstream; the redis D-P32-9 precedent, D-T9b); the pooling-independent `upstream_rq_total == 2` stays cross-side EQUAL.
- **The un-chosen transport×protocol pairs are envoy-go-strict DEPARTURE rejects (D-T4/AMEND-T7).** The reference parse-ACCEPTS the full 3×3 matrix; the MVP rejects every pair except framed×binary (+ AUTO) at config-parse → unit-tested, NOT cross-side boot-reject arms.
- **The non-router `thrift_filters` / full `route_config` richness / full struct-body parsing are PARSE-ACCEPTED-behavior-DEFERRED.** Only the implicit router sub-chain runs; `service_name`/header-matches/multi-cluster/mirror, `rate_limit`/`header_to_metadata`/`payload_to_metadata` thrift-filters, and full struct parsing (passthrough off) are parse-accepted, not consumed.
- **The upstream `seq_id` per-side difference.** The reference remaps the upstream `seq_id` to 0 + restores the original downstream; the MVP passes `seq_id` through. Downstream byte-equivalence holds; the upstream `seq_id` value is a per-side boundary (not asserted).
- **Malformed-frame silent-close (no local `ProtocolError`; D-S33-6).** A framing error counts `request_decoding_error` + closes silently — the reference would emit a local `ProtocolError` exception; the MVP does not (a coverage-boundary departure).
- **`seq_id`-keyed multiplexing / `max_requests_per_connection` / `access_log` DEFERRED.** The deferred multiplexing surface (seq_id-keyed demux / shared pool / deep queue / pipelining) stays DEFERRED for a future thrift latency/fan-out sub-phase.
- **Runtime-keys-at-defaults.** The filter behaves at the proto-configured values; runtime keys are at defaults (the §9 family boundary).

## Network filters — upstream connection-pool / cluster-routing seam (32.1)

*Introduced by phase 32.1 (`network-filter-redis-upstream-pool-seam-and-codec`) — the framework's SIXTH structural extension (after the ADR-0221 `network.WriteFilter` conn-wrap seam [28.1], the ADR-0219 upstream-cluster-override seam [27], the ADR-0226 async halt/resume seam [29.3], the ADR-0213 terminal-filter seam [26.2], and the listener/chain dispatch framework [26.1]). Justified by ADR-0230 (ACCEPTED — body LANDED at the 32.1 IMPL). The seam ADR — the analogue of ADR-0221 (the WriteFilter seam) / ADR-0226 (the async halt/resume seam). ADDITIVE: it activates ONLY for a `redis_proxy` terminal → all 56 pre-existing fixture dirs byte-identical (R1).*

The L4 filter framework gains the FIRST primitive for a terminal filter to ACTIVELY manage an upstream connection (dial + per-downstream lifecycle + ordered request↔reply round-trip), rather than OBSERVE a `tcp_proxy`-terminated one. This is the upstream-connection analogue of the ADR-0221 WriteFilter seam (which gave read-side observers a write-side callback) and the ADR-0226 halt/resume seam (which gave read-side observers an async control-flow primitive).

- **The exported surface (`internal/filter/network/upstreampool.go`).** THREE exported identifiers: `UpstreamDialFunc func(ctx context.Context) (net.Conn, error)` (the closure type — the consumer supplies a closure over `Cluster.Dial`, keeping `internal/filter/network` free of an `internal/cluster` import); `UpstreamConn struct` (one downstream connection's dedicated upstream connection: `dial UpstreamDialFunc` + `onRequest func()` hook + `conn net.Conn` + `r *bufio.Reader`); `NewUpstreamConn(dial UpstreamDialFunc, onRequest func()) *UpstreamConn` (binds the closure + the per-proxied-request hook; no dial at construction). THREE methods: `Send(ctx, reqBytes []byte) error` (lazy dial on first call; fires the `onRequest` hook → writes `reqBytes` upstream); `Reader() *bufio.Reader` (returns the stable buffered reader for reply-frame decode — valid after the first `Send`); `Close() error` (idempotent; no-op if never dialed).

- **One-conn-per-downstream multiplexing + synchronous single-flight FIFO (D-P32-3; AMEND-R8).** Each `Handle(ctx, downstream)` creates its own `*UpstreamConn` on the stack and `defer up.Close()`s it. The pump is synchronous: read one request frame → `up.Send` → read one reply frame from `up.Reader()` → write reply → loop. This is a DEGENERATE depth-1 FIFO — the FIFO/positional pop-front CONTRACT (R4) satisfied trivially. A pipelining client (multiple requests in one write before reading) is handled correctly: the requests sit buffered and are processed in order, replies returned in order. No concurrent in-flight upstream requests at 32.1.

- **Lazy dial + PING/AUTH-only connection never dials.** On the FIRST proxied (data) command, `Send` finds `u.conn == nil` and calls the closure → `Cluster.Dial(ctx)` → Incs `cluster.<name>.upstream_cx_total`/`upstream_cx_active` → stores the `net.Conn` + builds the `bufio.Reader`. A PING-only or AUTH-only connection never calls `Send` on a data command → `u.conn` stays `nil` → `Close()` is a no-op. The `*connWithGauge` returned by `Cluster.Dial` Decs `upstream_cx_active` on `Close()`.

- **Dial-closure decoupling (the `upstreamcluster.go` discipline preserved).** `internal/filter/network/` gains NO `internal/cluster` import — verified by `go list -deps ./internal/filter/network | grep internal/cluster` → empty (CLEAN). The `redisproxy` filter (which imports both, like `tcp_proxy`) resolves `catch_all_route.cluster` via `cm.Get(name)` and supplies the seam a `func(ctx) (net.Conn, error)` closure over `cl.Dial(ctx)` (endpoint discarded). The seam never sees a cluster name or a `*cluster.Cluster`.

- **The seam knows NOTHING about RESP framing.** `Reader()` returns a `bufio.Reader`; the codec (`decodeReply`) reads reply frames from it. The seam is a connection-lifecycle + ordered-round-trip primitive; it is reusable for any future terminal-proxy protocol (thrift — the last §9 candidate — reuses/extends it at its phase, the extract-at-second-consumer discipline).

- **Redis-scoped boundary (32.1).** NO thrift generalization (YAGNI at 32.1). The seam catches the `redisproxy` filter's routing decision (`catch_all` → `cluster.Manager.Get`) and the specific `onRequest` hook (`Cluster.IncUpstreamRqTotal`). Future consumers extend the seam at their phase; the ADR-0230 deferred boundary records what is intentionally NOT built.

- **No per-connection mutex at 32.1.** The single-flight synchronous pump is single-goroutine (each `Handle` on its own goroutine with its own `*UpstreamConn`); the ADR-0223 per-conn mutex is NOT needed at 32.1. It arrives only with the deferred two-goroutine pipelined model.

**ADDITIVE — existing-fixture byte-equivalence (R1).** The seam activates ONLY inside a `redis_proxy` terminal's `Handle`; it is a NEW file in `internal/filter/network/` with NO modification to any existing file in that package. All 56 pre-existing fixture dirs are byte-identical after the 32.1 IMPL (confirmed by the six-gate 58-dir full differential suite — 56 pre-existing dirs PASS byte-exact).

**Deferred (the consume-at-second-consumer boundary, §4.4):**
- **The shared per-host pool + cross-downstream multiplexing.** The reference's one-client-per-host model (one upstream conn multiplexed across all downstream conns, thread-local) → per-side `upstream_cx_*` divergence under concurrent downstream conns (D-P32-9; pinned per-side at the 32.2 SPEC).
- **The >1-depth pending queue + the two-goroutine read/write split.** A dedicated send goroutine + a receive goroutine with a bounded `[]pendingRequest` queue (the ADR-0223 per-conn mutex guard) — deferred to the pipelined model sub-phase.
- **`op_timeout` enforcement.** Parsed, not consumed at 32.1.

### Network filter chain framework — async halt/resume (29.3 amendment)

*Introduced by phase 29.3 (`network-filter-mongo-fault-delay-and-access-log`). Justified by ADR-0226 (the framework's FIFTH structural extension — the async halt/resume seam) CONSUMING the ADR-0221 §Consequences anticipated-halt-consumer allowance (REFINED to the READ side). NO operator-visible behavior change to any never-halting chain: every chain whose read filters all report `MayHalt()==false` (zookeeper, echo, direct_response, rbac_network, sni_cluster, mongo-without-`delay`) takes the byte-identical pre-29.3 path (R1, ratified by the full 53-dir back-compat suite + h2spec 53/53 + proxy-wasm 10/10 staying green). 29.3 is the framework-SURGERY sub-phase — the ONE consolidated ripple (ADR-0219) that also lands the close-direction recording + the drain-state accessor.*

The L4 chain framework gains a cross-goroutine async halt/resume capability, strictly opt-in behind a construction-time gate. It is the FIRST genuine cross-goroutine concurrency in the chain runtime (a `time.AfterFunc` timer goroutine resumes a chain the read-loop / pump goroutine parked).

- **The `MayHalt` haltable gate (the R1 fast-path).** A read filter may opt in by implementing `HaltingReadFilter` (`MayHalt() bool`). `newChainRuntime` checks it ONCE at construction: if ANY built read filter returns `MayHalt()==true`, the whole `chainRuntime` is `haltable`. When `!haltable`, `runData`/`replayRead`/`readChainConn.Read` take the byte-identical pre-29.3 path — the never-halting hot path pays NO synchronization cost (no lock, no cond, no hold). `mongo_proxy` reports `MayHalt()==true` ONLY when a `delay` is configured; every other network filter reports false.
- **The block-the-dispatcher hold (halt mutex + cond + `resumeReady`).** On a haltable chain, the halt/resume state (`held`/`resumeReady`/`replayIdx`/`replayHeld`) is guarded by a per-`chainRuntime` halt `sync.Mutex` (`haltMu`) + `sync.Cond` (`haltCond`) — the ADR-0223 minimal-critical-section discipline, guarding EXACTLY that state. When a halting filter returns a hold StopIteration, the dispatcher goroutine calls `holdUntilResume` and BLOCKS on `haltCond.Wait()`; the timer goroutine's `ContinueReading` → `releaseResume` broadcasts to unblock it, which then advances PAST the halting filter (upstream `std::next(filter->entry())` parity). Because the dispatcher BLOCKS, the async resume re-enters via the SAME goroutine — `serveNetworkChain`/`handleTerminal` are UNCHANGED (D-S29.3-2). **The `resumeReady` missed-wakeup guard:** a `ContinueReading` firing BETWEEN `OnData`'s return and `holdUntilResume` acquiring `haltMu` sets `resumeReady=true` (no holder to broadcast to); `holdUntilResume` checks `resumeReady` under `haltMu` before `Wait`ing and consumes it (no `Wait`), closing the timer-fires-before-the-holder-blocks race.
- **The post-handoff withhold-until-resume (the 28.1b §3.5 boundary lifted for HALT purposes ONLY).** With the production chain `[mongo_proxy, tcp_proxy]`, terminal handoff happens after the first successful `OnData` pass → in steady state ALL mongo decode runs post-handoff via `replayRead` on the terminal's downstream-pump goroutine, so the post-handoff path is the LOAD-BEARING one. On a haltable chain `replayRead` stops at the halting filter, parks `replayIdx`, and publishes the pinned park-state `replayHeld`; `readChainConn.Read` then holds (`holdUntilResume`) instead of returning the held bytes to the terminal's pump. On resume the parked replay finishes (`finishParkedReplayLocked`) and the bytes flow to the remaining filters + the pump. The three 28.1b post-handoff observational boundaries (StopIteration ignored / Close not acted on / ContinueReading meaningless) are UNCHANGED for never-halting filters — the boundary is lifted for HALT purposes only. **The `replayHeld` publication fence (the real production race the IMPL caught at Task 4):** `replayRead` originally published `rt.replayHeld = true` OUTSIDE `haltMu` while `ContinueReading` reads it UNDER `haltMu`; the halting filter arms its async resume DURING `OnData` (before that write lands), so the resume goroutine's guarded read raced the unguarded write → the fix publishes `replayHeld` UNDER `haltMu` (the fence). Proven NECESSARY by a `-count=1` deliberate-break `-race` DATA RACE (reverted clean; `TestSeamRaceReplayPublication` passes `-race -count=20`).
- **Never-halting byte-identical equivalence (R1).** The whole seam is gated behind the `haltable` flag; the never-halting population (every existing filter + mongo-without-delay) is byte-identical to the pre-29.3 framework. This is the R1 equivalence guarantee made concrete and is the back-compat / non-perturbation gate (the 53-dir suite + the `0052` no-delay arm + h2spec 53/53 + proxy-wasm 10/10).
- **The close-direction accessor (`CloseDirection()`).** A minimal accessor on `ReadFilterCallbacks`/`Connection` returns the post-handoff close direction (LOCAL / REMOTE / unset), backed by `chainRuntime.closeDir` (atomic first-wins CAS, recorded by `tcp_proxy`'s pump goroutines — whichever `io.Copy` EOF'd first; the downstream→upstream pump first = LOCAL, the upstream→downstream pump first = REMOTE). **The non-promoting forwarding trap (`reference_conn_wrap_method_no_promote`):** `prefixConn`/`writeChainConn` embed `net.Conn` as an INTERFACE so `readChainConn.SetCloseDirection` does NOT auto-promote — EXPLICIT forwarding methods on `prefixconn.go`+`writeconn.go` are MANDATORY (`TestCloseDirectionThroughWrapStack` is the through-the-real-wrap-stack guard).
- **The drain-state accessor (`Draining()`).** A minimal structural `DrainState` interface (`Draining() bool`) on `ReadFilterCallbacks`, backed by the phase-08.2 `*drain.Manager` (at the listener `rt.dm`) — NOT a `*drain.Manager` leak; threaded via `SetDrainState(rt.dm)` guarded `if rt.dm != nil` (a typed-nil `*drain.Manager` boxed in the interface is non-nil and would panic in `Draining()`). Folded into the SAME callbacks-extension ripple as `CloseDirection()` (ADR-0219 one ripple).

**New coverage boundaries (29.3):**
- **The access-log timing-bearing differential boundary (AMEND-B10).** The mongo access log's `time` field is per-message wall clock → cross-side log comparison is NOT viable → the proof is formatter unit goldens (timestamp asserted by shape) + this boundary, NO fixture dir.
- **The runtime-key-gating boundary (§2.6).** `mongo.drain_close_enabled` / `mongo.fault.*` / `mongo.logging_enabled` are unmirrored (the filter behaves at the proto-configured values); recorded at 29.3 (the natural home — fault/drain/logging all consult these keys upstream).
...
`echo`: 0 built-in stats. `direct_response`: 0 built-in stats. The framework adds 0 counters. The 26.2 `tcp_proxy`/HCM migration adds 0 counters (their existing stats are untouched). Project stat surface was **132** at 26.1 AND 26.2 phase-done; the phase-26.3 `rbac_network` 4-counter roster (`<stat_prefix>.rbac.{allowed,denied,shadow_allowed,shadow_denied}`) lands **132 → 136** (see the rbac_network stat-table extension block in `## Stat-name mapping`). Phase 26.3 also adds 2 fixture dirs (44 → 46) + 1 fuzzer (35 → 36). Phase 27 `sni_cluster` adds **0** counters (config-less; upstream parity) — stat surface stays **136** (+0); the `tcp_proxy` `downstream_cx_*` family stays unmirrored (a pre-existing gap; see the `tcp_proxy per-connection cluster resolution — 27 amendment` above). Phase 27 adds 1 fixture dir (46 → 47, `0045-sni-cluster`) + 0 fuzzers (36, DEFERRED — echo-parity config-less parse). Phase 28.1 `zookeeper_proxy` adds the **201**-counter eager roster (`<stat_prefix>.zookeeper.` + the dynamic `auth.<scheme>_rq` family) — stat surface **136 → 337** (the largest single-filter addition; the roll lands at 28.1b on the cross-side proof, not 28.1a; see the `### envoy.filters.network.zookeeper_proxy` subsection + the `## Stat-name mapping` 136 → 337 block). Phase 28.1 adds 2 fixture dirs (47 → 49: `0046-zookeeper-requests` re-enabled + `0047-zookeeper-boot-reject`) + 1 fuzzer (36 → 37, `FuzzZookeeperRequestDecode`). The ADR-0221 terminal-handoff conn-wrap seam (both directions) adds **0** stats of its own. Phase 28.2 adds **0** new counters (zero creation delta — all 201 `zookeeper_proxy` counters were created at 28.1; 28.2 wires the response-side increments only; stat surface stays **337**); adds 1 fixture dir (49 → 50: `0048-zookeeper-responses`) + 1 fuzzer (37 → 38: `FuzzZookeeperResponseDecode`). The THIRD §9 Network-filters-family row closes at 28.2; 4 family candidates remain (`redis`/`mongo`/`kafka_broker`/`thrift`); `mongo_proxy` is the natural next + anticipated consumer #2 of the ADR-0221 conn-wrap seam. Phase 29.1 `mongo_proxy` (request side) adds the **23**-stat eager roster (22 counters + the `op_query_active` gauge under `mongo.<stat_prefix>.` + the dynamic `cmd.*`/`collection.*`/callsite families) — stat surface **337 → 360** (the FOURTH §9 family row + the family's second stats-PRIMARY filter + consumer #2 of the ADR-0221 conn-wrap seam; see the `### envoy.filters.network.mongo_proxy` subsection + the `## Stat-name mapping` 337 → 360 block). Phase 29.1 adds 2 fixture dirs (50 → 52: `0049-mongo-requests` + `0050-mongo-boot-reject`) + 1 fuzzer (38 → 39, `FuzzMongoDecode`). Prometheus rendering is TAG-EXTRACTED (the four-rule `mongo.` arm at `internal/stats/name.go`; AMEND-B2 + AMEND-C1 — the inverse of the zookeeper FLAT finding). The response side (`op_reply*`/`op_command_reply` increments + the gauge inc/dec) lands at 29.2 (+0 creation); fault-delay + access-log + drain land at 29.3 (+0 creation). Phase 29.2 `mongo_proxy` (response side + correlation + the gauge) wires the response-side increments (the 5 `op_reply*`/`op_command_reply` counters) + the `op_query_active` gauge inc/dec/drain (the project's FIRST differentially-mirrored gauge) + the `emit_dynamic_metadata` Bucket emission — all created at 29.1, so **+0 creation** (stat surface stays **360**); adds 1 fixture dir (52 → 53: `0051-mongo-responses`) + 0 fuzzers (39, `FuzzMongoDecode` EXTENDED to both directions — D-P6, no 40th); the NEW `TCPMongoResponder` BackendKind **30**. Phase 29.3 `mongo_proxy` (fault-delay + access-log + drain + close-direction) wires the increments of the last four eager counters — `delays_injected` (fault-timer ARM) + `cx_drain_close` (reply-completion drain close) + `cx_destroy_local/remote_with_active_rq` (the D-P4 close-direction VALUE parity, D-P4 CLOSED) — all created at 29.1, so **+0 creation** (stat surface stays **360**); adds 1 fixture dir (53 → 54: `0052-mongo-fault-delay`, the `TCPMongoResponder` BackendKind 30 REUSED) + 0 fuzzers (39 unchanged — the async halt/resume seam concurrency is proven by `-race` tests, no 40th); the access log adds NO fixture dir (timing-bearing → unit goldens + a coverage boundary, AMEND-B10). The async halt/resume seam (the framework's FIFTH structural extension — see `### Network filter chain framework — async halt/resume (29.3 amendment)`) adds **0** stats of its own. The FOURTH §9 Network-filters-family row CLOSES at 29.3 (the parent-row-29 ROLLUP); after phase 29 **3 family candidates remained (`redis`/`kafka_broker`/`thrift`)**. Phase 31 `kafka_broker` adds the **176**-counter eager roster (86 `request.<root>_request` + 86 `response.<root>_response` + 4 fixed under `kafka.<stat_prefix>.`) — stat surface **360 → 536** (the FIFTH §9 family row + the family's third stats-PRIMARY filter + consumer #3 of the ADR-0221 conn-wrap seam + the project's FIRST `/contrib v1.32.4` consumer; the 86 response-duration histograms DEFERRED per ADR-0060, NOT counted; see the `### envoy.filters.network.kafka_broker` subsection + the `## Stat-name mapping` 360 → 536 block). Phase 31 adds 2 fixture dirs (54 → 56: `0053-kafka-requests` cross-side `StatsAsserter` + `0054-kafka-boot-reject`) + 1 fuzzer (39 → 40, `FuzzKafkaDecode` both directions) + the NEW `TCPKafkaResponder` BackendKind **31** (a correlation-id-echoing Kafka responder). Prometheus rendering is FULLY INLINED with EMPTY labels (the `kafka.` arm at `internal/stats/name.go`; AMEND-K2 — REFUTING the BRAINSTORM label-hoist hypothesis). kafka_broker is framework-ZERO-touch (no `cx_destroy_*` close-direction counters; ZERO-touch on the ADR-0226 async halt/resume seam — never-halting, byte-identical R1). The FIFTH §9 Network-filters-family row LANDS at 31 (a flat row, NO parent rollup); **2 family candidates remain (`redis`/`thrift`).** Phase 32.1 `redis_proxy` (sub-row 32.1 of the parent row 32 pre-split) adds the **10** fixed downstream stats (6 counters + 4 gauges under `redis.<stat_prefix>.`) + the framework's SIXTH structural extension (the upstream-pool seam, ADR-0230; see `## Network filters — upstream connection-pool / cluster-routing seam (32.1)`) — stat surface **536 → 546** (see the `## Stat-name mapping` 536 → 546 block). Phase 32.1 adds 2 fixture dirs (56 → 58: `0055-redis-roundtrip` cross-side [the §9 FIRST downstream-RESP byte-equivalence prong] + `0056-redis-boot-reject`) + fuzzers UNCHANGED at 40 (`FuzzRESPDecode` is 32.2) + the NEW `TCPRedisResponder` BackendKind **32** (FIFO/positional canned RESP). redis_proxy is close-direction-zero-touch (no `cx_destroy_*` counters; AMEND-R9) and ZERO-touch on the ADR-0226 seam (no `MayHalt()` → never-halting, byte-identical R1). Phase 32.2 `redis_proxy` (sub-row 32.2 — the consumer half) adds the **545** remaining stats (2 `splitter.*` + 3 REDIS_CLUSTER_STATS + 180×3=540 per-command `command.<cmd>.{total,success,error}` under `redis.<stat_prefix>.`) — stat surface **546 → 1091** (the 180 `command.<cmd>.latency` histograms + the `*_fault` counters DEFERRED per ADR-0060/faults, NOT counted; see the `## Stat-name mapping` 546 → 1091 block) — plus the 2 lifecycle gauges' inc/dec (`downstream_cx_active` MIRRORED + `downstream_rq_active` QUIESCED) + the `downstream_cx_protocol_error` increment + the `redis.` LABEL-HOISTED Prometheus arm at `internal/stats/name.go` (`envoy_redis_<leaf>{envoy_redis_prefix="<sp>"}` — the mongo `.rbac.` single-label-hoist shape). Phase 32.2 adds NO new fixture dir (`0055-redis-roundtrip` EXTENDED in place with the command matrix + the held-open gauge arm; fixtures stay **58**) + the 41st fuzzer **`FuzzRESPDecode`** (39 → 40 was kafka; 40 → 41 redis) + the `TCPRedisResponder` BackendKind **32** reply-table extended (NOT re-numbered). The `upstream_cx_total` per-side divergence is PINNED (ref=1 pooled / subj=4 one-conn-per-downstream, D-P32-9; `upstream_rq_total == 5` cross-side equal); the 2 `*_bytes_buffered` gauges are a subject-side coverage boundary. The SIXTH §9 Network-filters-family parent row CLOSES at 32.2 (the parent-row-32 ROLLUP — parent row 32 + sub-row 32.2 flip `in-progress → done` atomically per the 18/19/22/24/25/26/28/29 precedent); **1 §9 family candidate remains (`thrift`) after phase 32.** Phase 33 `thrift_proxy` (the project's SECOND terminal routing proxy after redis_proxy; the LAST §9 candidate) adds the **25** fixed stats (24 counters + 1 gauge `request_active` under `thrift.<stat_prefix>.`) — stat surface **1091 → 1116** (the `request_time_ms` histogram DEFERRED per ADR-0060, NOT counted; see the `## Stat-name mapping` 1091 → 1116 block) — REUSING the ADR-0230 upstream connection-pool / cluster-routing seam UNCHANGED (the FIRST reuse of a prior framework seam → framework-ZERO-touch, validating its redis-scoped YAGNI sizing). Phase 33 adds 2 fixture dirs (58 → 60: `0057-thrift-roundtrip` cross-side [the §9 SECOND downstream-response byte-equivalence prong] + `0058-thrift-boot-reject`) + the 42nd fuzzer `FuzzThriftDecode` (41 → 42) + the NEW `TCPThriftResponder` BackendKind **33** (seq_id-agnostic canned framed-binary REPLY). Prometheus rendering is SINGLE-label-HOISTED (the `thrift.` arm at `internal/stats/name.go` — `envoy_thrift_<leaf>{envoy_thrift_prefix="<sp>"}`, the redis-32 / mongo `.rbac.` shape, NOT the kafka inline shape). thrift_proxy's 2 close-direction counters (`cx_destroy_local/remote_with_active_rq`) + `downstream_response_drain_close` are created-but-NEVER-incremented (the `reference_close_direction_framework_gap` boundary, AMEND-T6) — SHARPER than redis-32, since the reference DOES fire them on the live `0057` routing-MISS arm (asserted PER-SIDE). The SEVENTH §9 Network-filters-family row LANDS at 33 (a flat row, NO parent rollup per ADR-0106) + **CLOSES the §9 Network-filters family — 0 candidates remain after phase 33.**

**Phase 36.1 extension — 1116 → 1119 internal names (the FIRST non-zero LB-stat delta):** phase 36.1 (`ring_hash`, the Load-balancing family's THIRD row) adds the **3** `ring_hash_lb.*` GAUGES under the EXISTING per-cluster `cluster.<name>.*` roster (NOT a new namespace; NOT a new filter scope): `cluster.<name>.ring_hash_lb.size`, `.min_hashes_per_host`, `.max_hashes_per_host`. This is the FIRST LB policy to move the stat surface (least_request-34 + random-35 were both ZERO-delta phases — live-confirmed empty `/stats` name-set diffs). Live-proven REFUTING the BRAINSTORM's anticipated zero delta: the `/stats` name-set diff RING_HASH-vs-ROUND_ROBIN is exactly these three names (458 vs 455 under 3 equal hosts); nothing else differs (the generic `lb_*` family is identical-and-present under both — NOT mirrored). The 3 gauges are MIRRORED (parity) because they describe the ring envoy-go DOES build (unlike the un-mirrored `lb_*` health/zone/subset machinery); they are registered + Set ONCE at ring build via `registerClusterMetrics` (which type-asserts `*ringHashLB` → registers ONLY on the RING_HASH path, reference parity; D-S36-6); they are STATIC (the ring is immutable) and CROSS-SIDE-EXACT — keyed only on `(minimum_ring_size, hash_function, host count, weights)`, NOT on endpoint ADDRESSES (1026 / 342 / 342 for a 3-equal-host default cluster on BOTH sides; the `0061` `StatsAsserter` cross-equal prong). The `ring_hash_lb.*` latency histograms stay DEFERRED (ADR-0060 — the registry has no Histogram type; these are GAUGES). NO new fuzzer (ring_hash decodes no untrusted wire bytes — the hash key derives from a source IP / parsed value; a ring-LOOKUP property test is unit-level — fuzzers STAY 42) + NO new BackendKind (`0061` reuses `TCPEcho` 0 — tail STAYS 33). Phase 36.1 total: **1116 → 1119 internal names** (1116 from pre-36.1 + **3 from phase 36.1**). NOTE: this is the BEHAVIOR_CONTRACT DOC count (the source-of-truth; there is NO programmatic golden asserting the number). The LB-stat surface stays a per-cluster `cluster.<name>.*` extension — the Load-balancing family contributes NO new filter-scoped namespace. (The 36.2 HTTP `hash_policy` plane adds **0** new stat names — it reuses these 3 gauges + the existing cluster roster.) See `### Load balancer — ring_hash (RING_HASH)`.

**Phase 37 extension — 1119 → 1121 internal names (the SECOND non-zero LB-stat delta):** phase 37 (`maglev`, the Load-balancing family's FOURTH row) adds the **2** `maglev_lb.*` GAUGES under the EXISTING per-cluster `cluster.<name>.*` roster (NOT a new namespace; NOT a new filter scope): `cluster.<name>.maglev_lb.min_entries_per_host`, `.max_entries_per_host`. This is the SECOND LB policy to move the stat surface (after ring_hash-36.1; least_request-34 + random-35 were both ZERO-delta phases). Live-proven: the `/stats` name-set diff MAGLEV-vs-ROUND_ROBIN is EXACTLY these two names (`downstream_cx_rx_bytes_total > 0` verified the decode ran); nothing else differs. There is NO `maglev_lb.size` gauge (the table size is config-known, unlike ring_hash's computed `ring_hash_lb.size` — confirmed absent live). The 2 gauges are MIRRORED (parity) because they describe the table envoy-go DOES build; they are registered + Set ONCE at table build via `registerClusterMetrics` (which type-asserts `*maglevLB` → registers ONLY on the MAGLEV path, reference parity); they are STATIC (the table is immutable) and CROSS-SIDE-EXACT — `min = floor(M/N)`, `max = ceil(M/N)`, keyed only on `(table_size, host count, weights)`, NOT on endpoint ADDRESSES (21845 / 21846 for a 3-equal-host default-65537 cluster on BOTH sides; the `0063` `StatsAsserter` cross-equal prong). The `maglev_lb.*` latency histograms stay DEFERRED project-wide (ADR-0060 — the registry has no Histogram type; these are GAUGES). NO new fuzzer (maglev decodes no untrusted wire bytes — the hash key derives from a parsed header value / source IP; a table-build/lookup property test is unit-level, FOLDED into `maglev_test.go` — fuzzers STAY 42) + NO new BackendKind (`0063` reuses the `0062` `HTTPEcho` — tail STAYS 33). Phase 37 total: **1119 → 1121 internal names** (1119 from pre-37 + **2 from phase 37**). NOTE: this is the BEHAVIOR_CONTRACT DOC count (the source-of-truth; there is NO programmatic golden asserting the number). The LB-stat surface stays a per-cluster `cluster.<name>.*` extension — the Load-balancing family contributes NO new filter-scoped namespace. ZERO new packages + ZERO new go.mod deps + ZERO new hash code (`xxHash64`/`xxHash64Seed` reused). See `### Load balancer — maglev (MAGLEV)`.

**Phase 38.1 extension — 1121 → 1125 internal names (the FIRST LB stats a `loadBalancer` Inc's from its own `Pick` path):** phase 38.1 (`subset`, the Load-balancing family's FIFTH construct and its FIRST WRAPPER) adds the **4** `lb_subsets_*` stats under the EXISTING per-cluster `cluster.<name>.*` roster (NOT a new namespace; NOT a new filter scope): `cluster.<name>.lb_subsets_active` (gauge), `.lb_subsets_created` / `.lb_subsets_selected` / `.lb_subsets_fallback` (counters). They are subset-only — registered ONLY when `lb_subset_config` is present (the `registerClusterMetrics` `*subsetLB` type-assert, reference parity). `active` (gauge) + `created` (counter) are Set / Add ONCE at build to the distinct-subset count — envoy-go pins the semantically-correct **×1** count, NOT the reference's **33×** magnitude artifact (a recorded DEPARTURE → these two are NOT cross-side-equal). `selected` / `fallback` are the FIRST stats a `loadBalancer` increments from its OWN `Pick` path (the maglev/ring_hash gauges are Set at build, not per-pick): `selected` +1 per subset hit, `fallback` +1 per fallback host-selection (a NO_FALLBACK no-host outcome does NOT increment `fallback`); both are request-counted and CROSS-SIDE-EQUAL — the `0064` `StatsAsserter` cross-equal prong. The deferred subset stats `lb_subsets_removed` / `fallback_panic` / `single_host_per_subset_duplicate` stay DEFERRED (no churn / panic deferred / single_host deferred). NO new fuzzer (subset decodes no untrusted wire bytes — the metadata derives from validated xDS config; a subset-resolution property test is unit-level, FOLDED into `subset_test.go` — fuzzers STAY 42) + NO new BackendKind (`0064` reuses the `0063` `HTTPEcho` — tail STAYS 33). Phase 38.1 total: **1121 → 1125 internal names** (1121 from pre-38.1 + **4 from phase 38.1**). NOTE: this is the BEHAVIOR_CONTRACT DOC count (the source-of-truth; there is NO programmatic golden asserting the number). The LB-stat surface stays a per-cluster `cluster.<name>.*` extension — the Load-balancing family contributes NO new filter-scoped namespace. ZERO new packages + ZERO new go.mod deps. See `### Load balancer — subset (lb_subset_config)`.

**Phase 38.2 — stat surface STAYS 1125 (ZERO new stat names; the FIRST LB-family leg to add NO stat surface):** phase 38.2 (`weighted_clusters` routing primitive, the pre-authorized SECOND leg of the phase-38 by-plane split) adds **ZERO** new stat names. The reference emits NO `*weighted*` stat anywhere (LIVE-confirmed D-WC4: a `/stats` scrape of a live 70/30 weighted-route bootstrap found zero `*weighted*` hits; `grep -i weight` matched ONLY `cluster.<name>.max_host_weight` — an ENDPOINT host-weight gauge present on EVERY cluster regardless of routing policy, value 1, UNRELATED to the route weight). The per-request cluster selection is observable purely through the PRE-EXISTING per-cluster `upstream_rq_total` / `upstream_rq_2xx` / `upstream_rq_completed` counters (already emitted by phase 06.1). The `max_host_weight` gauge is NOT a weighted-routing stat and is NOT asserted. **38.2 adds ZERO new stat names; surface STAYS 1125.** This is the FIRST Load-balancing-family leg to add NO stat surface (contrast: ring_hash-36.1 added 3 gauges; maglev-37 added 2 gauges; subset-38.1 added 4 counters+gauge). ZERO new fuzzer + ZERO new BackendKind + ZERO new packages + ZERO new go.mod deps. See `### Route action — weighted clusters (weighted_clusters)`.

**Phase 39.1 — 1125 → 1132 (+7) (the FIRST active-health-check stats + the FIRST cluster-level background-runtime stats):** phase 39.1 (active HTTP health checking, `internal/cluster`) adds **7** new stat names under the EXISTING per-cluster `cluster.<name>.*` roster (NOT a new namespace; NOT a new filter scope). They are emitted **ONLY on clusters with `health_checks` configured** — every existing fixture (no `health_checks`) is unaffected, the surface stays byte-stable on the pre-39.1 path. The 7 names: `cluster.<name>.health_check.attempt` (counter), `.health_check.success` (counter), `.health_check.failure` (counter), `.health_check.network_failure` (counter), `.health_check.healthy` (gauge — count of healthy hosts per checker), `.membership_healthy` (gauge), `.lb_healthy_panic` (counter). The four `health_check.*` counters + the `health_check.healthy`/`membership_healthy` gauges describe the ADR-0242 checker runtime (Inc'd per probe + transition); `lb_healthy_panic` describes the ADR-0243 health-aware pick (Inc'd when the healthy fraction is strictly `< healthy_panic_threshold`). The `0066-health-check-http` `StatsAsserter` cross-equals the deterministically-converging subset (`membership_healthy`, `health_check.attempt`/`success`/`failure`). DEFERRED (recorded departures, tied to deferred features): `health_check.{passive_failure[outlier], verify_cluster[service_name_matcher], degraded[degraded-state]}`, `membership_{change, degraded, excluded}`, `update_no_rebuild[EDS]`. Phase 39.1 total: **1125 → 1132 internal names** (1125 from pre-39.1 + **7 from phase 39.1**). NOTE: this is the BEHAVIOR_CONTRACT DOC count (the source-of-truth; there is NO programmatic golden asserting the number). ZERO new fuzzer (fuzzers STAY 42) + ZERO new BackendKind (`0066` reuses the HTTP backends — tail STAYS 33) + ZERO new packages + ZERO new go.mod deps. See `### Cluster — active health checks`.

**Phase 40.1 — 1132 → 1137 (+5) (the FIRST passive-health stats — the `outlier_detection.*` scoped sub-namespace):** phase 40.1 (passive outlier detection, `consecutive_5xx` keystone, `internal/cluster`) adds **5** new stat names under the EXISTING per-cluster `cluster.<name>.*` roster (NOT a new namespace; NOT a new filter scope — the new names carry the `outlier_detection.` segment). They are emitted **ONLY on clusters with `outlier_detection` configured** — every existing fixture (no `outlier_detection`) is unaffected; the surface stays byte-stable on the pre-40.1 path. The 5 names: `cluster.<name>.outlier_detection.ejections_active` (gauge — current ejected host count), `.outlier_detection.ejections_enforced_total` (counter — total enforced ejections across all detectors), `.outlier_detection.ejections_overflow` (counter — cap-blocked ejections), `.outlier_detection.ejections_detected_consecutive_5xx` (counter — threshold crossings), `.outlier_detection.ejections_enforced_consecutive_5xx` (counter — actual 5xx ejections). The double-count (`ejections_enforced_total` AND `ejections_enforced_consecutive_5xx` both ++ on one consecutive-5xx ejection) is REPLICATED per D-OD-STATS. `membership_healthy` STAYS active-HC-only (unchanged). The `0069-outlier-detection-consecutive-5xx` `StatsAsserter` cross-equals `{ejections_active, ejections_enforced_total, ejections_enforced_consecutive_5xx, ejections_detected_consecutive_5xx}` (NOT the gateway-detected counter — no gateway detector at 40.1). DEFERRED departures (NOT emitted at 40.1): the legacy `ejections_total`/`ejections_consecutive_5xx`/`ejections_success_rate`; the 14 other-variant `ejections_detected_*`/`ejections_enforced_*` (gateway/local-origin/failure-percentage/success-rate). Phase 40.1 total: **1132 → 1137 internal names** (1132 from pre-40.1 + **5 from phase 40.1**). NOTE: this is the BEHAVIOR_CONTRACT DOC count (the source-of-truth; there is NO programmatic golden asserting the number). ZERO new fuzzer (fuzzers STAY 42) + **+1 BackendKind** (`HTTP503Responder` = 35 — the always-503 in-process responder introduced by `0069`) + ZERO new packages + ZERO new go.mod deps. See `### Cluster — passive health (outlier detection)`.
...
- Phase 29.1/29.2/29.3 onward (`mongo_proxy` — the FOURTH §9 row, consumer #2 of the ADR-0221 conn-wrap seam: 29.1 request side + the eager 23-stat roster + the `mongo.` tag-extractor arm; 29.2 response side + correlation + the `op_query_active` gauge + `emit_dynamic_metadata`; **29.3 the async halt/resume seam** (the framework's FIFTH structural extension — the `MayHalt` haltable gate + the block-the-dispatcher halt mutex/cond + the post-handoff withhold-until-resume + the `replayHeld` publication fence; never-halting chains byte-identical R1) **+ fault-delay injection** (`delays_injected` at ARM; StopIteration-while-pending; eval at the five as-built request callbacks) **+ the mongo access log** (the `internal/accesslog` pluggable `Formatter func(any)` seam; differential-invisible) **+ `cx_drain_close`** (reply-completion drain close; the `DrainState` accessor) **+ the close-direction seam D-P4 CLOSED** (`CloseDirection()`; the `cx_destroy_*` VALUE parity; the non-promoting forwarding-method trap); ADR-0224/0225/0226. The FOURTH §9 Network-filters-family row closes; parent row 29 → done; 3 candidates remain — `redis`/`kafka_broker`/`thrift`).
...
- Other Network-family filters (`redis`, `kafka_broker`, `thrift`) — the §9 Network-filters family is OPEN; `mongo_proxy` closed at phase 29 (the FOURTH row; consumer #2 of the ADR-0221 conn-wrap seam), leaving **3 candidates** (`redis`/`kafka_broker`/`thrift`).
