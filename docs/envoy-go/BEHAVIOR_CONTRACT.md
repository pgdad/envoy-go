...
- **The goroutine-placement note + the 28.2 synchronization forward-pointer (§3.6).** Post-handoff, two pump goroutines run: goroutine A (downstream→upstream) calls `readChainConn.Read → replayRead →` the request decoder; goroutine B (upstream→downstream) calls `writeChainConn.Write →` the write filters' `OnWrite`. At 28.1b they share NO mutable state (B's `OnWrite` is a no-op; `writeChainConn.Write` allocates a fresh per-Write `Buffer`) — **the 28.1b race surface is EMPTY** (verified `go test -race` + a dedicated concurrent-pumps unit test over `net.Pipe`). **FORWARD-POINTER (28.2 / ADR-0223):** when 28.2's response decoder lands in `OnWrite`, goroutine B will READ + ERASE the correlation structures goroutine A WRITES — a data race under the as-built lock-free design. The 28.2 SPEC MUST add synchronization (anticipated: a per-connection `sync.Mutex` on the decoder guarding the correlation maps; the per-direction reassembly buffers stay unshared/lock-free). Upstream has no such race only because libevent serializes both directions onto one dispatcher thread.
...
### Does not yet apply to
- Response-side decode + latency counters (`zookeeper_proxy` `OnWrite`) — 28.2 / ADR-0223 (the response decoder consuming the R5 correlation structures + the `*_resp`/`*_resp_bytes`/`*_resp_fast`/`*_resp_slow` increments + the latency-threshold counters; the 28.2 SPEC MUST add the per-connection decoder mutex the ADR-0221 §3.6 forward-pointer flags).
...
- Other Network-family filters (redis, mongo, kafka_broker, thrift) — the §9 Network-filters family stays OPEN at 28.1b phase-done (`zookeeper_proxy` request side lands at 28.1; the response side + the parent-row-28 family rollup land at 28.2; `mongo_proxy` is the natural next + the anticipated consumer #2 of the ADR-0221 conn-wrap seam).
