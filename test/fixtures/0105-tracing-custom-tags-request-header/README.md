# 0105-tracing-custom-tags-request-header

Cross-side differential (subject envoy-go vs reference Envoy `contrib-v1.37.2` in
Docker) for **phase 62 request_header `custom_tags`** (ADR-0283). Cloned from
`0102-tracing-custom-tags-literal` (phase 59 literal custom_tags precedent,
itself cloned from `0087-tracing-otlp`, phase 46.1b span emission + OTLP
export); this fixture swaps the literal custom-tag source for the
**request_header** source — the HCM `tracing` block's `custom_tags` entry
resolves its value from an inbound request header (`x-trace-user`) rather than
a static config-time literal, and the resolved value is emitted as an OTLP span
...
`custom_tags` is a **sibling of `provider`** — a single request_header entry
with a NON-colliding key (`trace_user`), so both sides converge on a clean
pass. The driver sends `x-trace-user: u-42` on **every** driven request (the
present case), so every span resolves `trace_user == "u-42"`. The
`default_value: anon` / header-absent edges are exercised by the deterministic
`internal/tracing/resolve_test.go` unit tests, not this fixture. The
collision/override case (a `custom_tags` key matching a well-known span
attribute name) and the custom-vs-custom first-wins dedup case are exercised
by the phase 62 Zipkin/span encoder unit tests, not this fixture.
...
The request_header custom tag applies uniformly to every span regardless of
prong (plain or continuation) — the driver sets `x-trace-user: u-42` on every
`fireProbe` call (plain AND continuation requests alike), so the resolved
value is identical across both prongs.
...
## Request_header custom tag — asserted by KEY, every span, both sides (phase 62)

`assertCustomTag` iterates every span (both prongs, both sides) and asserts
`attrs["trace_user"] == "u-42"`, matched **by key** — OTLP attribute order is
non-deterministic (SPEC §11), so the assertion never depends on attribute
position. Unlike the other `t.Fatalf`-based per-span checks in this fixture,
`assertCustomTag` uses `t.Errorf` per property (continues past a single span's
failure) so one bad span does not mask assertion failures on the rest — useful
when debugging a partial-rollout regression across N+M spans.
...
- This fixture CLOSES ADR-0283 at IMPL (phase 62 — request_header `custom_tags`
  parse + per-request resolve + span-emit upsert), the SECOND custom_tag source
  type after phase 59's literal (`0102-tracing-custom-tags-literal`, ADR-0277),
  and the FIRST per-request-resolved custom tag. The tracing family traces back
  to phase 46/46.1a/46.1b (which closed ADR-0260, tracked by sibling fixture
  `0087-tracing-otlp`).
- Do NOT mutate `0087-tracing-otlp`, `0088-tracing-zipkin`, or
  `0102-tracing-custom-tags-literal` — this fixture is a full clone in its own
  directory, its own package, its own runner branch.
