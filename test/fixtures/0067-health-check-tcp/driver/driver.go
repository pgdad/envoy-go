// Package driver registers the 0067-health-check-tcp cross-side differential
// fixture (phase 39.2 SPEC §8.1 / PLAN Task 4).
//
// This is a CROSS-SIDE [http_connection_manager + router] fixture over ONE
// cluster c_hc (lb_policy ROUND_ROBIN) with active TCP health checking over
...
// (dead) upstream host is DETECTED by the connect-only TCP health checker and
// REMOVED from LB rotation on BOTH the envoy-go (subject) side and the
...
//
//   - backend0 → c_hc endpoint 0 (LIVE; HTTPEcho 200s every path; TCP connect succeeds)
//   - backend1 → c_hc endpoint 1 (LIVE; HTTPEcho 200s every path; TCP connect succeeds)
...
// # Cross-references
//
//   - phase 39.2 SPEC §8.1 / PLAN Task 4 (the fixture design).
//   - 0066-health-check-http (the HTTP HC shape this fixture is copied from; only
//     the healthChecksBlock is changed — tcp_health_check:{} vs http_health_check).
...
//   - reference_differential_run_selector (target -run 'TestDifferential/0067').
...
const (
	fixtureName = "0067-health-check-tcp"
	...
	// 0066 takes 19155, this takes 19156.
	refContainerListenerPort = 19156
...
// sides — NAT-transparent static config). One TCP checker (connect-only, empty
// body), fast convergence (interval/timeout 0.5s, thresholds 1/1).
...
func (d *hcDriver) ReferenceBootstrap(backendPorts []int) string {
	...
	// 2 live backends + the dead host, with the active TCP health checker.
...
func (d *hcDriver) SubjectConfig(_, subjListenerPort int, backendPorts []int, subjAdminPort int) string {
	...
	// + the dead host, with the active TCP health checker.
	return fmt.Sprintf(`
node: { id: envoy-go-subject-0067, cluster: envoy-go-differential }
admin:
  address:
...
func pollMembershipHealthy(side, adminAddr string) error {
	...
	for {
		...
		if time.Now().After(deadline) {
			return fmt.Errorf("%s: cluster.c_hc.membership_healthy did not converge to %d within %s (last seen %d) — dead host not detected? (live backends accept TCP connect? dead host actually refuses? subject checker started post-Freeze?)",
