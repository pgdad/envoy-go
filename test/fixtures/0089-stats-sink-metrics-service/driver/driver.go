// Package driver registers the 0089-stats-sink-metrics-service fixture with the
// differential runner. It is the behavioral proof of the phase 47.1 core
// metrics_service stats sink: cross-side EXACT (subject envoy-go vs reference
// Envoy contrib-v1.37.2 in Docker) on a deterministic COUNTER NAME SUBSET
// aggregated across the streamed MetricFamily flushes + the identifier node.
//
...
//     entry (transport_api_version V3) naming an h2c metrics cluster + a SHORT
//     stats_flush_interval (500ms) for fast deterministic convergence + a
//     node{id,cluster} fixed identically on both sides.
//
...
//     the deterministic COUNTER subset converges to == K AND the identifier node
//     has arrived — a release barrier (reference_concurrency_differential_release_barrier;
//     the proxy flushes the registry snapshot every stats_flush_interval; poll
//     not sleep). The per-side family snapshot (the three subset values + the
//     captured identifier node id/cluster) is captured from that side's own
//     receiver before the next side runs.
//
...
//     http.hcm_local.downstream_rq_2xx} present with type==COUNTER and value==K;
//     the identifier node.id/.cluster == the configured values. PLUS decode-ran
//     (Count() > 0) on each side before asserting.
//
// Decode-ran proof: the poll-to-converge guarantees the subset families arrived
// (value == K) on each side's own receiver before asserting — a zero-family pass
...
// non-deterministic; presence-only); help; non-deterministic gauges
// (server.uptime, *_active, connection churn); the identifier user_agent_*/
// extensions[]; message/stream framing + per-message family count.
package driver
...
const (
	fixtureName = "0089-stats-sink-metrics-service"

	// In-container reference Envoy ports. Convention "100NN" for fixture "00NN";
	// fixture 0089 takes 10089 for the single plaintext listener.
	refAdminPort    = 9901
	refListenerPort = 10089

	// numReq is the per-side request count K. After K 2xx requests the
	// deterministic COUNTER subset converges to value == K on each side.
	numReq = 7
	...
	wantNodeID      = "envoy-go-subject-0089"
	wantNodeCluster = "envoy-go-differential"

	// Converge-poll discipline (reference_concurrency_differential_release_barrier):
	// POLL the subset Family() values to == K; never sleep-to-wait.
	pollInterval = 200 * time.Millisecond
	pollDeadline = 30 * time.Second
)

// subsetNames is the deterministic COUNTER name subset asserted cross-side. All
// three are COUNTERs that reach exactly numReq after K 2xx requests on BOTH the
// reference and the subject (cross-side name-equality + post-K determinism).
...
// (value, type) readings + the captured identifier node id/cluster.
...
// COUNTER subset converges to == numReq AND the identifier node has arrived, and
// snapshots the subset readings + the captured identifier node. No Reset() is
// needed: each side owns a private receiver, so its accumulator is exclusively
// that side's data even though the other proxy keeps streaming into ITS receiver.
func (d *metricsSinkDriver) driveSide(ctx context.Context, addr string, srv *metricsservice.Server) ([]byte, sideSnapshot, error) {
	...
	for _, name := range subsetNames {
		v, typ, ok := srv.Family(name)
		snap.fams[name] = familyReading{value: v, typ: typ, ok: ok}
	}
	if n := srv.Node(); n != nil {
		snap.nodeID = n.GetId()
...
// received with value == numReq AND the identifier node has arrived (or the
// context / deadline elapses). The proxy flushes the full registry snapshot every
// stats_flush_interval (500ms), so a fixed sleep would be both flaky and slow;
// the poll converges as soon as the first post-K flush arrives. This is the
...
func pollSubset(ctx context.Context, srv *metricsservice.Server) error {
	...
	for {
		...
		if time.Now().After(deadline) {
			return fmt.Errorf("metrics receiver: timed out waiting for COUNTER subset == %d + identifier node (%s, node=%v)",
				numReq, describeSubset(srv), srv.Node() != nil)
		}
		select {
		case <-ctx.Done():
			return fmt.Errorf("metrics receiver: context done waiting for COUNTER subset == %d (%s): %w",
...
// subsetConverged is true iff every subset name has been received with value
// exactly numReq.
func subsetConverged(srv *metricsservice.Server) bool {
	for _, name := range subsetNames {
		v, _, ok := srv.Family(name)
...
// describeSubset renders the current subset readings for a timeout diagnostic.
...
// present with type==COUNTER and value==numReq; the identifier node.id/.cluster
// == the configured values. The poll-to-converge already proved the subset
// arrived at == numReq AND the identifier node arrived on each side's private
// receiver (decode ran — a zero-family pass is structurally impossible), but the
// assertion re-checks the captured snapshot.
func (d *metricsSinkDriver) AssertStats(t fixture.TB, refAdminAddr, subjAdminAddr string) {
	t.Helper()

	if os.Getenv("FIXTURE_0089_DUMP") != "" {
		fmt.Fprintf(os.Stderr, "=== 0089 ref node{id=%q cluster=%q} subj node{id=%q cluster=%q} ===\n",
			d.ref.nodeID, d.ref.nodeCluster, d.subj.nodeID, d.subj.nodeCluster)
		for _, name := range subsetNames {
			rf := d.ref.fams[name]
			sf := d.subj.fams[name]
			fmt.Fprintf(os.Stderr, "  %s: ref{v=%v typ=%v ok=%v} subj{v=%v typ=%v ok=%v}\n",
				name, rf.value, rf.typ, rf.ok, sf.value, sf.typ, sf.ok)
		}
	}
...
// value==numReq) and the identifier node id/cluster for one side's snapshot.
func assertSide(t fixture.TB, side string, snap sideSnapshot) {
	...
	for _, name := range subsetNames {
		...
		if fr.value != float64(numReq) {
			t.Fatalf("%s: family %q value = %v, want %d (== K)", side, name, fr.value, numReq)
		}
	}
...
func fixtureDir() string {
	...
	// thisFile is .../test/fixtures/0089-stats-sink-metrics-service/driver/driver.go
