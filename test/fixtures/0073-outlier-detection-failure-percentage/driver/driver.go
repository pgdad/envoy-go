// Package driver registers the 0073-outlier-detection-failure-percentage
// cross-side differential fixture (phase 40.3 SPEC / PLAN Task 9).
//
// This is a CROSS-SIDE [http_connection_manager + router] fixture over ONE
// cluster c_fp (lb_policy ROUND_ROBIN) with STATISTICAL (failure-percentage)
// outlier detection over SIX endpoints — 5 HEALTHY HTTPEcho backends + 1
// always-503 host (the HTTP503Responder, BackendKind 35). It proves that an
// upstream host whose failure percentage crosses the configured threshold (the
// bad host's 100% failure vs the failure_percentage_threshold of 85%) is
// DETECTED and EJECTED via the FAILURE_PERCENTAGE detector on BOTH the envoy-go
// (subject) side and the reference-Envoy side.
//
// # The symmetric twin of 0072 (success_rate enforces) — here FP enforces
//
// 0073 mirrors 0072's topology (K=5 healthy + 1 always-503) but FLIPS the
// enforcer: failure_percentage is the SOLE enforcer (enforcing_failure_percentage
// 100), success_rate is DETECT-ONLY (enforcing_success_rate 0). The load-bearing
// isolation assertion is ejections_enforced_success_rate == 0 (SR does NOT
// enforce) + ejections_enforced_failure_percentage >= 1 (FP enforces).
//
// # Topology: 5 HEALTHY + 1 ALWAYS-503 (all runner-spawned)
//
//   - backend0..4 → c_fp endpoints 0..4 (HTTPEcho; 200s every path; serve load)
//   - backend5    → c_fp endpoint 5    (HTTP503Responder; always 503; ejected)
//
// All six hosts are runner-spawned live listeners; the runner selects host5's
// kind via PerHostBackendKind (BackendKindAt(5) → HTTP503Responder).
// BackendCount() is 6. The bad host's 100% failure ≥ the 85% threshold (and ≫ the
// 10-request volume floor within one interval), so the failure_percentage
// detector ejects it.
//
// # Cluster shape (both sides)
//
//		c_fp: lb_policy ROUND_ROBIN, 6 endpoints, outlier_detection: {
...
// statistical failure_percentage detector ALONE. The success_rate detector is
// armed (minimum_hosts/request_volume match the FP detector) but DETECT-ONLY
// (enforcing_success_rate: 0) — the bad host's 0% success rate MAY trip its
// detected counter on both sides without ejecting via that path.
//
// # The driver: ejection-drive + poll-to-converge + warmup (the 0072 template)
//
//	AssertStats:
...
//	    ~ejectDriveRequests/6 ≫ reqVolume failures; at the next sweep its failure
//	    percentage is 100% ≥ the 85% threshold so the failure_percentage detector
//	    ejects it.
//	 2. Poll /stats on BOTH sides until
//	    cluster.c_fp.outlier_detection.ejections_active == 1 (deadline, poll 200ms;
...
//	    ejections_enforced_total == 1, ejections_detected_failure_percentage >= 1,
//	    ejections_enforced_failure_percentage >= 1, AND
//	    ejections_enforced_success_rate == 0 (detect-only at enforcing 0 — a LIVE
//	    == 0 assertion isolating the failure_percentage enforcer), AND
...
//	    arm is DEFERRED. ejections_detected_success_rate is LEFT UNASSERTED — it is
//	    detect-only and not the detector under test; whether the bad host's 0% rate
//	    crosses the SR threshold depends on the cross-host distribution and need not
//	    match cross-side.
//
// # Cross-references
//
//   - phase 40.3 SPEC / PLAN Task 9 (the fixture design).
//   - 0072-outlier-detection-success-rate (the immediate template: same K=5+1
//     topology + PerHostBackendKind + HTTP503Responder; this is the symmetric twin
//     with the enforcer flipped from success_rate to failure_percentage).
//   - 0069-outlier-detection-consecutive-5xx (the cross-side outlier StatsAsserter
//     template: PerHostBackendKind mixed cluster with the always-503 host).
...
//   - reference_differential_run_selector (target -run 'TestDifferential/0073').
...
const (
	fixtureName = "0073-outlier-detection-failure-percentage"

	// In-container reference Envoy listener port for l_http. Fixtures run
	// sequentially; a distinct value avoids confusion (0072 took 19161, this takes
	// the next-free 19162).
	refContainerListenerPort = 19162
	...
	// the stat assertions read these). failure_percentage is the SOLE enforcer;
	// success_rate is detect-only.
	fpThreshold        = 85               // failure_percentage_threshold (the bad host's 100% ≥ 85)
	fpMinHosts         = 2                // failure_percentage_minimum_hosts (<= 6 eligible)
	srMinHosts         = 2                // success_rate_minimum_hosts (for the detect-only SR detector)
	reqVolume          = 10               // BOTH failure_percentage_request_volume AND success_rate_request_volume (single-sourced volume floor)
	...
	// sweep with a 100% failure percentage.
	...
	statEjectionsActive    = "cluster.c_fp.outlier_detection.ejections_active"
	statEjectEnforcedTotal = "cluster.c_fp.outlier_detection.ejections_enforced_total"
	statEjectDetectedFP    = "cluster.c_fp.outlier_detection.ejections_detected_failure_percentage"
	statEjectEnforcedFP    = "cluster.c_fp.outlier_detection.ejections_enforced_failure_percentage"
	statEjectEnforcedSR    = "cluster.c_fp.outlier_detection.ejections_enforced_success_rate"
	statEjectDetected5xx   = "cluster.c_fp.outlier_detection.ejections_detected_consecutive_5xx"
	statUpstreamRqTotal    = "cluster.c_fp.upstream_rq_total"
	statUpstreamRq2xx      = "cluster.c_fp.upstream_rq_2xx"
	statUpstreamRq5xx      = "cluster.c_fp.upstream_rq_5xx"
)
...
// HTTPEcho, host 5 is the always-503 responder the failure_percentage detector
// ejects.
...
// failure_percentage alone. success_rate is detect-only (enforcing 0).
var outlierBlock = fmt.Sprintf(`      outlier_detection:
        failure_percentage_threshold: %d
...
// routeTable routes / to c_fp (the data path). Identical on both sides.
...
func (d *odDriver) ReferenceBootstrap(backendPorts []int) string {
	...
	// STRICT_DNS + host.docker.internal (the 0069 reference shape). c_fp over the
...
func (d *odDriver) SubjectConfig(_, subjListenerPort int, backendPorts []int, subjAdminPort int) string {
	...
	d.mu.Unlock()
	// STATIC + 127.0.0.1 (the 0069 subject shape). c_fp over the 5 healthy
	// backends + the always-503 host, with statistical outlier detection.
	return fmt.Sprintf(`
node: { id: envoy-go-subject-0073, cluster: envoy-go-differential }
admin:
  address:
...
func pollEjectionsActive(side, adminAddr string) error {
	...
	for {
		...
		if time.Now().After(deadline) {
			return fmt.Errorf("%s: %s did not converge to 1 within %s (last seen %d) — 503 host not ejected? (failure_percentage window accruing >= reqVolume? the bad host's 100%% failure >= the %d%% threshold? the sweep firing on the interval? RecordUpstreamResult feeding the windowed counters?)",
				side, statEjectionsActive, convergeDeadline, last, fpThreshold)
		}
...
func (d *odDriver) AssertStats(t fixture.TB, refAdminAddr, subjAdminAddr string) {
	...
	// 1. Ejection drive: 503-tolerant load to accrue a 100% failure percentage on
	// host5 within one interval (≫ reqVolume picks of the bad host).
	...
	for _, sd := range []struct {
		...
		// Outlier-detection parity: host5 ejected and held via the FAILURE_
		// PERCENTAGE detector. ejections_active and ejections_enforced_total are
		// EXACT 1 (the single bad host ejected once; recovery deferred).
		...
		assertAtLeast(t, sd.side, sd.st, statEjectEnforcedFP, 1)

		// The success_rate detector is ARMED (minimum_hosts/request_volume match
		// the FP detector) but DETECT-ONLY (enforcing_success_rate: 0). It does NOT
		// eject via this path, so ejections_enforced_success_rate MUST be exactly 0
		// — the LOAD-BEARING isolation assertion that the failure_percentage detector
		// (not success_rate) is the enforcer (a LIVE == 0; the assertEq absent-as-0
		// accommodation swallows ABSENT, not present > 0 — so a regression that
		// enforced via the SR path would bite). ejections_detected_success_rate is
		// LEFT UNASSERTED: it is detect-only and not the detector under test, and
		// whether the bad host's 0% rate crosses the SR threshold depends on the
		// cross-host distribution and need not match cross-side.
		assertEq(t, sd.side, sd.st, statEjectEnforcedSR, 0)
...
func assertEq(t fixture.TB, side string, st map[string]uint64, key string, want uint64) {
	...
	if !ok {
		...
		// enforced_success_rate or detected_consecutive_5xx present > 0) bites.
		if want == 0 {
			return
		}
