...
func TestBackendIdxFromBody_Bad(t *testing.T) {
	for _, body := range []string{
		...
		"released",    // the /__release control-response body, not a host body
	} {
...
func TestConstants(t *testing.T) {
	...
	if maxRequests != 4 {
		t.Errorf("maxRequests=%d, want 4", maxRequests)
	}
	if clusterName != "c_cb" {
		t.Errorf("clusterName=%q, want c_cb", clusterName)
	}
	if refContainerListenerPort != 19163 {
		t.Errorf("refContainerListenerPort=%d, want 19163 (next-free)", refContainerListenerPort)
	}
}
