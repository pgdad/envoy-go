// Package driver registers the 0062-lb-ring-hash-http cross-side differential
// fixture (phase 36.2 SPEC §10 / 36.2 PLAN Task 5).
//
// This is a CROSS-SIDE [http_connection_manager + router] fixture over ONE
// 3-endpoint cluster with lb_policy: RING_HASH + ring_hash_lb_config: {}
// (defaults) + a ROUTE-LEVEL hash_policy: [{header: {header_name: "x-hash"}}]
// on BOTH sides (the 0003 HTTP shape: reference STRICT_DNS / host.docker.internal,
// subject STATIC / 127.0.0.1). It is the end-to-end proof of Tasks 2-4 (the HTTP
// route hash_policy producer → cluster.HashHeaderValues digest → applyHashKey →
// cluster.WithHashKey → ringHashLB pick → per-header-value affinity).
//
// It is the HTTP sibling of 0061-lb-ring-hash (the 36.1 tcp source_ip plane). The
// crucial difference: 0061's source_ip key is REWRITTEN by Docker NAT, so the
// reference collapsed to a single key (cross-side host identity infeasible —
// subject-side affinity only). 0062's X-Hash HEADER is NAT-TRANSPARENT — it
// survives the Docker hop verbatim — so the consistent-hash affinity invariant
// holds on BOTH the envoy-go subject AND the reference Envoy. This is a TRUE
// cross-side affinity proof (D-S362-6).
//
...
//     ref and subj — whose rings are built over DIFFERENT endpoint address strings
...
//     (D-S362-6): every per-backend count ≡ 0 mod K=16 (affinity — one header value
//     → one digest → one ring point → one backend, so each value contributes all 16
//     or 0 to a given backend) AND >= 2 backends nonzero (spread). UNLIKE 0061, this
//     holds on BOTH sides because the X-Hash header is NAT-transparent;
//   - a cross-side StatsAsserter prong: cross-equal upstream_cx_total==totalReqs /
//     membership_total==3 / upstream_cx_active==0 + the 3 ring_hash_lb.* gauges
//     (1026/342/342 — address-independent → cross-equal) + the NEW cross-equal
//     upstream_rq_total==totalReqs prong (the HTTP plane DOES Inc upstream_rq_total
//     — the contrast with 0061's tcp_proxy per-side rq boundary). SPEC §7.
//
// NO new BackendKind (reuses HTTPEcho — the 0003 backend; the tail STAYS 33). NO
// boot-reject dir (the route hash_policy reject arms land UNIT-LEVEL in
// config_test.go). NOT asserted: cross-side host IDENTITY (the two rings are built
...
//     stats set incl upstream_rq_total) + D-S362-6 (the NAT-transparent both-side
//     affinity invariant).
//   - 0061-lb-ring-hash (the tcp source_ip sibling — SAME ring, subject-side
//     affinity only; this is the NAT-transparent HTTP transposition).
...
//   - reference_differential_run_selector (target -run 'TestDifferential/0062-lb-ring-hash-http').
//   - reference_differential_break_protocol_count1 (the Task-6 liveness proofs run
...
const (
	fixtureName = "0062-lb-ring-hash-http"
	...
	// avoids confusion — 0061-lb-ring-hash takes 19150, this takes 19151.
	refContainerListenerPort = 19151
...
func init() {
	fixture.RegisterFixture(fixtureName, &ringHashHTTPDriver{})
}

type ringHashHTTPDriver struct{}

func (ringHashHTTPDriver) BackendCount() int                { return 3 }
func (ringHashHTTPDriver) BackendKind() fixture.BackendKind { return fixture.HTTPEcho }
func (ringHashHTTPDriver) SubjectListenerName() string      { return "l_http" }
func (ringHashHTTPDriver) ReferenceListenerPort() int       { return refContainerListenerPort }

func (ringHashHTTPDriver) ReferenceBootstrap(backendPorts []int) string {
	if len(backendPorts) != 3 {
		panic(fmt.Sprintf("%s: expected 3 backend ports, got %d", fixtureName, len(backendPorts)))
	}
	// STRICT_DNS + host.docker.internal (the 0003 reference shape) + RING_HASH
	// (ring_hash_lb_config: {} — defaults 1024/8388608/XX_HASH) + the route-level
	// header hash_policy.
...
func (ringHashHTTPDriver) SubjectConfig(_, subjListenerPort int, backendPorts []int, subjAdminPort int) string {
	if len(backendPorts) != 3 {
		panic(fmt.Sprintf("%s: expected 3 backend ports, got %d", fixtureName, len(backendPorts)))
	}
	// STATIC + 127.0.0.1 (the 0003 subject shape) + RING_HASH (ring_hash_lb_config:
	// {} — defaults) + the route-level header hash_policy.
	return fmt.Sprintf(`
node: { id: envoy-go-subject-0062, cluster: envoy-go-differential }
admin:
  address:
...
// rings — built over different endpoint address strings — may land a value on a
...
func (ringHashHTTPDriver) DriveReference(ctx context.Context, addr string) ([]byte, error) {
	return drive(ctx, addr)
}

func (ringHashHTTPDriver) DriveSubject(ctx context.Context, addr string) ([]byte, error) {
	return drive(ctx, addr)
}

// ProbeAdmin issues GET /ready against each proxy's admin endpoint (the 0003
// raw-socket /ready probe, verbatim).
func (ringHashHTTPDriver) ProbeAdmin(ctx context.Context, refAdminAddr, subjAdminAddr string) (refBytes, subjBytes []byte, err error) {
...
// one ring point → one backend, so each value contributes all 16 or 0 to a given
...
// not one). UNLIKE 0061, the invariant holds on BOTH sides because the X-Hash
// header is NAT-transparent (it survives the Docker hop verbatim — D-S362-6). A
// scattered key (a value splitting across backends) breaks the multiple-of-16
// invariant; a collapsed ring breaks spread. Cross-side host IDENTITY is NOT
// asserted (the two rings are over different endpoint address strings —
// reference_differential_hash_key_cross_side_infeasible).
func (ringHashHTTPDriver) AssertDistribution(refCounts, subjCounts []uint64) error {
	for _, sd := range []struct {
		...
		if nonzero < 2 {
			return fmt.Errorf("%s spread: only %d backend(s) nonzero, want >= 2 (ring collapsed?)", sd.side, nonzero)
		}
	}
	return nil
}
...
// Connection: close on both sides) + the THREE ring_hash_lb gauges (size==1026 /
// min_hashes_per_host==342 / max_hashes_per_host==342 — they depend ONLY on
// ring-config + host COUNT, NOT addresses, so they ARE cross-equal) + upstream_rq
// _total==totalReqs (cross-equal — the HTTP plane Inc's upstream_rq_total on BOTH
// sides, the NEW prong vs 0061's tcp_proxy per-side rq boundary). The cx/rq wants
// are the DERIVED totalReqs constant (N*K), never a hardcoded literal.
func (ringHashHTTPDriver) AssertStats(t fixture.TB, refAdminAddr, subjAdminAddr string) {
	t.Helper()
	...
	// post-drive; the 3 ring_hash_lb gauges are address-independent → identical).
...
// "name: value" lines into a map[name]uint64. (The 0061 driver scrapeStats,
// verbatim.)
...
var (
	_ fixture.Driver               = (*ringHashHTTPDriver)(nil)
	_ fixture.DistributionAsserter = (*ringHashHTTPDriver)(nil)
	_ fixture.StatsAsserter        = (*ringHashHTTPDriver)(nil)
	_ fixture.BackendKindAware     = (*ringHashHTTPDriver)(nil)
)
