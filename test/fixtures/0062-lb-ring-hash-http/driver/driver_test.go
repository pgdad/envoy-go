...
// invariant) passes on BOTH sides. UNLIKE 0061, the reference is ALSO held to the
// affinity invariant (the X-Hash header is NAT-transparent — D-S362-6).
func TestAssertDistribution_BothSideAffinity(t *testing.T) {
	d := ringHashHTTPDriver{}
...
// affinity leg. This is the NEW both-side prong vs 0061 (where the reference was
// conservation-only). The subject side is a valid sum-256 affine distribution, so
// the asserted break is the reference's scatter, not a conservation miss.
...
// backend (a collapsed ring) conserves AND is a multiple of 16, but only ONE backend
