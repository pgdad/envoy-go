// Package driver registers the 0068-health-check-grpc cross-side differential
// fixture (phase 39.2 SPEC §8.2 / PLAN Task 9) — the KEYSTONE gRPC active-HC
// fixture.
//
// This is a CROSS-SIDE [http_connection_manager + router] fixture over ONE
// cluster c_hc (lb_policy ROUND_ROBIN) with active gRPC health checking
// (grpc_health_check{} → grpc.health.v1.Health/Check over H2) across THREE
...
// probe's transport setup fails). It proves that an unhealthy (dead) upstream
// host is DETECTED by the gRPC health checker and REMOVED from LB rotation on
// BOTH the envoy-go (subject) side and the reference-Envoy side.
//
// # gRPC HC REQUIRES an H2 upstream cluster (the +1 BackendKind, the H2 data plane)
//
// The gRPC checker speaks grpc.health.v1 over HTTP/2, so the cluster MUST be H2
// (Task 7's buildCluster reject: a gRPC HC on a non-H2 cluster is rejected). The
// cluster is therefore made H2 on BOTH sides via typed_extension_protocol_options
// → HttpProtocolOptions.explicit_http_config.http2_protocol_options (the 0021
// gRPC-upstream shape). The proxy then speaks cleartext-H2/h2c to the
// GRPCHealthResponder backend for BOTH the HC probe AND the forwarded
// data-plane request.
//
// THE DOWNSTREAM LISTENER STAYS codec_type: HTTP1 (unchanged from 0067): the
// driver sends HTTP/1.1 GET / via helpers.HTTPRoundTrip (the same client as
// 0066/0067); the proxy translates H1-downstream → H2-upstream. So NO H2 client
// helper is needed — loadAndTally/warmupUntilStable are byte-identical to 0067.
// The GRPCHealthResponder h2c handler writes "backend-<idx>:<path>" for the
// non-gRPC data-plane request, so backendIdxFromBody host-attribution is
// unchanged.
//
...
//
// # Topology: 2 LIVE backends (runner-spawned GRPCHealthResponder) + 1 DEAD host
//
//   - backend0 → c_hc endpoint 0 (LIVE; h2c; gRPC Check → SERVING; data → 200)
//   - backend1 → c_hc endpoint 1 (LIVE; h2c; gRPC Check → SERVING; data → 200)
...
// GRPCHealthResponder backends). The driver allocates a host port, binds it to
// learn the number, then CLOSES the listener so the port stays unbound for the
// run — both sides reference that same port number (reference via
// host.docker.internal:<dead>, subject via 127.0.0.1:<dead>), and a probe to it
// is refused on both sides.
//
// # Cluster shape (both sides)
//
//		c_hc: H2 (HttpProtocolOptions/http2_protocol_options), lb_policy ROUND_ROBIN,
//		      3 endpoints, health_checks: [{
...
//
// # The driver: poll-to-converge (the determinism mechanism, inherited from 0067)
//
...
//	    upstream_rq_5xx==0 both sides; upstream_cx_active is a PER-SIDE exact pin
//	    (Task 10 cx_active experiment, 20/20 zero-flap): the H2 upstream's idle-pool
//	    retention differs per side — reference keeps one pooled connection per live
//	    host (==backendCount), subject tears them down (==0) — so NOT a cross-side
//	    equality but a wantRef/wantSubj pin (the 0053-kafka-requests precedent); both
//	    values <= backendCount so NO connection is held to the filtered dead host.
//
// # Cross-references
//
//   - phase 39.2 SPEC §8.2 / PLAN Task 9 (the keystone fixture design).
//   - 0067-health-check-tcp (the TCP HC shape this fixture is copied from; the
//     healthChecksBlock changes to grpc_health_check{}, the cluster gains
//     http2_protocol_options, and the backend kind becomes GRPCHealthResponder).
//   - 0021-http-ext-authz-grpc (the H2-upstream cluster YAML shape: the
...
//     host.docker.internal, subject STATIC / 127.0.0.1; scrapeStats; the
//     Bootstrap/Config builders).
...
//   - reference_health_check_propagation_warmup (membership_healthy converges
//     before worker host-sets drop the dead host → poll-the-gauge + warmup-until-
//     K-consecutive-200s + DELTA per-request counters).
//   - reference_differential_run_selector (target -run 'TestDifferential/0068').
...
const (
	fixtureName = "0068-health-check-grpc"

	// In-container reference Envoy listener port for l_http. Fixtures run
	// sequentially; a distinct value avoids confusion — 0066 takes 19155,
	// 0067 takes 19156, this takes 19157.
	refContainerListenerPort = 19157

	refAdminPort = 9901

	// backendCount is the number of LIVE runner-spawned GRPCHealthResponder
	// backends. The DEAD host is a separately-allocated unbound port (NOT a
	// runner backend).
	backendCount = 2
	...
	// Per-side upstream_cx_active pin (Task 10 cx_active experiment — 20/20 runs,
	// zero flap). The H2 data plane multiplexes over a connection pool whose
	// idle-retention policy legitimately DIFFERS per implementation: the reference
	// Envoy keeps one persistent pooled H2 connection per LIVE host (== 2), whereas
	// envoy-go's subject pool has already torn the idle connections down (== 0).
	// Both are valid H2 quiescence states, so this is NOT a cross-side equality but
	// a PER-SIDE exact pin (the 0053-kafka-requests wantRef/wantSubj precedent).
	// Each value equals backendCount (ref) / 0 (subj) — never exceeds backendCount,
	// i.e. NO connection is ever held to the filtered dead host (a 3 on either side
	// would mean the dead host got pooled), so it still bites a dead-host-pooling
	// regression while pinning each side's real, observed-stable behavior.
	wantCxActiveRef  = backendCount // reference: one pooled H2 conn per live host
	wantCxActiveSubj = 0            // subject: idle H2 pool already torn down
)
...
func (*hcDriver) BackendKind() fixture.BackendKind { return fixture.GRPCHealthResponder }
...
// sides — NAT-transparent static config). One gRPC checker (grpc.health.v1
// Check over H2, empty body → default unnamed service ""), fast convergence
// (interval/timeout 0.5s, thresholds 1/1).
...
// h2ProtocolOptions makes the cluster speak cleartext-H2/h2c upstream (REQUIRED
// by the gRPC checker — Task 7's reject — and used for the forwarded data-plane
// request too). The 0021 gRPC-upstream shape, verbatim. Identical on both sides.
const h2ProtocolOptions = `      typed_extension_protocol_options:
...
func (d *hcDriver) ReferenceBootstrap(backendPorts []int) string {
	...
	// 2 live backends + the dead host, H2 upstream, with the active gRPC checker.
      ...
      load_assignment:
        ...
        endpoints:
          - lb_endpoints:
              ...
              - endpoint: { address: { socket_address: { address: host.docker.internal, port_value: %d } } }
              - endpoint: { address: { socket_address: { address: host.docker.internal, port_value: %d } } }
`, refAdminPort, refContainerListenerPort, routeTable, h2ProtocolOptions, healthChecksBlock, backendPorts[0], backendPorts[1], dead)
}
...
func (d *hcDriver) SubjectConfig(_, subjListenerPort int, backendPorts []int, subjAdminPort int) string {
	...
	// + the dead host, H2 upstream, with the active gRPC checker.
	return fmt.Sprintf(`
node: { id: envoy-go-subject-0068, cluster: envoy-go-differential }
admin:
  address:
      ...
      load_assignment:
        ...
        endpoints:
          - lb_endpoints:
              ...
              - endpoint: { address: { socket_address: { address: 127.0.0.1, port_value: %d } } }
              - endpoint: { address: { socket_address: { address: 127.0.0.1, port_value: %d } } }
`, subjAdminPort, subjListenerPort, routeTable, h2ProtocolOptions, healthChecksBlock, backendPorts[0], backendPorts[1], dead)
}
...
func pollMembershipHealthy(side, adminAddr string) error {
	...
	for {
		...
		if time.Now().After(deadline) {
			return fmt.Errorf("%s: cluster.c_hc.membership_healthy did not converge to %d within %s (last seen %d) — dead host not detected? (live backends speak h2c gRPC Check → SERVING? dead host actually refuses? subject checker started post-Freeze?)",
...
// backendIdxFromBody parses the GRPCHealthResponder canned body "backend-<idx>:<seg>"
// and returns the embedded backend idx (the host-attribution signal).
...
func (d *hcDriver) AssertStats(t fixture.TB, refAdminAddr, subjAdminAddr string) {
	...
	for _, sd := range []struct {
		...
		wantCxActiv uint64 // per-side upstream_cx_active pin (Task 10)
	}{{"reference", ref, refBase, wantCxActiveRef}, {"subject", subj, subjBase, wantCxActiveSubj}} {
		...
		// H2 upstream connection count — a PER-SIDE exact pin (Task 10 cx_active
		// experiment, 20/20 runs, zero flap). Unlike 0067 (HTTP/1.1 Connection:
		// close → each request is a fresh dial that completes → upstream_cx_active
		// always quiesces to 0 on BOTH sides), the H2 data plane multiplexes over a
		// connection pool whose IDLE-RETENTION policy legitimately DIFFERS per
		// implementation: the reference Envoy keeps one persistent pooled H2
		// connection per LIVE host (== 2 = backendCount), whereas envoy-go's subject
		// pool has already torn the idle connections down (== 0). Both are valid H2
		// quiescence states and each is fully STABLE across 20 runs, so this is NOT a
		// cross-side equality but a per-side pin (the 0053-kafka-requests
		// wantRef/wantSubj precedent). Both values are <= backendCount — NO
		// connection is ever held to the filtered dead host (a 3 on either side would
		// mean the dead host got pooled) — so this still bites a build that fails to
		// filter the dead host while faithfully pinning each side's real behavior.
		assertEq(t, sd.side, sd.st, "cluster.c_hc.upstream_cx_active", sd.wantCxActiv)
	}
}
