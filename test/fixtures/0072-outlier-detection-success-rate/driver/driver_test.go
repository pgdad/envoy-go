...
func TestConstants(t *testing.T) {
	...
	if srMinHosts > backendCount {
		t.Errorf("srMinHosts=%d exceeds backendCount=%d (no host eligible → never ejects)", srMinHosts, backendCount)
	}
	if fpMinHosts > backendCount {
		t.Errorf("fpMinHosts=%d exceeds backendCount=%d", fpMinHosts, backendCount)
	}
